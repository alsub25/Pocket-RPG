...
export function createErrorBoundary(engine, {
  enabled = true,
  maxErrors = 25
} = {}) {
  const errors = []
  ...
  function _push(kind, err, extra) {
    ...
    try { engine?.emit?.('engine:error', rec) } catch (_) {}
    try { engine?.log?.error?.('errorBoundary', rec.message, rec) } catch (_) {}
    return rec
  }
  ...
  function buildReport({ includeState = false } = {}) {
    const report = {
      ...
      pluginReport: (() => { try { return engine?.getPluginReport?.() } catch (_) { return null } })(),
      lastEvents: (() => { try { return engine?.events?.getRecords?.() } catch (_) { return null } })(),
      lastCommands: (() => { try { return engine?.commands?.getLog?.() } catch (_) { return null } })(),
      log: (() => { try { return engine?.log?.getRecords?.() } catch (_) { return null } })(),
      errors: errors.slice()
    }
    if (includeState) {
      try { report.state = engine?.getState?.() } catch (_) {}
    }
    return report
  }
