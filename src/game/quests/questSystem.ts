...
export function openElderRowanDialog(state, api = {}) {
    ...
    if (typeof openModal !== 'function') {
        if (!accepted) {
            ...
            flags.metElder = true
            if (state.quests && state.quests.main) state.quests.main.step = 0.25
    ...
    const stepText =
        mainQuest && QUEST_DEFS.main.steps
...
function _getObjectivesFor(def, step) {
    if (!def || !def.objectives) return null
    const s = String(step)
...
export function updateQuestBox(state) {
    ...
    if (!mainForBox && sideForBox.length === 0) {
        ...
        const anyActive =
            (mainQuest && String(mainQuest.status) === 'active') || (Array.isArray(activeSide) && activeSide.length > 0)
    ...
    if (hasValidPin) lines.push('Pinned Quest')


    if (mainForBox) {
        const stepLine =
            (QUEST_DEFS.main.steps && QUEST_DEFS.main.steps[mainForBox.step]) ||
...
export function openQuestJournal(state, api = {}) {
    ...
    openModal('Journal', (body) => {
        ...
        try {
            const pinRaw = state.quests ? state.quests.pinned : null
            let pin = null
            if (pinRaw && typeof pinRaw === 'object') pin = pinRaw
            else if (typeof pinRaw === 'string' && pinRaw.trim()) {
                pin = pinRaw.trim().toLowerCase() === 'main' ? { kind: 'main' } : { kind: 'side', id: pinRaw.trim() }
            }
            if (pin) {
                ...
                let label = ''
                if (pin.kind === 'main') label = 'Pinned: Main Quest'
        ...
        const makeQuestDetails = ({
            ...
            try {
                ...
                if (isActive && (questKind === 'main' || questKind === 'side')) {
                    ...
                    const pinned =
                        (questKind === 'main' && pin && pin.kind === 'main') ||
                    ...
                    btn.addEventListener('click', () => {
                        try {
                            if (!state.quests) state.quests = createDefaultQuestState()
                            if (pinned) state.quests.pinned = null
                            else if (questKind === 'main') state.quests.pinned = { kind: 'main' }
                            else state.quests.pinned = { kind: 'side', id: questId }
                        } catch (_) {}
                        try {
                            updateQuestBox(state)
                        } catch (_) {}
...
export function maybeTriggerSideQuestEvent(state, areaId, api = {}) {
    ...
    if (grain && grain.status === 'active') {
        ...
        if (areaId === 'forest' && grain.step === 2) {
            ...
            if (!_areStepObjectivesComplete(grain, def) && rngFloat(state, 'quest.grainThief') < 0.45) {
                ...
                setScene(
                    'Grain Thieves',
                    'Lantern-glints move between trunks. A goblin darts with a bulging pouch — then turns and bares teeth.'
                )
        ...
        if (areaId === 'village' && grain.step === 2) {
            ...
            if (!_areStepObjectivesComplete(grain, def)) {
                ...
                setScene(
                    'Grain – Not Yet Settled',
                    'The keeper’s eyes search your hands. You do not have enough proof yet.\n\n' + lines.join('\n')
                )
    ...
    if (runner && runner.status === 'active') {
        ...
        if (areaId === 'forest' && runner.step === 1 && !runner.banditAmbushStarted) {
            ...
            setScene(
                'Old Road – Bandit Ambush',
                'You step onto the road and the brush explodes with movement. Bandits rush you — and one of them carries a satchel with a village clasp.'
            )
        ...
        if (areaId === 'forest' && runner.step === 2) {
            ...
            if (!_areStepObjectivesComplete(runner, def) && rngFloat(state, 'quest.runnerBandit') < 0.35) {
                setScene(
                    'Old Road – Bandits',
                    'You hear boots on gravel and the jingle of stolen coin. The road is still not safe.'
                )
        ...
        if (areaId === 'village' && runner.step === 2) {
            ...
            if (!_areStepObjectivesComplete(runner, def)) {
                ...
                setScene(
                    'Runner – Not Yet Found',
                    'The family waits for news. You still need to pry answers from the road.\n\n' + lines.join('\n')
                )
...
export function handleExploreQuestBeats(state, areaId, api = {}) {
    ...
    if (areaId === 'village') {
        ...
        if (flags.mainQuestAccepted && mainQuest && !flags.metElder) {
            ...
            setScene(
                'Elder Rowan',
                'Rowan gives you a task — but not a blind one. He urges you to speak with Captain Elara and the Bark‑Scribe before you head into the forest.'
            )
        ...
        if (
            ...
            setScene(
                'Captain Elara',
                'At the guard table, Captain Elara unrolls a torn ledger and points to the missing lines.'
            )
        ...
        if (
            ...
            setScene(
                'Elder Rowan',
                'Rowan hears the news in silence. Then he draws a circle of ash on the table, as if to show you where the world has started leaking.'
            )
        ...
        if (flags.chapter3IntroQueued && !flags.chapter3IntroShown) {
            ...
            setScene(
                ...
                'A crown without a king has started walking through dreams. The council calls you to the Town Hall.'
            )
            ...
            openModal &&
                openModal('The Ritual Question', (body) => {
                    ...
                    const pick = (who) => {
                        ...
                        if (mainQuest) mainQuest.step = 21
        ...
        if (
            ...
            setScene(
                ...
                'A summons arrives written in bark‑grain and frost‑rime. The forest convenes a court — and it names you as witness and accused.'
            )
                ...
                openModal('Chapter IV: The Rootbound Court', (body) => {
                    ...
                    actions.appendChild(
                        makeActionButton(
                            'Begin the hunt',
                            () => {
                                if (mainQuest) mainQuest.step = 24
                                updateQuestBox(state)
                                saveGame && saveGame()
                                closeModal && closeModal()
                            },
        ...
        if (flags.obsidianKingDefeated && !flags.blackbarkChapterStarted) {
            ...
            if (mainQuest && mainQuest.step === 7 && !flags.rowanDebriefShown) {
                ...
                setScene(
                    'Elder Rowan',
                    'Rowan listens without blinking. When you finish, he reaches for an old cedar box — like he has been waiting to need it.'
                )
            ...
            if (mainQuest && mainQuest.step === 8) {
                ...
                setScene(
                    'The Blackbark Oath',
                    'Rowan reveals the Blackbark Oath — but Captain Elara insists you learn what the goblins are counting before you chase ink in the tavern.'
                )
            ...
            if (mainQuest && Number(mainQuest.step) === 8.25 && !flags.ch2ElaraTalliesShown) {
                ...
                setScene(
                    'Tallies in the Dirt',
                    'Captain Elara shows you the goblins’ marks — and why the village fears being counted.'
                )
    ...
    if (areaId === 'forest') {
        ...
        if (mainQuest && mainQuest.status !== 'completed' && mainQuest.step === 16 && !flags.chapter3CrownEchoTaken) {
            ...
            setScene(
                'Night at the Blackbark Gate',
                'The seam between trees is wider tonight — as if it heard your footsteps hours ago and made room for them.'
            )
        ...
        if (mainQuest && mainQuest.status !== 'completed' && mainQuest.step === 21 && flags.chapter3RitualAllyChosen && !flags.hollowRegentDefeated) {
            ...
            setScene(
                'Return to the Gate',
                'You stand with your chosen ally. The forest is quiet — not peaceful. Waiting.'
            )
        ...
        if (mainQuest && mainQuest.status !== 'completed' && mainQuest.step === 29 && !flags.chapter4MagistrateDefeated) {
            ...
            setScene(
                'The Gate as Tribunal',
                'The Blackbark Gate stands open like a mouth. Paper‑thin shadows gather into robed shapes. A Magistrate steps forward, woven from root and rule.'
            )
        ...
        if (mainQuest && mainQuest.status === 'active' && !flags.goblinBossDefeated) {
            ...
            if (s === 1.1 && !_areStepObjectivesComplete(mainQuest, QUEST_DEFS.main)) {
                if (!flags.ch1SnarelineFound) {
                    ...
                    setScene(
                        'Snareline',
                        'A too-straight path through brush: rope fibers snagged on thorns. A trapper is near.'
                    )
            ...
            if (s === 1.25 && !_areStepObjectivesComplete(mainQuest, QUEST_DEFS.main)) {
                if (rngFloat(state, 'quest.main.ch1.cache.bias') < 0.85) {
                    setScene(
                        'Stolen Cache',
                        'Behind a thorn wall, stolen crates are stacked like a shrine. A packmaster watches them like treasure.'
                    )
            ...
            if (s === 1.3 && !_areStepObjectivesComplete(mainQuest, QUEST_DEFS.main)) {
                if (rngFloat(state, 'quest.main.ch1.drums.bias') < 0.85) {
                    setScene(
                        'War Drums',
                        'A pounding rhythm moves through the trunks — not sound, but instruction. The drummer is close.'
                    )
            ...
            if (s === 1.4 && !_areStepObjectivesComplete(mainQuest, QUEST_DEFS.main)) {
                if (rngFloat(state, 'quest.main.ch1.captain.bias') < 0.85) {
                    setScene(
                        'Captain’s Trail',
                        'Boot prints — actual boots, not bare goblin feet — lead to a small command fire.'
                    )
    ...
    if (areaId === 'ruins') {
        ...
        if (mainQuestAccepted && !flags.dragonDefeated) {
            ...
            if (mainQuest && mainQuest.status !== 'completed' && Number(mainQuest.step) < 2) return false
        ...
        if (mainQuest && mainQuest.status !== 'completed' && mainQuest.step === 18 && !flags.chapter3StarIronPin) {
            setScene(
                'Mirror Warden',
                'At the spire’s shattered crown, a figure of polished voidglass watches you — reflecting not your face, but your choices.'
            )
        ...
        if (mainQuest && mainQuest.status !== 'completed' && mainQuest.step === 24 && !flags.chapter4VerdantLens) {
            setScene(
                'Echo Archivist',
                'Among the spire’s broken shelves, a clerk made of glass‑ink and old breath turns a page that is not there. The record is being edited.'
            )
    ...
    if (areaId === 'frostpeak') {
        ...
        if (mainQuest && mainQuest.status !== 'completed' && mainQuest.step === 26 && !flags.chapter4FrozenWrit) {
            setScene(
                'Ice Censor',
                'A figure of frost‑paper stands on the ridge, stamping seals into the wind. Every breath feels like a verdict.'
            )
    ...
    if (areaId === 'catacombs') {
        ...
        if (mainQuest && mainQuest.status !== 'completed' && mainQuest.step === 27 && !flags.chapter4BoneWrit) {
            setScene(
                'Bone Notary',
                'In a drowned chapel, a scribe made of bone‑splinters stamps seals into wet stone. It has recorded too many deaths to forget yours.'
            )
        ...
        if (mainQuest && mainQuest.status !== 'completed' && mainQuest.step === 19 && flags.lichDefeated && !flags.chapter3GraveLatch) {
            setScene(
                'The Grave‑Latch',
                'Past the drowned sanctum, stone ribs form a doorway no map remembers. A lock made of bone clicks once — and something answers.'
            )
    ...
    if (areaId === 'keep') {
        ...
        if (mainQuest && mainQuest.status !== 'completed' && mainQuest.step === 28 && !flags.chapter4SealOfVerdict) {
            setScene(
                'Oathbinder',
                'An armored adjudicator blocks the hall — etched with seals that bleed voidlight. It does not ask why you came. It already wrote the answer.'
            )
    ...
    // --- OATHGROVE ---------------------------------------------------------
    if (areaId === 'oathgrove') {
        if (maybeTriggerSideQuestEvent(state, 'oathgrove', api)) return true
        ...
        if (mainQuest && mainQuest.status !== 'completed' && Number(mainQuest.step) === 10.75) {
            ...
            if (!_areStepObjectivesComplete(mainQuest, QUEST_DEFS.main)) {
                ...
                if (!wardenKilled && rngFloat(state, 'quest.main.oathgrove.warden') < (resin >= 2 ? 0.65 : 0.25)) {
                    setScene(
                        'Sapbound Warden',
                        'The grove shudders. A guardian rises from fused roots and oath‑sap — a Warden that never learned to stop keeping score.'
                    )
    ...
    if (areaId === 'blackbarkDepths') {
        if (maybeTriggerSideQuestEvent(state, 'blackbarkDepths', api)) return true
    ...
    if (areaId === 'starfallRidge') {
        if (maybeTriggerSideQuestEvent(state, 'starfallRidge', api)) return true
        ...
        if (mainQuest && mainQuest.status !== 'completed' && Number(mainQuest.step) === 17.5) {
            ...
            if (!_areStepObjectivesComplete(mainQuest, QUEST_DEFS.main)) {
                ...
                if (shards >= 2 && !flags.starfallSentinelDefeated && rngFloat(state, 'quest.main.starfall.sentinel') < 0.65) {
                    ...
                    setScene(
                        'Falling Star Sentinel',
                        'A plated shape rises from the crater-dust, eyes burning with pale meteor-light. It moves like a verdict with legs.'
                    )
...
export function applyQuestProgressOnItemGain(state, itemId, quantity = 1, api = {}) {
    ...
    if (Array.isArray(triggers) && triggers.length) {
        ...
        triggers.forEach((t) => {
            ...
            if (!q || q.status !== 'active') return
            if (Number(q.step) !== Number(t.step)) return
        ...
        const applyToQuest = (q, def) => {
            ...
            for (let i = 0; i < objs.length; i++) {
                const o = objs[i]
                if (!o || o.type !== 'collect') continue
                if (String(o.itemId || '') !== String(itemId)) continue
        ...
        if (main && main.status === 'active') applyToQuest(main, QUEST_DEFS.main)

        const side = state.quests.side || {}
        Object.values(side).forEach((q) => {
            const def = q && q.id ? QUEST_DEFS.side[q.id] : null
            if (def) applyToQuest(q, def)
        })
    }
        ...
        try {
            ...
            if (
                ...
                setScene &&
                    setScene(
                        'Marsh Writs Secured',
                        'The bailiffs fall apart into damp parchment. Two writs remain — stamped, wet, and angry. The Court points north.'
                    )
            ...
            if (
                ...
                setScene &&
                    setScene(
                        'Star‑Iron Gathered',
                        'The shards bite cold into your palm. Enough star‑iron to take a Pin from the Spire. Return to the Ruined Spire.'
                    )
    ...
    try {
        ...
        if (id === 'oathSplinterSapRun') splinterChanged = markSplinter('oathShardSapRun', 'Sap‑Run') || splinterChanged
        if (id === 'oathSplinterWitchReed') splinterChanged = markSplinter('oathShardWitchReed', 'Witch‑Reed') || splinterChanged
        if (id === 'oathSplinterBoneChar') splinterChanged = markSplinter('oathShardBoneChar', 'Bone‑Char') || splinterChanged
        ...
        if (splinterChanged && hasAllOathSplinters(state)) {
            ...
            if (typeof setScene === 'function' && !f.allOathSplintersSceneShown) {
                ...
                setScene(
                    'Oath‑Splinters Gathered',
                    'The three splinters warm in your palm at once, like a heartbeat syncing to yours. The forest has noticed.'
                )
            }
            updateQuestBox(state)
            saveGame && saveGame()
...
export function applyQuestProgressOnEnemyDefeat(state, enemy, api = {}) {
    ...
    const applyCollectDropsToQuest = (q, def) => {
        ...
        for (let i = 0; i < objs.length; i++) {
            ...
            if (o.type !== 'collect' || !o.itemId || !addItemToInventory) continue
            const ids = Array.isArray(o.dropsFrom) ? o.dropsFrom : null
            if (!ids || !ids.includes(enemy.id)) continue
    ...
    Object.values(side).forEach((q) => {
        const def = q && q.id ? QUEST_DEFS.side[q.id] : null
        if (def) applyCollectDropsToQuest(q, def)
    })
    ...
    try {
        ...
        if (enemy.id === 'starfallSentinel') {
            ...
            if (addItemToInventory) addItemToInventory('starIronShard', 1)
        ...
        if (
            ...
            setScene &&
                setScene(
                    'Quiet Ink Secured',
                    'The resin is yours. The splinters stop tugging apart — as if the oath has been forced to listen. Return to Emberwood for the Quiet Roots Trial.'
                )
        ...
        if (
            ...
            setScene &&
                setScene(
                    'Marks in the Dark',
                    'Your charcoal rubbings show the same crown‑shape repeated — not a symbol of rule, but of ownership. Something is preparing a ritual at the gate. Return at Night.'
                )
    ...
    if (enemy.id === 'goblinBoss') {
        ...
        if (mainQuest) mainQuest.step = Math.max(Number(mainQuest.step) || 0, 1.5)
    ...
    if (enemy.id === 'crownShade') {
        ...
        if (mainQuest) mainQuest.step = Math.max(mainQuest.step || 0, 17)
        addLog && addLog('You seize the Crown‑Echo. It is cold as a promise kept by someone else.', 'good')
        setScene &&
            setScene(
                'Crown‑Echo',
                'The thing you took is not an object. It is a memory with teeth. Bring it to the Bark‑Scribe.'
            )
        updateQuestBox(state)
        return true
    }
    ...
    if (enemy.id === 'mirrorWarden') {
        ...
        if (mainQuest) mainQuest.step = Math.max(mainQuest.step || 0, 19)
        addLog && addLog('The Mirror Warden shatters. In the debris: a Star‑Iron Pin that refuses to rust.', 'good')
        setScene &&
            setScene(
                'Star‑Iron Pin',
                'A cold weight that can anchor a vow — or a lie. The next piece waits in the catacombs.'
            )
        updateQuestBox(state)
        return true
    }
    ...
    if (enemy.id === 'graveLatchWarden') {
        ...
        if (mainQuest) mainQuest.step = Math.max(mainQuest.step || 0, 20)
        addLog && addLog('The Grave‑Latch breaks free. The lock yields — and the catacombs exhale.', 'good')
        setScene &&
            setScene(
                'Grave‑Latch',
                'You carry a key that was never meant to be carried. Return to Emberwood and decide who leads the ritual.'
            )
        updateQuestBox(state)
        return true
    }
    ...
    if (enemy.id === 'hollowRegent') {
        ...
        if (mainQuest) mainQuest.step = Math.max(mainQuest.step || 0, 22)
        addLog && addLog('The Hollow Regent falls. The crown hits the ground without sound.', 'good')
        setScene &&
            setScene(
                'The Hollow Crown',
                'The forest is listening, and for the first time it does not interrupt. Return to Emberwood and choose what happens next.'
            )
        updateQuestBox(state)
        return true
    }
    ...
    if (enemy.id === 'echoArchivist') {
        ...
        if (mainQuest) mainQuest.step = Math.max(mainQuest.step || 0, 25)
        addLog && addLog('The Archivist breaks. In the shards: a Verdant Lens that makes lies flare green.', 'good')
        setScene &&
            setScene(
                'Verdant Lens',
                'The lens is cold and damp, like leaf‑mold. Through it, even your own thoughts look edited. The Court will want writs next.'
            )
        updateQuestBox(state)
        return true
    }
    ...
    if (enemy.id === 'iceCensor') {
        ...
        if (mainQuest) mainQuest.step = Math.max(mainQuest.step || 0, 27)
        addLog && addLog('The Ice Censor fractures. A Frozen Writ slides from its sleeve like a confession.', 'good')
        setScene &&
            setScene(
                'Frozen Writ',
                'The paper is rimed and rigid. The ink does not run — it bites. One writ remains below, where the dead keep records.'
            )
        updateQuestBox(state)
        return true
    }
    ...
    if (enemy.id === 'boneNotary') {
        ...
        if (mainQuest) mainQuest.step = Math.max(mainQuest.step || 0, 28)
        addLog && addLog('The Bone Notary crumbles. Its stamp remains: a Bone Writ, signed in silence.', 'good')
        setScene &&
            setScene(
                'Bone Writ',
                'You carry the last writ like a weight. The Court’s chain now points to the keep — where oaths are forged into shackles.'
            )
        updateQuestBox(state)
        return true
    }
    ...
    if (enemy.id === 'oathBinder') {
        ...
        if (mainQuest) mainQuest.step = Math.max(mainQuest.step || 0, 29)
        addLog && addLog('The Oathbinder falls. A Seal of Verdict clicks open in your palm.', 'good')
        setScene &&
            setScene(
                'Seal of Verdict',
                'With writs and seal, the Court can be summoned — or challenged. Return to the Blackbark Gate at Night.'
            )
        updateQuestBox(state)
        return true
    }
    ...
    if (enemy.id === 'rootboundMagistrate') {
        ...
        if (mainQuest) mainQuest.step = Math.max(mainQuest.step || 0, 30)
        addLog && addLog('The Magistrate unravels. Its sigil remains, warm with judgement.', 'good')
        setScene &&
            setScene(
                'Magistrate’s Sigil',
                'The Court has been forced to listen. Return to Emberwood and decide how you answer the verdict.'
            )
        updateQuestBox(state)
        return true
    }
