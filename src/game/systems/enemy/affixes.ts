...
function _applySingleEnemyAffix(enemy, def) {
    if (!enemy || !def || !def.id) return
    if (!Array.isArray(enemy.affixes)) enemy.affixes = []
    if (enemy.affixes.includes(def.id)) return

    enemy.affixes.push(def.id)

    // Runtime knobs (kept as flat fields for speed + save resilience)
    if (def.thornsReflectPct) enemy.affixThornsPct = Math.max(enemy.affixThornsPct || 0, def.thornsReflectPct)
    if (def.vampiricHealPct) enemy.affixVampiricHealPct = Math.max(enemy.affixVampiricHealPct || 0, def.vampiricHealPct)
    if (def.regenPct) enemy.affixRegenPct = Math.max(enemy.affixRegenPct || 0, def.regenPct)

    if (def.chillChance) enemy.affixChillChance = Math.max(enemy.affixChillChance || 0, def.chillChance)
    if (def.chillTurns) enemy.affixChillTurns = Math.max(enemy.affixChillTurns || 0, def.chillTurns)

    if (def.hexTurns) enemy.affixHexTurns = Math.max(enemy.affixHexTurns || 0, def.hexTurns)
    if (def.hexAtkDown) enemy.affixHexAtkDown = Math.max(enemy.affixHexAtkDown || 0, def.hexAtkDown)
    if (def.hexArmorDown) enemy.affixHexArmorDown = Math.max(enemy.affixHexArmorDown || 0, def.hexArmorDown)
    if (def.hexResDown) enemy.affixHexResDown = Math.max(enemy.affixHexResDown || 0, def.hexResDown)

    if (def.berserkThreshold) enemy.affixBerserkThreshold = Math.max(enemy.affixBerserkThreshold || 0, def.berserkThreshold)
    if (def.berserkAtkPct) enemy.affixBerserkAtkPct = Math.max(enemy.affixBerserkAtkPct || 0, def.berserkAtkPct)
...
export function applyEnemyAffixes(enemy, opts = {}, ctx = {}) {
    ...
    if (force) {
        ...
        if (tier >= 3) baseChance += 0.08 // Rare+
        if (tier >= 4) baseChance += 0.18 // Epic+
        if (tier >= 5) baseChance += 0.10 // Legendary+
        if (tier >= 6) baseChance += 0.06 // Mythic+
        ...
        if (enemy.isElite) {
            ...
            if (tier >= 4) extraChance += 0.10
            if (tier >= 6) extraChance += 0.05
            if (rand('affix.extra') < extraChance) count += 1
        } else if (rolled) {
            count = 1
            let extraChance = (enemy.level || 1) >= 14 ? 0.10 : 0.06
            extraChance += 0.01 * clampFinite(band, -2, 6, 0)
            if (tier >= 3) extraChance += 0.04
            if (tier >= 4) extraChance += 0.06
            if (tier >= 6) extraChance += 0.04
            if (rand('affix.extra') < extraChance) count += 1
        }

        // Epic+ enemies should almost always have at least one modifier outside the village.
        if (tier >= 4 && count < 1) count = 1
        if (tier >= 5 && count < 1) count = 1
    }
    ...
    for (let i = 0; i < count; i++) {
        ...
        const eligible = ENEMY_AFFIX_DEFS.filter((a) => {
            ...
            if (picked.find((p) => p && p.id === a.id)) return false
...
export function getEnemyAffixLabels(enemy) {
    if (!enemy) return []
    if (Array.isArray(enemy.affixLabels) && enemy.affixLabels.length) return enemy.affixLabels
    if (!Array.isArray(enemy.affixes)) return []
...
export function applyEnemyAffixesOnEnemyHit(enemy, player, info = {}, ctx = {}) {
    if (!enemy || !player || !player.status) return
    ...
    if (enemy.affixVampiricHealPct && enemy.affixVampiricHealPct > 0 && hpDamage > 0) {
        ...
        if (gained > 0) addLog(enemy.name + ' siphons ' + gained + ' HP.', 'system')
    }
    ...
    if (enemy.affixHexTurns && enemy.affixHexTurns > 0) {
        ...
        if (hpDamage > 0 && rand('affix.hex.log') < 0.25) addLog('A lingering hex weakens you.', 'system')
    }
}
