...
export function createUiComposeBridgePlugin() {
  return {
    ...
    init(engine) {
      ...
      if (!uiCompose || typeof uiCompose.setAdapter !== 'function') return

      try {
        uiCompose.setAdapter({
          toast: (msg, opts) => { try { showToast(msg, opts) } catch (_) {} },
          setBusy: (busy, opts) => { try { setBusyOverlay(busy, opts) } catch (_) {} },
          transition: (name, opts) => { try { applyUiTransition(name, opts) } catch (_) {} },
          clearTransition: (name) => { try { applyUiTransition(null, { clear: true, name }) } catch (_) {} },
          setHudState: (st) => { try { applyHudState(st) } catch (_) {} }
        })
      } catch (_) {}
    }
  }
