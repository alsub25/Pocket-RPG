...
export function createSimTickPlugin({ getState, runDailyTicks } = {}) {
    return {
        ...
        start(engine) {
            ...
            const check = () => {
                ...
                if (typeof runDailyTicks === 'function') {
                    for (let d = last + 1; d <= day; d++) {
                        try { runDailyTicks(state, d) } catch (_) {}
                    }
                }

                state.sim.lastDailyTickDay = day
                try { engine.emit('sim:dailyTicksCatchUp', { from: last, to: day }) } catch (_) {}
            }
            ...
            try { engine.on('save:loaded', check) } catch (_) {}
        ...
        stop(engine) {
            try { if (engine && engine.__ewSimTickCheck) engine.off('save:loaded', engine.__ewSimTickCheck) } catch (_) {}
            try {
            ...
            try { delete engine.__ewSimTickCheck } catch (_) {}
        }
    }
}
