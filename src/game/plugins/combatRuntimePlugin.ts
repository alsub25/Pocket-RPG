...
export function createCombatRuntimePlugin(opts = {}) {
  ...
  return {
    ...
    init(engine) {
      ...
      try { engine.registerService('combat', combat) } catch (_) {}
      try { engine.registerService('combat.math', combatMath) } catch (_) {}
      try { engine.registerService('combat.status', statusEngine) } catch (_) {}
      try { engine.registerService('combat.turnSequencer', turnSequencer) } catch (_) {}
    },
    dispose() {
      combatMath = null
      statusEngine = null
