...
export function createUiRuntimePlugin(opts = {}) {
  ...
  return {
    ...
    init(engine) {
      ...
      try { uiRuntime.configureUI({ ...uiConfig, engine }) } catch (_) {}

      // Expose the UI runtime via services so other plugins can depend on it.
      try { engine.registerService('ui', uiRuntime) } catch (_) {}
      try { engine.registerService('ui.bindings', { initUIBindings }) } catch (_) {}
    },
    start() {
      ...
      const install = () => {
        try {
          if (!_engine || !_engine.ui || typeof _engine.ui.setAdapter !== 'function') return
          _engine.ui.setAdapter({
            open: (_id, props = {}) => {
              ...
              if (builderFn) return uiRuntime.openModalDom(title, builderFn, { owner })
