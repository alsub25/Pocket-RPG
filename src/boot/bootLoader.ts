...
function ensure() {
  ...
  if (!root) return { root: null, fill: null, text: null };
...
function show(msg = 'Loading…') {
  ...
  try { root.setAttribute('aria-busy', 'true'); } catch (_) {}
...
function hide() {
  ...
  try { root.setAttribute('aria-busy', 'false'); } catch (_) {}
}

function nextFrame() {
  return new Promise((resolve) => {
    try { requestAnimationFrame(() => resolve()); } catch (_) { setTimeout(resolve, 0); }
  });
}
