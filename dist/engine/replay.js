...
export function createReplay(engine, { owner = 'system:replay', maxCommands = 3000 } = {}) {
    ...
    function startRecording({ name = 'replay', includeSnapshot = true } = {}) {
        ...
        const seed = (() => {
            try {
                return engine?.rng?.getSeed?.() ?? engine?.rng?.rootSeed ?? null;
            }
            catch (_) {
                return null;
            }
        ...
        const snapshot = (includeSnapshot && engine?.save) ? (() => {
            try {
                return engine.save({ kind: 'replay', name });
            }
            catch (_) {
                return null;
            }
        ...
        try {
            engine?.on?.('command:dispatched', _listener);
        }
        catch (_) { }
        try {
            engine?.emit?.('replay:recordingStart', { t: _nowIso(), name: _tape.name });
        }
        catch (_) { }
        return true;
    }
    function stopRecording() {
        ...
        try {
            if (_listener)
                engine?.off?.('command:dispatched', _listener);
        }
        catch (_) { }
        _listener = null;
        if (_tape) {
            ...
            try {
                engine?.emit?.('replay:recordingStop', { t: _tape.stoppedAt, name: _tape.name, commands: _tape.commands.length });
            }
            catch (_) { }
            _lastTape = _tape;
        }
    ...
    async function play(tape, { immediate = false, intervalMs = 0 } = {}) {
        const t = tape || _lastTape;
        if (!t)
            return false;
        try {
            engine?.emit?.('replay:playStart', { t: _nowIso(), name: t.name });
        }
        catch (_) { }
        // Load snapshot (if present)
        if (t.snapshot && engine?.load) {
            try {
                engine.load(t.snapshot, { reason: 'replay' });
            }
            catch (_) { }
        }
        // Apply seed
        ...
        if (!cmds.length) {
            try {
                engine?.emit?.('replay:playEnd', { t: _nowIso(), name: t.name, commands: 0 });
            }
            catch (_) { }
            return true;
        }
        ...
        if (immediate || !engine?.schedule?.after || intervalMs <= 0) {
            for (let i = 0; i < cmds.length; i++)
                dispatchOne(cmds[i]);
            try {
                engine?.emit?.('replay:playEnd', { t: _nowIso(), name: t.name, commands: cmds.length });
            }
            catch (_) { }
            return true;
        }
