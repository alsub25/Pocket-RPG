...
export function createInputRouter({ emit = null, logger = null } = {}) {
    ...
    function removeContext(id) {
        ...
        for (let i = contexts.length - 1; i >= 0; i--) {
            if (contexts[i] && contexts[i].id === target)
                contexts.splice(i, 1);
        }
    }
    ...
    function trigger(action, payload = {}) {
        ...
        try {
            if (emit)
                emit('input:action', { action: a, payload });
        }
        catch (_) { }
    ...
    function _resolveKey(key) {
        ...
        if (!k)
            return { action: null, contextId: null };
        for (let i = contexts.length - 1; i >= 0; i--) {
            const ctx = contexts[i];
            const act = ctx && ctx.keyBindings ? ctx.keyBindings.get(k) : null;
            if (act)
                return { action: act, contextId: ctx.id };
        }
