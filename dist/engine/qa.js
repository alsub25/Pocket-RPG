...
function _redactProps(obj, { maxKeys = 60, maxStr = 500 } = {}) {
    ...
    for (let i = 0; i < keys.length; i++) {
        ...
        const v = obj[k];
        if (typeof v === 'string')
            out[k] = (v.length > maxStr) ? (v.slice(0, maxStr - 1) + '…') : v;
        else if (v && typeof v === 'object')
            out[k] = _redactProps(v, { maxKeys: 40, maxStr });
        else
            out[k] = v;
    }
    return out;
}
export function createQaService(engine, { maxTail = 140 } = {}) {
    ...
    function _svc(name) {
        try {
            return engine?.getService?.(name) || engine?.[name] || null;
        }
        catch (_) {
            return null;
        }
    ...
    function buildEngineQaTail() {
        ...
        const commandTail = (() => {
            try {
                return _tail(commands?.getLog?.(), maxTail);
            }
            catch (_) {
                return [];
            }
        ...
        const eventTail = (() => {
            try {
                return _tail(events?.getRecords?.(), maxTail);
            }
            catch (_) {
                return [];
            }
        ...
        const perfTail = (() => {
            try {
                return _tail(perf?.getRecords?.(), maxTail);
            }
            catch (_) {
                return [];
            }
        ...
        const replayMeta = (() => {
            try {
                return replay?.getLastMeta?.() || null;
            }
            catch (_) {
                return null;
            }
        ...
        return {
            ...
            locale: (() => { try {
                return i18n?.getLocale?.() || null;
            }
            catch (_) {
                return null;
            } })(),
            flags: (() => { try {
                return flags?.dump?.() || null;
            }
            catch (_) {
                return null;
            } })(),
            savePolicy: (() => { try {
                return savePolicy?.getStatus?.() || null;
            }
            catch (_) {
                return null;
            } })(),
            telemetryTail: (() => { try {
                return _tail(telemetry?.getTail?.(maxTail) || [], maxTail);
            }
            catch (_) {
                return null;
            } })(),
    ...
    function buildBugReportBundle() {
        ...
        const base = (() => {
            try {
                if (typeof hooks.buildBugReportBundle === 'function')
                    return hooks.buildBugReportBundle();
            }
            catch (_) { }
    ...
    function formatBugReportBundle(bundle) {
        try {
            if (typeof hooks.formatBugReportBundle === 'function')
                return hooks.formatBugReportBundle(bundle);
        }
        catch (_) { }
        try {
    ...
    function runIntegrityAudit(state, reason) {
        try {
            return typeof hooks.runIntegrityAudit === 'function' ? hooks.runIntegrityAudit(state, reason) : null;
        }
        catch (_) {
            return null;
        }
    ...
    function formatIntegrityAuditReport(rep) {
        try {
            return typeof hooks.formatIntegrityAuditReport === 'function' ? hooks.formatIntegrityAuditReport(rep) : JSON.stringify(rep, null, 2);
        }
        catch (_) {
            return String(rep || '');
        }
    ...
    function runScenarioRunner(opts) {
        try {
            return typeof hooks.runScenarioRunner === 'function' ? hooks.runScenarioRunner(opts) : null;
        }
        catch (_) {
            return null;
        }
    ...
    function formatScenarioRunnerReport(rep) {
        try {
            return typeof hooks.formatScenarioRunnerReport === 'function' ? hooks.formatScenarioRunnerReport(rep) : JSON.stringify(rep, null, 2);
        }
        catch (_) {
            return String(rep || '');
        }
