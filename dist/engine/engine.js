 ...
 * }
 */
export function createEngine({ initialState = null, patch = '', patchName = '', 
// Engine service defaults (safe for file:// + headless).
logger = null, clock = null, rngSeed = null, installErrorBoundary = true, startPerfWatchdog = true, autoInstallInput = true, autoTick = true, autoTickMode = 'raf' } = {}) {
    ...
    function emit(eventName, payload) {
        ...
        for (let i = 0; i < arr.length; i++) {
            try {
                ...
                try {
                    log.warn('events', `listener failed for ${key}`, { e });
                }
                catch (_) { }
            }
        }
    ...
    function _normalizePlugin(plugin) {
        ...
        if (!id)
            throw _engineError('PLUGIN_MISSING_ID', 'Plugin is missing required `id`.', { plugin });
        ...
        for (let i = 0; i < list.length; i++) {
            ...
            if (_engineStarted) {
                for (let j = 0; j < normalized.requires.length; j++) {
                    ...
                    if (!_started.has(dep)) {
                        throw _engineError('PLUGIN_LATE_REGISTER_MISSING_DEP', `Plugin ${id} cannot be registered after engine start because dependency ${dep} is not started.`, { id, dep });
                    }
                }
        ...
        function visit(id) {
            ...
            for (let i = 0; i < p.requires.length; i++) {
                ...
                if (!_pluginById.has(dep)) {
                    throw _engineError('PLUGIN_MISSING_DEP', `Plugin ${id} requires missing dependency ${dep}.`, { id, dep });
                }
                visit(dep);
            }
    ...
    function _startAutoTickLoop() {
        ...
        try {
            _autoTickLastT = (typeof performance !== 'undefined' && performance.now) ? performance.now() : Date.now();
        }
        catch (_) {
            _autoTickLastT = Date.now();
        }
        ...
        function step(now) {
            ...
            if (dt > 250) {
                ...
                try {
                    if (emit)
                        emit('engine:warn', { where: 'autoTick', message: 'Large time delta clamped', dt, clamped: 250 });
                }
                catch (_) { }
                dt = 250;
            }
            try {
                ...
                try {
                    emit('engine:error', { where: 'autoTick', e: String(e && e.message ? e.message : e) });
                }
                catch (_) { }
            }
            ...
            if (_autoTickRunning) {
                if (useRaf) {
                    try {
                        _autoTickHandle = requestAnimationFrame(step);
                    }
                    catch (_) {
    ...
    const _tweenListener = () => { try {
        tween.update();
    }
    catch (_) { } };
    on('clock:tick', _tweenListener);
    own('system:tween', () => { try {
        off('clock:tick', _tweenListener);
    }
    catch (_) { } });
    // ---- Input / UI ----------------------------------------------------------
