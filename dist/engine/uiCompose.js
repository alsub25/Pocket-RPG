...
export function createUiCompose({ emit = null, schedule = null, logger = null } = {}) {
    let _adapter = null;
    ...
    function toast(message, opts = {}) {
        ...
        try {
            emit && emit('uiCompose:toast', payload);
        }
        catch (_) { }
        try {
            _adapter?.toast?.(payload.message, payload.opts);
        }
        catch (e) {
            try {
                logger?.warn?.('uiCompose', 'toast adapter failed', { e });
            }
            catch (_) { }
        }
    }
    function setBusy(isBusy, opts = {}) {
        ...
        try {
            emit && emit('uiCompose:busy', payload);
        }
        catch (_) { }
        try {
            _adapter?.setBusy?.(payload.isBusy, payload.opts);
        }
        catch (e) {
            try {
                logger?.warn?.('uiCompose', 'busy adapter failed', { e });
            }
            catch (_) { }
        }
    }
    function transition(name = 'fade', opts = {}) {
        ...
        try {
            emit && emit('uiCompose:transition', payload);
        }
        catch (_) { }
        try {
            _adapter?.transition?.(payload.name, payload.opts);
        }
        catch (e) {
            try {
                logger?.warn?.('uiCompose', 'transition adapter failed', { e });
            }
            catch (_) { }
        }
        ...
        if (schedule && Number.isFinite(durationMs) && durationMs > 0) {
            try {
                schedule.after(durationMs, () => {
                    try {
                        _adapter?.clearTransition?.(payload.name);
                    }
                    catch (_) { }
    ...
    function setHudState(hudState = {}) {
        ...
        try {
            emit && emit('uiCompose:hud', payload);
        }
        catch (_) { }
        try {
            _adapter?.setHudState?.(payload.hudState);
        }
        catch (e) {
            try {
                logger?.warn?.('uiCompose', 'hud adapter failed', { e });
            }
            catch (_) { }
        }
    }
