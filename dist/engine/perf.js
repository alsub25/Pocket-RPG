...
function _splitLabel(label) {
    ...
    if (!m)
        return { subsystem: null, label: raw };
...
export function _ensurePerfDebug(s) {
    try {
        if (!s || typeof s !== 'object')
            return null;
        if (!s.debug || typeof s.debug !== 'object')
            s.debug = {};
        const d = s.debug;
        if (typeof d.capturePerf !== 'boolean')
            d.capturePerf = false;
        if (!Array.isArray(d.perfLog))
            d.perfLog = [];
        return d;
    }
    catch (_) {
...
export function perfRecord(s, label, ms, extra = null) {
    ...
    try {
        ...
        if (!subsystem && split.subsystem)
            subsystem = split.subsystem;
        ...
        if (d.perfLog.length > 200)
            d.perfLog.splice(0, d.perfLog.length - 200);
    }
    catch (_) { }
}
