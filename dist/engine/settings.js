...
export function createSettingsService({ emit = null, logger = null, storageGet = null, storageSet = null, storageKey = 'locus_settings_v1', schema = 1 } = {}) {
    let _defaults = {};
    let _values = {};
    let _loaded = false;
    ...
    function load() {
        let raw = null;
        try {
            raw = storageGet ? storageGet(storageKey) : null;
        }
        catch (_) {
            raw = null;
        }
        ...
        try {
            if (emit)
                emit('settings:loaded', { key: storageKey });
        }
        catch (_) { }
        return getAll();
    }
    ...
    function set(key, value, { persist = true } = {}) {
        ...
        try {
            if (emit)
                emit('settings:changed', { key: k, value, previous: prev });
        }
        catch (_) { }
        if (persist)
            save();
