...
export function createScheduler(clock, { maxTasks = 10000 } = {}) {
    ...
    function _add(task) {
        if (tasks.size >= maxTasks)
            throw new Error('Scheduler task limit reached');
        tasks.set(task.id, task);
        order.push(task.id);
        try {
            if (task && task.owner)
                _indexOwner(task.owner, task.id);
        }
        catch (_) { }
        return task.id;
    }
    ...
    function cancelOwner(owner) {
        ...
        try {
            if (owners.get(key) && owners.get(key).size === 0)
                owners.delete(key);
        }
        catch (_) { }
        return n;
