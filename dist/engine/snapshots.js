...
export function createSnapshotManager({ getState, setState, getVersion, migrateState, // (state, fromVersion, toVersion) => state
emit = null, logger = null } = {}) {
    function _log(lvl, msg, data) {
        try {
            if (logger && logger[lvl])
                logger[lvl]('snapshots', msg, data);
        }
        catch (_) { }
    }
    function save(meta = {}) {
        ...
        try {
            if (emit)
                emit('save:created', { version, checksum });
        }
        catch (_) { }
        return snap;
    }
    ...
    function load(snapshot, { allowMigrate = true } = {}) {
        ...
        try {
            if (emit)
                emit('save:loaded', { fromVersion: from, toVersion: to });
        }
        catch (_) { }
        return nextState;
    }
