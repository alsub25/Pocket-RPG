...
export function createAssetRegistry({ emit = null, logger = null } = {}) {
    ...
    async function preload(keys = [], { group = null } = {}) {
        ...
        for (let i = 0; i < urls.length; i++) {
            ...
            try {
                ...
                try {
                    if (emit)
                        emit('assets:error', { url, group, error: String(e?.message || e) });
                }
                catch (_) { }
            }
            try {
                if (emit)
                    emit('assets:progress', { group, done: i + 1, total: urls.length, url });
            }
            catch (_) { }
        }
        return results;
    ...
    async function fetchJson(keyOrUrl, { cache = true } = {}) {
        ...
        try {
            ...
            try {
                if (emit)
                    emit('assets:error', { url, error: String(e?.message || e) });
            }
            catch (_) { }
            return null;
        }
