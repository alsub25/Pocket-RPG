...
function wirePillTapHighlight() {
    ...
    ids.forEach((id) => {
        ...
        if (!el)
            return;
        if (el.dataset && el.dataset.pillTapWired)
            return;
        try {
            el.dataset.pillTapWired = "1";
        }
        catch (_) { }
        const fire = () => {
            try {
                ...
                if (el.__pillTapTimer)
                    clearTimeout(el.__pillTapTimer);
...
function diagPush(kind, payload) {
    try {
        ...
        if (d && typeof d === 'object' && Array.isArray(d.errors)) {
            ...
            d.errors.push(entry);
            if (d.errors.length > 80)
                d.errors.splice(0, d.errors.length - 80);
        }
    }
    catch (_) { }
}
...
function shouldRunPreflight() {
    try {
        ...
        if (qp.get('preflight') === '1' || qp.get('diag') === '1')
            return true;
    }
    catch (_) { }
    try {
...
async function loadGameVersion(version, { onFail } = {}) {
    ...
    const mark = (k) => {
        try {
            boot.steps[String(k)] = _bootNow();
        }
        catch (_) { }
    };
    try {
        window.__EW_BOOT_METRICS__ = boot;
    }
    catch (_) { }
    mark('start');
    ...
    if (location.protocol === 'file:') {
        ...
        try {
            ...
            if ((result.missing && result.missing.length) || (result.bad && result.bad.length)) {
                ...
                if (window.PQ_BOOT_DIAG && window.PQ_BOOT_DIAG.renderOverlay)
                    window.PQ_BOOT_DIAG.renderOverlay();
                BootLoader.hide();
                try {
                    boot.ok = false;
                ...
                if (onFail)
                    onFail(new Error('Preflight detected missing modules'));
                return;
            }
            mark('preflightEnd');
        }
...
function showBootstrapModal(titleText, contentNode) {
    ...
    try {
        modal.dataset.owner = "bootstrap";
        modal.dataset.lock = "0";
    }
    catch (_) { }
...
function hideBootstrapModal() {
    ...
    try {
        if (modal.dataset.lock === "1" && modal.dataset.owner && modal.dataset.owner !== "bootstrap")
            return;
    }
    catch (_) { }
    ...
    if (modal.dataset.bootstrapOpen === "1" && (!modal.dataset.owner || modal.dataset.owner === "bootstrap")) {
        ...
        try {
            if (modal.dataset.owner === "bootstrap")
                modal.dataset.owner = "";
        }
        catch (_) { }
        if (body)
...
function initBootstrap() {
    onDocReady(async () => {
        ...
        const loadPromise = (async () => {
            ...
            if (q && picked)
                safeStorageSet(STORAGE_KEY, picked.id);
            if (picked) {
