...
export function createDiagnosticsUI(deps) {
    const { engine, getState, cheatsEnabled, patchLabel, patchName, openModal, closeModal, getModalOnClose, setModalOnClose, copyToClipboard, 
    // QA tools
    runSmokeTests, runIntegrityAudit, formatIntegrityAuditReport, runScenarioRunner, formatScenarioRunnerReport, buildBugReportBundle, 
    // Perf snapshot helpers
    qaReadBootMetrics, qaReadMemorySnapshot, qaSummarizePerfLog, qaCollectPerfSnapshotSync, qaFormatPerfSnapshotText, qaSampleFps } = deps || {};
    function _getReplay() {
        try {
            return engine && (engine.getService ? engine.getService('replay') : engine.replay);
        }
        catch (_) {
            return null;
        }
    ...
    function openSmokeTestsModal() {
        ...
        openModal('Smoke Tests & Bug Report', (b) => {
            ...
            try {
                window.addEventListener('resize', _qaResizeHandler);
            }
            catch (_) { }
            ...
            if (typeof setModalOnClose === 'function') {
                setModalOnClose(() => {
                    try {
                        window.removeEventListener('resize', _qaResizeHandler);
                    }
                    catch (_) { }
                    try {
                        if (typeof _prevModalOnClose === 'function')
                            _prevModalOnClose();
                    }
                    catch (_) { }
                });
            ...
            function refreshReplayStatus() {
                ...
                if (meta) {
                    ...
                    if (meta.startedAt)
                        lines.push('  started: ' + String(meta.startedAt));
                    if (meta.stoppedAt)
                        lines.push('  stopped: ' + String(meta.stoppedAt));
            ...
            btnRec.onclick = () => {
                const r = _getReplay();
                if (!r || !r.startRecording)
                    return;
                try {
                    r.startRecording({ name: 'qa', includeSnapshot: true });
                }
                catch (_) { }
                try {
                    engine?.uiCompose?.toast?.(_t('toast.replay.recording', 'Recording replay…'));
                }
                catch (_) { }
                refreshReplayStatus();
            };
            btnStop.onclick = () => {
                ...
                let tape = null;
                try {
                    tape = r.stopRecording();
                }
                catch (_) { }
                try {
                    lastTapeJson = (r.exportTape && r.exportTape(tape)) || '';
                }
                catch (_) {
                    lastTapeJson = '';
                }
                try {
                    engine?.uiCompose?.toast?.(_t('toast.replay.stopped', 'Replay captured.'));
                }
                catch (_) { }
            ...
            btnCopyTape.onclick = () => {
                if (!lastTapeJson) {
                    const r = _getReplay();
                    try {
                        lastTapeJson = (r && r.exportTape) ? r.exportTape() : '';
                    }
                    catch (_) {
                        lastTapeJson = '';
                    }
            ...
            btnPlay.onclick = () => {
                ...
                if (!r || !r.play)
                    return;
                try {
                    engine?.uiCompose?.toast?.(_t('toast.replay.playing', 'Playing replay…'));
                }
                catch (_) { }
                try {
                    r.play(null, { immediate: true });
                }
                catch (_) { }
                refreshReplayStatus();
            };
                ...
                if (st && st.debug && !st.debug.lastFpsSample && typeof qaSampleFps === 'function') {
                    qaSampleFps(25, 1200).then((fps) => {
                        try {
                            ...
                            if (!s2)
                                return;
                            if (!s2.debug || typeof s2.debug !== 'object')
                                s2.debug = {};
                ...
                try {
                    ...
                    if (!worst.length) {
                        ...
                        worst.slice(0, 15).forEach((r) => {
                            ...
                            lines.push(pad(r.subsystem || 'other', 10) + '  ' +
                                pad(r.label || '', 28) + '  ' +
                                pad(r.avgMs, 6) + '  ' +
                                pad(r.maxMs, 6) + '  ' +
                                pad(r.count, 4) + '  ' +
                    ...
                    nextTick(() => {
                        try {
                            ...
                            if (bugBundle) {
                                ...
                                try {
                                    bugJsonPre.textContent = JSON.stringify(bugBundle, null, 2);
                                }
                                catch (_) {
                                    bugJsonPre.textContent = '{"error":"failed to stringify"}';
                                }
                            }
                ...
                try {
                    ...
                    nextTick(() => {
                        try {
                            ...
                            if (!last)
                                summary.textContent = 'Audit: ' + String(rep && rep.severity ? rep.severity : 'done');
                        }
                        catch (e) {
                            audit.pre.textContent = 'Audit failed: ' + (e && e.message ? e.message : String(e));
                            if (!last)
                                summary.textContent = 'Audit failed';
                        }
                        finally {
                ...
                try {
                    ...
                    nextTick(() => {
                        try {
                            ...
                            if (!last)
                                summary.textContent = 'Scenario: ' + String(res && res.severity ? res.severity : (res && res.ok ? 'ok' : 'done'));
                        }
                        catch (e) {
                            scenario.pre.textContent = 'Scenario failed: ' + (e && e.message ? e.message : String(e));
                            if (!last)
                                summary.textContent = 'Scenario failed';
                        }
                        finally {
            ...
            btnCopyFails.addEventListener('click', () => {
                try {
                    ...
                    try {
                        if (typeof copyToClipboard === 'function')
                            copyToClipboard((smoke.pre && smoke.pre.textContent) ? smoke.pre.textContent : '');
                    }
                    catch (_) { }
                }
            });
