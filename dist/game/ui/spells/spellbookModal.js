...
export function createSpellbookModal(deps) {
    ...
    const { getState, ABILITIES, MAX_EQUIPPED_SPELLS, ABILITY_UPGRADE_RULES, ensurePlayerSpellSystems, normalizeElementType, clampNumber, buildAbilityContext, getAliveEnemies, getActiveDifficultyConfig, getPlayerElementalBonusPct, getEnemyAffinityMultiplier, getEnemyElementalResistPct, playerHasTalent, _getMageRhythmBonus, _roundIntStable, addLog, openModal, closeModal, saveGame, useAbilityInCombat, getAbilityUpgrade, getEffectiveAbilityCost } = deps;
    if (!ABILITIES)
        throw new Error('createSpellbookModal: missing ABILITIES');
    ...
    function openSpellsModal(inCombat) {
        ...
        function inferAbilityElementType(id, ab) {
            ...
            if (direct)
                return normalizeElementType(direct);
            ...
            if (key.indexOf('shadow') >= 0 || key.indexOf('void') >= 0 || key.indexOf('nec') >= 0)
                return 'shadow';
            if (key.indexOf('holy') >= 0 || key.indexOf('light') >= 0 || key.indexOf('radi') >= 0)
                return 'holy';
            if (key.indexOf('poison') >= 0 || key.indexOf('toxin') >= 0 || key.indexOf('venom') >= 0)
                return 'poison';
            ...
            if (key.indexOf('earth') >= 0 || key.indexOf('stone') >= 0 || key.indexOf('quake') >= 0)
                return 'earth';
            if (key.indexOf('nature') >= 0 || key.indexOf('thorn') >= 0 || key.indexOf('vine') >= 0)
                return 'nature';
            return null;
        }
        function getSpellBadges(id) {
            ...
            if (meta && Array.isArray(meta.badges)) {
                if (meta.badges.indexOf('AoE') >= 0)
                    targetBadge = 'AoE';
                else if (meta.badges.indexOf('Chain') >= 0)
                    targetBadge = 'Chain';
            }
        ...
        function _previewCalcMagic(baseStat, elementType, enemy, ctx) {
            ...
            if (p && p.classId === 'paladin' && etKey === 'holy' && playerHasTalent(p, 'paladin_radiant_focus'))
                dmg *= 1.10;
            if (p && p.classId === 'cleric' && etKey === 'holy' && playerHasTalent(p, 'cleric_holy_focus'))
                dmg *= 1.10;
            if (p && p.classId === 'necromancer' && etKey === 'shadow' && playerHasTalent(p, 'necromancer_shadow_mastery'))
                dmg *= 1.10;
            if (p && p.classId === 'necromancer' && etKey === 'poison' && playerHasTalent(p, 'necromancer_plague_touch'))
                dmg *= 1.10;
            if (p && p.classId === 'shaman' && etKey === 'lightning' && playerHasTalent(p, 'shaman_tempest_focus'))
                dmg *= 1.10;
            if (p && p.classId === 'shaman' && etKey === 'nature' && playerHasTalent(p, 'shaman_nature_attunement'))
                dmg *= 1.10;
            if (p && p.classId === 'vampire' && etKey === 'shadow' && playerHasTalent(p, 'vampire_shadow_focus'))
                dmg *= 1.10;
        ...
        function formatCost(cost, abilityId) {
            ...
            if (cost.fury)
                parts.push(cost.fury + ' Fury');
            if (cost.blood)
                parts.push(cost.blood + ' Blood');
            if (cost.essence)
                parts.push(cost.essence + ' Essence');
            if (cost.hp)
                parts.push(cost.hp + ' HP');
        ...
        openModal(inCombat ? 'Abilities' : 'Spells & Abilities', (body) => {
            ...
            function openAbilityUpgradeModal(id) {
                ...
                openModal('Upgrade: ' + ab.name, (b) => {
                    ...
                    function doUpgrade(which) {
                        ...
                        if (which === 'potency') {
                            if (nextPot >= ABILITY_UPGRADE_RULES.maxTier)
                                return;
                            nextPot = Math.min(ABILITY_UPGRADE_RULES.maxTier, nextPot + 1);
                        }
                        else if (which === 'efficiency') {
                            if (nextEff >= ABILITY_UPGRADE_RULES.maxTier)
                                return;
            ...
            function _isSupportAbilityCard(ab) {
                try {
                    ...
                    return (/heal|ward|shield|guard|barrier|restore|regen|buff/i.test(nm) ||
            ...
            function _isAbilityMeterBuffed(abilityId, ab) {
                ...
                try {
                    ...
                    if (cid === 'rogue') {
                        ...
                        if (cp >= 5 && abilityId === 'eviscerate')
                            return true;
                    }
                    // Shaman: With an active Totem, Tempest is empowered.
                    if (cid === 'shaman') {
                        if ((st.totemTurns || 0) > 0 && abilityId === 'tempest')
                            return true;
                    }
                    ...
                    if (cid === 'berserker') {
                        ...
                        if (missingPct >= 0.9) {
                            if (abilityId === 'execute' || abilityId === 'bloodFrenzy')
                                return true;
                        }
                    }
                }
            ...
            function _applyMeterBuffOutline(card, abilityId, ab) {
                if (!card || !inCombat)
                    return;
                if (!_isAbilityMeterBuffed(abilityId, ab))
                    return;
            ...
            function makeAbilityCard(id, { label = '' } = {}) {
                ...
                const potTier = up.potencyTier || 0;
                const effTier = up.efficiencyTier || 0;
                const isMaxTier = potTier >= ABILITY_UPGRADE_RULES.maxTier &&
                ...
                if (isEq && !inCombat)
                    bits.push('Equipped');
                if (potTier > 0 || effTier > 0)
                    bits.push('P' + potTier + '/E' + effTier);
                ...
                const upText = potTier > 0 || effTier > 0
                    ...
                    if (!inCombat) {
                        if (isEq) {
                            ...
                            btnUn.addEventListener('click', (e) => {
                                e.preventDefault();
                                p.equippedSpells = (p.equippedSpells || []).filter((x) => x !== id);
                                ensurePlayerSpellSystems(p);
                                saveGame();
                                render();
                            });
                            ...
                            btnEq.addEventListener('click', (e) => {
                                ...
                                if ((p.equippedSpells || []).length >= MAX_EQUIPPED_SPELLS) {
                                    addLog('Loadout is full. Unequip an ability first.', 'system');
                                    return;
                                }
