 ...
 * ============================================================================= */
import { closeModal, closeEnemyModal, switchScreen, renderLog, initLogAutoScroll, scrollLogToBottom, getLogUiState } from './uiRuntime.js';
function applyVersionLabels(patchLabel) {
    try {
        const el = document.getElementById('versionLabel');
        if (el)
            el.textContent = patchLabel;
    }
    catch (_) { }
    try {
        const el2 = document.getElementById('footerVersion');
        if (el2)
            el2.textContent = patchLabel;
    }
    catch (_) { }
}
...
function setupCollapsingPanels() {
    ...
    function toggleAnimatedHeight(boxEl, syncAriaFn, opts = {}) {
        ...
        // Reduced motion: skip height transitions entirely.
        if (noMotion) {
            try {
            ...
            try {
                ...
                boxEl.style.height = '';
                if (typeof syncAriaFn === 'function')
                    syncAriaFn();
            }
            catch (_) { }
            return;
        }
        if (boxEl.dataset.panelAnimating === '1')
            return;
        ...
        const onEnd = (e) => {
            if (!e || e.target !== boxEl || e.propertyName !== 'height')
                return;
...
export function initUIBindings(api) {
    const { patchLabel, initCrashCatcher, getState, engine, randInt, resetDevCheatsCreationUI, buildCharacterCreationOptions, openSaveManager, initSettingsFromState, openChangelogModal, openFeedbackModal, openCharacterSheet, startNewGameFromCreation, applySettingsChanges, safeStorageGet, setTheme, SAVE_KEY, migrateSaveData, updateEnemyPanel, openEnemySheet, cycleTargetEnemy, toggleHudEntity, openPauseMenu, quests, syncSmokeTestsPillVisibility, openSmokeTestsModal, openCheatMenu, cheatsEnabled } = api || {};
    onDocReady(() => {
        try {
            if (typeof initCrashCatcher === 'function')
                initCrashCatcher();
        }
        catch (_) { }
        ...
        try {
            ...
            if (hudName && !(hudName.dataset && hudName.dataset.charSheetWired)) {
                try {
                    hudName.dataset.charSheetWired = '1';
                }
                catch (_) { }
                try {
                ...
                hudName.addEventListener('click', () => {
                    try {
                        ...
                        if (!st || !st.player)
                            return;
                        if (typeof openCharacterSheet === 'function')
                            openCharacterSheet();
                    }
                    catch (_) { }
                });
        ...
        if (btnNewGame) {
            btnNewGame.addEventListener('click', () => {
                try {
                    if (typeof resetDevCheatsCreationUI === 'function')
                        resetDevCheatsCreationUI();
                    if (typeof buildCharacterCreationOptions === 'function')
        ...
        if (btnSettingsMain) {
            btnSettingsMain.addEventListener('click', () => {
                try {
                    if (typeof initSettingsFromState === 'function')
        ...
        if (btnStartGame) {
            btnStartGame.addEventListener('click', () => {
                try {
                    if (typeof startNewGameFromCreation === 'function')
        ...
        try {
            ...
            if (modalEl && !(modalEl.dataset && modalEl.dataset.dismissWired)) {
                try {
                    modalEl.dataset.dismissWired = '1';
                }
                catch (_) { }
        ...
        try {
            ...
            if (enemyModalEl && !(enemyModalEl.dataset && enemyModalEl.dataset.dismissWired)) {
                try {
                    enemyModalEl.dataset.dismissWired = '1';
                }
                catch (_) { }
        ...
        try {
            ...
            if (btnQuestJournalPill && !(btnQuestJournalPill.dataset && btnQuestJournalPill.dataset.questJournalWired)) {
                try {
                    btnQuestJournalPill.dataset.questJournalWired = '1';
                }
                catch (_) { }
                btnQuestJournalPill.addEventListener('click', () => {
                    try {
                        if (!quests)
                            return;
                        if (typeof quests.openQuestJournal === 'function')
                            quests.openQuestJournal();
                        else if (typeof quests.openQuestJournalModal === 'function')
                            quests.openQuestJournalModal();
                    }
                    catch (_) { }
                });
            }
        ...
        try {
            ...
            if (btnGameMenu && !(btnGameMenu.dataset && btnGameMenu.dataset.gameMenuWired)) {
                try {
                    btnGameMenu.dataset.gameMenuWired = '1';
                }
                catch (_) { }
        ...
        try {
            ...
            if (hudTop && !(hudTop.dataset && hudTop.dataset.hudSwipeWired)) {
                try {
                    hudTop.dataset.hudSwipeWired = '1';
                }
                catch (_) { }
                ...
                hudTop.addEventListener('touchend', (e) => {
                    if (hudTouchStartX == null || hudTouchStartY == null)
                        return;
                    ...
                    if (Math.abs(dx) > 30 && Math.abs(dx) > Math.abs(dy)) {
                        try {
                            if (typeof toggleHudEntity === 'function')
                                toggleHudEntity();
                        }
                        catch (_) { }
                    }
        ...
        try {
            if (typeof updateEnemyPanel === 'function')
                updateEnemyPanel();
        }
        catch (_) { }
        ...
        try {
            ...
            if (enemyPanel && !(enemyPanel.dataset && enemyPanel.dataset.enemySheetWired)) {
                try {
                    enemyPanel.dataset.enemySheetWired = '1';
                }
                catch (_) { }
                ...
                const open = () => {
                    const st = getState ? getState() : null;
                    if (!st || !st.inCombat || !st.currentEnemy)
                        return;
                    if (typeof openEnemySheet === 'function')
                        openEnemySheet();
                };
                ...
                enemyPanel.addEventListener('touchend', (e) => {
                    ...
                    if (Math.abs(dx) > 30 && Math.abs(dx) > Math.abs(dy)) {
                        ...
                        try {
                            if (dx < 0) {
                                if (typeof cycleTargetEnemy === 'function')
                                    cycleTargetEnemy(1);
                            }
                            else {
                                if (typeof cycleTargetEnemy === 'function')
        ...
        try {
            if (typeof syncSmokeTestsPillVisibility === 'function')
        ...
        try {
            ...
            if (pill) {
                pill.addEventListener('click', () => {
                    try {
                        if (typeof openSmokeTestsModal === 'function')
                            openSmokeTestsModal();
                    }
                    catch (_) { }
                });
        ...
        try {
            ...
            if (pill) {
                pill.addEventListener('click', () => {
                    try {
                        if (typeof openCheatMenu === 'function')
                            openCheatMenu();
                    }
                    catch (_) { }
                });
