...
function _getState() {
    try {
        if (_deps && typeof _deps.getState === 'function')
            return _deps.getState();
    }
    catch (_) { }
    try {
...
function _ensureToastLayer() {
    ...
    if (!_hasDocument)
        return null;
    try {
        if (_toastLayer && _toastLayer.isConnected)
            return _toastLayer;
...
export function showToast(message, opts = {}) {
    ...
    const noMotion = (() => {
        try {
            return !!(document && document.body && document.body.classList && document.body.classList.contains('no-motion'));
        }
        catch (_) {
            return false;
        }
    ...
    try {
        ...
        if (!noMotion) {
            ...
            if (tw && typeof tw.to === 'function') {
                try {
                    tw.to(toastEl, { style: { opacity: 1 } }, { ms: 120, owner: 'ui:toast' });
                }
                catch (_) { }
                try {
    ...
    const remove = () => {
        try {
            ...
            if (tw && typeof tw.to === 'function') {
                ...
                try {
                    scheduleAfter(eng, 140, () => { try {
                        toastEl.remove();
                    }
                    catch (_) { } }, { owner: 'ui:toast' });
                }
                catch (_) {
                    try {
                        toastEl.remove();
    ...
    const eng = _getEngine();
    try {
        scheduleAfter(eng, durationMs, remove, { owner: 'ui:toast' });
    }
    catch (_) { }
}
function _ensureBusyOverlay() {
    ...
    if (!_hasDocument)
        return null;
    try {
        if (_busyOverlay && _busyOverlay.isConnected)
            return _busyOverlay;
...
export function applyUiTransition(name, opts = {}) {
    ...
    try {
        ...
        try {
            ...
            if (eng && eng.schedule && typeof eng.schedule.cancelOwner === 'function')
                eng.schedule.cancelOwner('ui:transition');
            if (tw && typeof tw.cancelOwner === 'function')
                tw.cancelOwner('ui:transition');
        }
        catch (_) { }
        ...
        if (tw && typeof tw.to === 'function') {
            ...
            scheduleAfter(eng, hold, () => {
                try {
                    tw.to(el, { style: { opacity: 0 } }, { ms: half, owner: 'ui:transition', onComplete: remove });
                }
                catch (_) {
                    remove();
                }
...
export function applyHudState(st = {}) {
    ...
    try {
        if (st.devPillsVisible != null) {
            ...
            if (smokePill)
                smokePill.classList.toggle('hidden', !show);
            if (cheatPill)
                cheatPill.classList.toggle('hidden', !show);
        }
...
function _getAudioState() {
    try {
        ...
        if (audio && typeof audio.getAudioState === 'function')
            return audio.getAudioState();
    }
    catch (_) { }
    try {
        if (_deps && typeof _deps.getAudioState === 'function')
            return _deps.getAudioState();
    }
    catch (_) { }
    return null;
}
function _playMusicTrack(trackId) {
    ...
    try {
        if (_deps && typeof _deps.playMusicTrack === 'function')
            _deps.playMusicTrack(trackId);
    }
    catch (_) { }
}
function _updateAreaMusic() {
    ...
    try {
        if (_deps && typeof _deps.updateAreaMusic === 'function')
            _deps.updateAreaMusic();
    }
    catch (_) { }
}
...
function _installEnemyModalFocusTrap() {
    ...
    _enemyModalTrapHandler = (e) => {
        if (!enemyModalEl || enemyModalEl.classList.contains('hidden'))
            return;
...
export function openModalDom(title, builderFn, opts = null) {
    ...
    const owner = (() => {
        try {
            if (opts && typeof opts === 'object' && typeof opts.owner === 'string' && opts.owner.trim())
    ...
    try {
        ...
        if (prevOwner && prevOwner !== owner)
            _cancelOwner(prevOwner);
    }
    catch (_) { }
    try {
        modalEl.dataset.timerOwner = owner;
    }
    catch (_) { }
...
export function closeModalDom() {
    ...
    const _closingOwner = (() => {
        try {
            return (modalEl.dataset && modalEl.dataset.timerOwner) ? modalEl.dataset.timerOwner : '';
        }
        catch (_) {
            return '';
        }
...
export function closeModal() {
    ...
    try {
        if (engine && engine.ui && typeof engine.ui.close === 'function') {
            const hasStack = (() => {
                try {
                    return Array.isArray(engine.ui.list ? engine.ui.list() : null) && engine.ui.list().length > 0;
                }
                catch (_) {
                    return true;
                }
...
export function closeEnemyModal() {
    ...
    const _closingOwner = (() => {
        try {
            return (enemyModalEl.dataset && enemyModalEl.dataset.timerOwner) ? enemyModalEl.dataset.timerOwner : (_enemyModalOwner || '');
        }
        catch (_) {
            return _enemyModalOwner || '';
        }
    })();
    try {
        if (_closingOwner)
            _cancelOwner(_closingOwner);
        if (enemyModalEl.dataset)
            enemyModalEl.dataset.timerOwner = '';
    }
    catch (_) { }
...
export function isEnemyModalOpen() {
    try {
        ...
        if (!enemyModalEl)
            return false;
        if (enemyModalEl.classList.contains('hidden'))
            return false;
        if (enemyModalEl.dataset && enemyModalEl.dataset.open === '1')
            return true;
    ...
    try {
        if (snap.logUi && typeof snap.logUi === 'object') {
            _logUi = {
                ...
                stickToBottom: typeof snap.logUi.stickToBottom === 'boolean'
...
export function formatDamageBreakdownForLog(b) {
    ...
    const bits = [];
    if (b.damageType)
        bits.push(b.damageType);
    if (b.elementType)
        bits.push(String(b.elementType));
    // Elemental modifiers:
    ...
    if (typeof b.playerElementResistPct === 'number' && b.playerElementResistPct > 0)
        bits.push('your resist ' + b.playerElementResistPct + '%');
    // Penetration naming varies across legacy / newer breakdowns.
    const pen = typeof b.armorPenPct === 'number'
        ? b.armorPenPct
    ...
    const effMres = typeof b.effectiveMagicRes === 'number'
        ? b.effectiveMagicRes
...
function logPassesFilter(entry, filter) {
    ...
    const m = entry.meta;
    if (filter === 'combat')
        return !!(m && m.domain === 'combat');
    if (filter === 'damage')
        return !!(m && m.domain === 'combat' && m.kind === 'damage');
    if (filter === 'procs')
        return !!(m && m.domain === 'combat' && m.kind === 'proc');
    if (filter === 'status')
        return !!(m && m.domain === 'combat' && m.kind === 'status');
    return true;
}
...
export function initLogAutoScroll() {
    ...
    try {
        if (logEl.dataset && logEl.dataset.autoscrollWired === '1')
            return;
        if (logEl.dataset)
            logEl.dataset.autoscrollWired = '1';
    }
    catch (_) { }
...
export function renderLog() {
    ...
    if (needsFull) {
        ...
        entries.forEach((entry) => {
            if (!logPassesFilter(entry, activeFilter))
                return;
    ...
    let any = false;
    entries.forEach((entry) => {
        if (!entry || entry.id <= _logUi.renderedUpToId)
            return;
        if (!logPassesFilter(entry, activeFilter))
            return;
        frag.appendChild(_buildLogLineEl(entry, activeFilter));
...
export function updateEnemyPanel() {
    ...
    const tags = [];
    if (enemy.level)
        tags.push('Lv ' + enemy.level);
    if (enemy.rarityLabel)
        tags.push(enemy.rarityLabel);
    if (enemy.isBoss)
        tags.push('Boss');
    if (enemy.behavior === 'bossDragon')
        tags.push('Dragon');
    else if (enemy.behavior === 'bossGoblin')
        tags.push('Warlord');
