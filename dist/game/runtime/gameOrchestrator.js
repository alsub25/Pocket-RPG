import { escapeHtml, switchScreen, screens, openModal, closeModal, openEnemyModal, closeEnemyModal, isEnemyModalOpen, setModalAdapter, getModalAdapter, setModalOnClose, getModalOnClose, addLog, renderLog, updateTimeDisplay, setScene, updateEnemyPanel, setUiDisabled, isUiDisabled, formatDamageBreakdownForLog } from '../ui/runtime/uiRuntime.js';
...
import { recordCrash, getLastCrashReport, initCrashCatcher, copyFeedbackToClipboard } from './debugHelpers.js';
/* =============================================================================
...
import { DAY_PARTS, initTimeState, getTimeInfo, formatTimeShort, advanceTime, jumpToNextMorning } from '../systems/timeSystem.js';
...
import { qaReadBootMetrics, qaReadMemorySnapshot, qaSummarizePerfLog, qaCollectPerfSnapshotSync, qaFormatPerfSnapshotText, qaSampleFps } from '../qa/perfSnapshot.js';
import { GAME_PATCH as CURRENT_PATCH, GAME_PATCH_NAME as CURRENT_PATCH_NAME } from '../systems/version.js';
import { initRngState, rngFloat, rngInt, rngPick, setRngSeed, setDeterministicRngEnabled, setRngLoggingEnabled } from '../systems/rng.js';
import { validateState, formatIssues } from '../systems/assertState.js';
import { initVillageEconomyState, getVillageEconomySummary, getMerchantPrice, getRestCost, handleEconomyDayTick, handleEconomyAfterBattle, handleEconomyAfterPurchase } from '../locations/village/villageEconomy.js';
import { initGovernmentState, handleGovernmentDayTick, getGovernmentSummary, getVillageGovernmentEffect } from '../systems/kingdomGovernment.js';
...
import { openMerchantModalImpl, handleMerchantDayTick, executeMerchantBuy } from '../locations/village/merchant.js'; // ⬅️ NEW
import { generateLootDrop, generateArmorForSlot, getItemPowerScore, getSellValue, formatRarityLabel, pickWeighted } from '../systems/lootGenerator.js';
import { openTownHallModalImpl, handleTownHallDayTick, cleanupTownHallEffects } from '../locations/village/townHall.js';
import { ensureVillagePopulation, handlePopulationDayTick } from '../locations/village/villagePopulation.js';
...
import { buildEnemyForBattle, applyEnemyRarity as applyEnemyRarityImpl, applyEliteModifiers as applyEliteModifiersImpl, applyEnemyAffixes as applyEnemyAffixesImpl, applyEnemyAffixesOnEnemyHit as applyEnemyAffixesOnEnemyHitImpl, applyEnemyAffixesOnPlayerHit as applyEnemyAffixesOnPlayerHitImpl, getEnemyRarityDef, getEnemyAffixDef, getEnemyAffixLabels as getEnemyAffixLabelsImpl, rebuildEnemyDisplayName as rebuildEnemyDisplayNameImpl, ensureEnemyRuntime as ensureEnemyRuntimeImpl, syncEnemyBaseStats as syncEnemyBaseStatsImpl, computeEnemyPostureMax as computeEnemyPostureMaxImpl } from '../systems/enemy/index.js';
import { createSaveManager } from '../persistence/saveManager.js';
import { _STORAGE_DIAG_KEY_LAST_CRASH, safeStorageGet, safeStorageSet, safeStorageRemove } from '../../engine/storageRuntime.js';
import { _perfNow, perfRecord, perfWrap, perfWrapAsync } from '../../engine/perf.js';
...
function getActiveDifficultyConfig() {
    ...
    const aiSmartness = Math.max(0.35, Math.min(0.98, normalBase.aiSmartness + stepAi * band));
...
function ensurePlayerTalents(p) {
    if (!p)
        return;
    if (p.talentPoints == null)
        p.talentPoints = 0;
    if (!p.talents || typeof p.talents !== 'object')
        p.talents = {};
}
function ensurePlayerStatsDefaults(p) {
    ...
    if (!Number.isFinite(Number(s.attack)))
        s.attack = 0;
    if (!Number.isFinite(Number(s.magic)))
        s.magic = 0;
    if (!Number.isFinite(Number(s.armor)))
        s.armor = 0;
    if (!Number.isFinite(Number(s.speed)))
        s.speed = 0;
    if (!Number.isFinite(Number(s.magicRes)))
        s.magicRes = 0;
    // Derived % / misc stats used across UI + scanners
    const ensureNum = (k) => { if (!Number.isFinite(Number(s[k])))
        s[k] = 0; };
    [
        'critChance',
        'dodgeChance',
    ...
    if (!Number.isFinite(Number(s.dodge)))
        s.dodge = Number.isFinite(Number(s.dodgeChance)) ? Number(s.dodgeChance) : 0;
    // Elemental containers
    if (!s.elementalBonuses || typeof s.elementalBonuses !== 'object')
        s.elementalBonuses = {};
    if (!s.elementalResists || typeof s.elementalResists !== 'object')
        s.elementalResists = {};
    if (!s.elementalBonusBreakdown || typeof s.elementalBonusBreakdown !== 'object') {
        s.elementalBonusBreakdown = { gear: {}, talent: {} };
    }
    else {
        if (!s.elementalBonusBreakdown.gear || typeof s.elementalBonusBreakdown.gear !== 'object')
            s.elementalBonusBreakdown.gear = {};
        if (!s.elementalBonusBreakdown.talent || typeof s.elementalBonusBreakdown.talent !== 'object')
            s.elementalBonusBreakdown.talent = {};
    }
    if (!s.elementalResistBreakdown || typeof s.elementalResistBreakdown !== 'object') {
        s.elementalResistBreakdown = { gear: {}, talent: {} };
    }
    else {
        if (!s.elementalResistBreakdown.gear || typeof s.elementalResistBreakdown.gear !== 'object')
            s.elementalResistBreakdown.gear = {};
        if (!s.elementalResistBreakdown.talent || typeof s.elementalResistBreakdown.talent !== 'object')
            s.elementalResistBreakdown.talent = {};
    }
    if (s.weaponElementType === undefined)
        s.weaponElementType = null;
}
...
function canUnlockTalent(p, tdef) {
    if (!p || !tdef)
        return false;
    ensurePlayerTalents(p);
    if (playerHasTalent(p, tdef.id))
        return false;
    if ((p.level || 1) < (tdef.levelReq || 1))
        return false;
...
function _getTalentSpellElementBonusMap(p) {
    ...
    if (!p)
        return out;
    try {
        if (playerHasTalent(p, 'mage_ember_focus'))
            out.fire = (out.fire || 0) + 10;
        if (playerHasTalent(p, 'mage_glacial_edge'))
            out.frost = (out.frost || 0) + 10;
        if (playerHasTalent(p, 'blood_hemomancy'))
            out.shadow = (out.shadow || 0) + 10;
        if (playerHasTalent(p, 'ranger_nature_attunement'))
            out.nature = (out.nature || 0) + 10;
        if (playerHasTalent(p, 'paladin_radiant_focus'))
            out.holy = (out.holy || 0) + 10;
        if (playerHasTalent(p, 'cleric_holy_focus'))
            out.holy = (out.holy || 0) + 10;
        if (playerHasTalent(p, 'necromancer_shadow_mastery'))
            out.shadow = (out.shadow || 0) + 10;
        if (playerHasTalent(p, 'necromancer_plague_touch'))
            out.poison = (out.poison || 0) + 10;
        if (playerHasTalent(p, 'shaman_tempest_focus'))
            out.lightning = (out.lightning || 0) + 10;
        if (playerHasTalent(p, 'shaman_nature_attunement'))
            out.nature = (out.nature || 0) + 10;
        if (playerHasTalent(p, 'vampire_shadow_focus'))
            out.shadow = (out.shadow || 0) + 10;
    }
    catch (_) { }
    return out;
...
function _getElementalBreakdownsForPlayer(p) {
    const gearBonus = (p && p.stats && p.stats.elementalBonusBreakdown && p.stats.elementalBonusBreakdown.gear) ||
        (p && p.stats && p.stats.elementalBonuses) ||
        {};
    let talentBonus = (p && p.stats && p.stats.elementalBonusBreakdown && p.stats.elementalBonusBreakdown.talent) ||
        {};
    if (!talentBonus || typeof talentBonus !== 'object')
        talentBonus = {};
    if (!Object.keys(talentBonus).length) {
        // Back-compat: compute focus bonuses on demand.
        talentBonus = _getTalentSpellElementBonusMap(p);
    }
    const gearResist = (p && p.stats && p.stats.elementalResistBreakdown && p.stats.elementalResistBreakdown.gear) ||
        {};
    const talentResist = (p && p.stats && p.stats.elementalResistBreakdown && p.stats.elementalResistBreakdown.talent) ||
        {};
...
function computeElementSummariesForPlayer(p) {
    ...
    const bonusKeys = _orderedElementKeys(Object.keys(bd.gearBonus || {}).concat(Object.keys(bd.talentBonus || {})));
    const bonusParts = [];
    bonusKeys.forEach((k) => {
    ...
    const weaponElement = p && p.stats && p.stats.weaponElementType ? _capWord(p.stats.weaponElementType) : 'None';
...
function renderElementalBreakdownHtml(p) {
    ...
    const keys = _orderedElementKeys(Object.keys(bd.gearBonus || {})
...
function refreshCharacterSheetLiveValues(p, root) {
    ...
    try {
        ...
        if (we)
            we.textContent = sums.weaponElement;
        if (eb)
            eb.textContent = sums.elementalBonusSummary;
        if (er)
            er.textContent = sums.elementalResistSummary;
        if (we2)
            we2.textContent = sums.weaponElement;
        if (eb2)
            eb2.textContent = sums.elementalBonusSummary;
        if (er2)
            er2.textContent = sums.elementalResistSummary;
    }
    catch (_) { }
    ...
    try {
        ...
        const hpLine = Math.round(finiteNumber(p.hp, 0)) + ' / ' + Math.round(finiteNumber(p.maxHp, 0));
        const resLine = Math.round(finiteNumber(p.resource, 0)) +
            ' / ' +
...
function refreshCharacterSheetIfOpen() {
    ...
    try {
        ...
        if (!titleEl || !bodyEl)
            return;
        if ((titleEl.textContent || '').trim() !== 'Character Sheet')
            return;
...
function ensurePlayerSpellSystems(p) {
    ...
    p.equippedSpells = Array.from(new Set(p.equippedSpells.filter((id) => p.spells.includes(id)))).slice(0, MAX_EQUIPPED_SPELLS);
    ...
    if (typeof p.status.comboPoints !== 'number' || !Number.isFinite(p.status.comboPoints))
        p.status.comboPoints = 0;
    if (typeof p.status.soulShards !== 'number' || !Number.isFinite(p.status.soulShards))
        p.status.soulShards = 0;
    if (typeof p.status.lichTurns !== 'number' || !Number.isFinite(p.status.lichTurns))
        p.status.lichTurns = 0;
    if (typeof p.status.totemType !== 'string')
        p.status.totemType = '';
    if (typeof p.status.totemTurns !== 'number' || !Number.isFinite(p.status.totemTurns))
        p.status.totemTurns = 0;
    if (typeof p.status.vanishTurns !== 'number' || !Number.isFinite(p.status.vanishTurns))
        p.status.vanishTurns = 0;
}
function getAbilityUpgrade(p, id) {
    ...
    if (typeof u.tier !== 'undefined' && typeof u.path === 'string') {
        ...
        if (u.path === 'potency')
            potencyTier = tier;
        if (u.path === 'efficiency')
            efficiencyTier = tier;
    }
    // Current: independent tiers (preferred).
    if (typeof u.potencyTier !== 'undefined') {
        potencyTier = Math.max(potencyTier, Math.max(0, Math.min(ABILITY_UPGRADE_RULES.maxTier, Math.floor(Number(u.potencyTier || 0)))));
    }
    if (typeof u.efficiencyTier !== 'undefined') {
        efficiencyTier = Math.max(efficiencyTier, Math.max(0, Math.min(ABILITY_UPGRADE_RULES.maxTier, Math.floor(Number(u.efficiencyTier || 0)))));
    }
    return { potencyTier, efficiencyTier };
}
function getAbilityEffectMultiplier(p, id) {
    const up = getAbilityUpgrade(p, id);
    if (!up || up.potencyTier <= 0)
        return 1;
    return 1 + ABILITY_UPGRADE_RULES.potencyPerTier * up.potencyTier;
}
function applyUpgradeToCost(p, id, cost) {
    const up = getAbilityUpgrade(p, id);
    if (!up || up.efficiencyTier <= 0)
        return cost;
...
function _dealPlayerPhysical(p, enemy, baseStat, elementType, postureMeta) {
    ...
    applyEnemyPostureFromPlayerHit(enemy, dmg, Object.assign({ damageType: 'physical', elementType: et || null }, postureMeta || null));
    return dmg;
}
function _dealPlayerMagic(p, enemy, baseStat, elementType, postureMeta) {
    ...
    applyEnemyPostureFromPlayerHit(enemy, dmg, Object.assign({ damageType: 'magic', elementType: et || null }, postureMeta || null));
    return dmg;
}
...
function _consumeCompanionBoonIfNeeded(p, ctx) {
    ...
    if (!p || !p.status || !ctx || !ctx.consumeCompanionBoon)
        return;
    p.status.buffFromCompanion = 0;
    p.status.buffFromCompanionTurns = 0;
...
function _applyCostPassives(p, paidCost) {
    ...
    if (p.classId === 'blood') {
        ...
        if (cost.blood && cost.blood > 0) {
            ...
            if (actual > 0)
                addLog('Crimson Exchange restores ' + actual + ' HP.', 'system');
        }
    }
}
function _getMageRhythmBonus(p, ab, abilityId) {
    if (!p || p.classId !== 'mage')
        return { costMult: 1, critBonus: 0, active: false };
    if (!ab || !ab.cost || !ab.cost.mana)
        return { costMult: 1, critBonus: 0, active: false };
    const st = p.status || (p.status = {});
...
function buildAbilityContext(p, abilityId) {
    ...
    if (playerHasTalent(p, 'cleric_mending_prayer'))
        ctx.healMult *= 1.15;
    if (playerHasTalent(p, 'shaman_ancestral_mending'))
        ctx.healMult *= 1.15;
...
function pickEnemyAbilitySet(enemy) {
    ...
    if (ENEMY_ABILITY_SETS[b])
        return ENEMY_ABILITY_SETS[b];
    return ENEMY_ABILITY_SETS.basic;
}
/* =============================================================================
...
function _setState(next) {
    ...
    try {
        if (_engine && typeof _engine.setState === 'function')
            _engine.setState(next);
    }
    catch (_) { }
    return state;
}
...
function _getSavePolicy() {
    try {
        return _engine && _engine.getService ? _engine.getService('savePolicy') : null;
    }
    catch (_) {
        return null;
    }
...
function setArea(nextArea, { source = 'unknown', emit = true } = {}) {
    ...
    try {
        if (_engine && typeof _engine.emit === 'function') {
            try {
                if (from)
                    _engine.emit('area:leave', { area: from, to, source: String(source || ''), owner: `area:${from}` });
            }
            catch (_) { }
...
function _ensureCombatEnginesBound() {
    ...
    try {
        if (!CombatMath)
            CombatMath = _engine.getService('combat.math') || (_engine.getService('combat') && _engine.getService('combat').math) || null;
    }
    catch (_) { }
    try {
        if (!StatusEngine)
            StatusEngine = _engine.getService('combat.status') || (_engine.getService('combat') && _engine.getService('combat').status) || null;
    }
    catch (_) { }
    try {
...
function getMasterVolume01() {
    const v = state && typeof state.settingsVolume === 'number'
...
function applyMasterVolumeTo(el) {
    ...
    const cat = audioState.categories.get(el) ||
        (Object.values(audioState.sfx).includes(el) ? 'sfx' : 'music');
    const chanEnabled = cat === 'sfx'
        ? audioState.sfxEnabled
            ? 1
            : 0
        : audioState.musicEnabled
...
function initAudio() {
    ...
    const villageDay = registerAudio(new Audio(new URL('../../../assets/audio/village_day.wav', import.meta.url).href), 0.4, 'music');
    villageDay.loop = true;
    audioState.tracks.villageDay = villageDay;
    // Global night ambience (plays anywhere at night)
    const nightAmbience = registerAudio(new Audio(new URL('../../../assets/audio/night-ambience.wav', import.meta.url).href), 0.35, 'music');
    nightAmbience.loop = true;
    audioState.tracks.nightAmbience = nightAmbience;
    // Inside initAudio(), after other ambient/sfx tracks:
    const tavernAmbience = registerAudio(new Audio(new URL('../../../assets/audio/Tavern.wav', import.meta.url).href), 0.45, 'music');
    tavernAmbience.loop = true;
    audioState.tracks.tavernAmbience = tavernAmbience;
    // ---- SFX ----------------------------------------------------------------
    const doorOpen = registerAudio(new Audio(new URL('../../../assets/audio/old-wooden-door.wav', import.meta.url).href), 0.7, 'sfx');
    doorOpen.loop = false;
...
function playDoorOpenSfx() {
    ...
    const sfxOn = typeof state !== 'undefined' && state
...
function isMorning(info) {
    ...
    if (typeof info?.partIndex === 'number')
        return info.partIndex === 0; // Morning
...
function recordInput(action, payload) {
    ...
    if (!Array.isArray(d.inputLog))
        d.inputLog = [];
    d.inputLog.push({ t: Date.now(), action: d.lastAction, payload: payload || null });
    if (d.inputLog.length > 200)
        d.inputLog.splice(0, d.inputLog.length - 200);
}
...
function _updateHUDImpl() {
    ...
    if (!nameEl || !classDiffEl || !hpFill || !hpLabel || !resFill || !resLabel || !hudLevel || !hudGold || !hudBottom)
        return;
    ...
    if (mode === 'player') {
        ...
        const rk = p.resourceKey === 'mana' || p.resourceKey === 'fury' || p.resourceKey === 'blood' || p.resourceKey === 'essence'
            ? p.resourceKey
            : 'mana';
    ...
    if (hudBottom) {
        ...
        if (hudLevel)
            hudLevel.classList.toggle('hidden', inCombatNow);
        if (hudGold)
            hudGold.classList.toggle('hidden', inCombatNow);
        if (hudTime)
            hudTime.classList.toggle('hidden', inCombatNow);
    }
...
function updateClassMeterHUD() {
    ...
    const iconUse = (symbolId) => {
        ...
        return ('<svg class="meter-icon" viewBox="0 0 24 24" aria-hidden="true" focusable="false">' +
    ...
    if (meter.kind === 'pips+value') {
        ...
        if (v)
            html += '<span class="meter-turns">' + v + '</span>';
    }
    el.innerHTML = html;
...
function renderExploreActions(actionsEl) {
    ...
    if (showVillageMenu) {
        actionsEl.appendChild(makeActionButton('Elder Rowan', () => {
            if (!dispatchGameCommand('GAME_OPEN_ELDER_ROWAN', {})) {
                if (quests && quests.openElderRowanDialog)
                    quests.openElderRowanDialog();
            }
        }));
        actionsEl.appendChild(makeActionButton('Tavern', () => {
            if (!dispatchGameCommand('GAME_OPEN_TAVERN', {}))
                openTavernModal();
        }));
        actionsEl.appendChild(makeActionButton('Bank', () => {
            if (!dispatchGameCommand('GAME_OPEN_BANK', {}))
                openBankModal();
        }));
        actionsEl.appendChild(makeActionButton('Merchant', () => {
            if (!dispatchGameCommand('GAME_OPEN_MERCHANT', {}))
                openMerchantModal();
        }));
        actionsEl.appendChild(makeActionButton('Town Hall', () => {
            if (!dispatchGameCommand('GAME_OPEN_TOWN_HALL', {}))
                openTownHallModal();
        }));
        actionsEl.appendChild(makeActionButton('Back', () => {
    ...
    if (inVillage) {
        actionsEl.appendChild(makeActionButton('Village ▸', () => {
            ui.villageActionsOpen = true;
            renderActions();
        }));
        // ✅ Only show Realm & Council if you're in the village
        actionsEl.appendChild(makeActionButton('Realm & Council', () => {
            if (!dispatchGameCommand('GAME_OPEN_GOVERNMENT', {}))
                openGovernmentModal();
        }));
    }
    actionsEl.appendChild(makeActionButton('Explore', () => {
        if (!dispatchGameCommand('GAME_EXPLORE', {}))
            handleExploreClick();
    }, ''));
    actionsEl.appendChild(makeActionButton('Change Area', () => {
...
function renderCombatActions(actionsEl) {
    ...
    actionsEl.appendChild(makeActionButton('Interrupt', () => {
    ...
    actionsEl.appendChild(makeActionButton('Items', () => {
...
function startNewGameFromCreation() {
    ...
    const startingSkills = CLASS_STARTING_SKILLS[classId] || CLASS_STARTING_SKILLS.default;
    const player = {
        name,
        classId,
        level: 1,
        ...
        maxResource: classDef.resourceKey === 'mana'
            ? 100
            : classDef.resourceKey === 'essence'
                ? 90
                : 60,
        resource: classDef.resourceKey === 'mana'
            ? 80
            : classDef.resourceKey === 'essence'
                ? 45
                : 0,
    ...
    setScene('Emberwood Village', 'You arrive at Emberwood, a frontier village stalked by shadows. The village elder seeks a champion.');
...
function addItemToInventory(itemId, quantity) {
    ...
    try {
        _engine && _engine.emit && _engine.emit('world:itemGained', { itemId: def.id, quantity });
    }
    catch (_) { }
...
function addGeneratedItemToInventory(item, quantity = 1) {
    ...
    if (cloned.type === 'potion') {
        const existingIndex = inv.findIndex((it) => it.id === cloned.id && it.type === 'potion');
        if (existingIndex >= 0) {
            const prev = Math.floor(Number(inv[existingIndex].quantity));
            inv[existingIndex].quantity = (Number.isFinite(prev) ? prev : 0) + quantity;
            try {
                _engine && _engine.emit && _engine.emit('world:itemGained', { itemId: cloned.id, quantity });
            }
            catch (_) { }
            if (!_questEventsEnabled()) {
                try {
    ...
    inv.push(cloned);
    try {
        _engine && _engine.emit && _engine.emit('world:itemGained', { itemId: cloned.id, quantity: 1 });
    }
    catch (_) { }
    if (!_questEventsEnabled()) {
        try {
...
function unequipItemIfEquipped(player, item) {
    if (!player || !player.equipment || !item)
        return false;
    ...
    openModal('Inventory', (body) => {
        ...
        function renderList() {
            ...
            filtered.sort((a, b) => {
                const ia = a.item;
                const ib = b.item;
                const aEq = isItemEquipped(ia);
                const bEq = isItemEquipped(ib);
            ...
            filtered.forEach(({ item, idx }) => {
                ...
                const bits = [];
                if (item.itemLevel)
                    bits.push('iLv ' + item.itemLevel);
                if (item.rarity)
                    bits.push(formatRarityLabel(item.rarity));
                if (item.type === 'potion') {
                    bits.push('Potion');
                }
                ...
                if (tight.childNodes.length)
                    actions.appendChild(tight);
...
function usePotionFromInventory(index, inCombat, opts = {}) {
    ...
    if (item.resourceRestore) {
        if (item.resourceKey && item.resourceKey !== p.resourceKey) {
            addLog(item.name + " doesn't restore your " + (p.resourceName || 'power') + '.', 'system');
        }
        else {
...
function equipItemFromInventory(index, opts = {}) {
    ...
    if (!p.equipment)
        p.equipment = {};
    if (p.equipment.weapon === undefined)
        p.equipment.weapon = null;
    if (p.equipment.armor === undefined)
        p.equipment.armor = null;
    if (p.equipment.head === undefined)
        p.equipment.head = null;
    if (p.equipment.hands === undefined)
        p.equipment.hands = null;
    if (p.equipment.feet === undefined)
        p.equipment.feet = null;
    if (p.equipment.belt === undefined)
        p.equipment.belt = null;
    if (p.equipment.neck === undefined)
        p.equipment.neck = null;
    if (p.equipment.ring === undefined)
        p.equipment.ring = null;
    const stayOpen = !!opts.stayOpen;
    const onAfterEquip = typeof opts.onAfterEquip === 'function' ? opts.onAfterEquip : null;
    const slotLabel = (slot) => slot === 'weapon'
        ? 'weapon'
        : slot === 'armor'
            ? 'armor'
            : slot === 'head'
    ...
    if (item.type === 'weapon') {
        ...
        const slot = item.slot || 'armor';
        if (p.equipment[slot] === undefined)
            p.equipment[slot] = null;
        p.equipment[slot] = item;
        addLog('You equip ' + item.name + ' as your ' + slotLabel(slot) + '.', 'good');
    }
    recalcPlayerStats();
    updateHUD();
    requestSave('legacy');
...
function recalcPlayerStats() {
    ...
    if (!p.skills) {
        const fallback = CLASS_STARTING_SKILLS[p.classId] || CLASS_STARTING_SKILLS.default;
        p.skills = {
    ...
    const applyItemBonuses = (it, slot) => {
        ...
        // Core, older fields
        if (it.attackBonus)
            p.stats.attack += it.attackBonus;
        if (it.magicBonus)
            p.stats.magic += it.magicBonus;
        if (it.armorBonus)
            p.stats.armor += it.armorBonus;
        if (it.speedBonus)
            p.stats.speed += it.speedBonus;
        // Optional: support explicit magical resistance bonuses (future-proof).
        if (it.magicResBonus)
            p.stats.magicRes += it.magicResBonus;
        if (it.magicRes)
            p.stats.magicRes += it.magicRes;
        // Maxima
        if (it.maxHPBonus)
            p.maxHp += it.maxHPBonus;
        if (it.maxHpBonus)
            p.maxHp += it.maxHpBonus; // legacy
        if (it.maxResourceBonus)
            extraMaxRes += it.maxResourceBonus;
        // NEW affixes (all rolled as % in loot generator; store as % here)
        if (it.critChance)
            p.stats.critChance += it.critChance;
        if (it.dodgeChance)
            p.stats.dodgeChance += it.dodgeChance;
        if (it.resistAll)
            p.stats.resistAll += it.resistAll;
        if (it.lifeSteal)
            p.stats.lifeSteal += it.lifeSteal;
        if (it.armorPen)
            p.stats.armorPen += it.armorPen;
        if (it.haste)
            p.stats.haste += it.haste;
        // Defensive utilities
        if (it.thorns)
            p.stats.thorns += it.thorns;
        if (it.hpRegen)
            p.stats.hpRegen += it.hpRegen;
        // Elemental bonus
        ...
        if (it.elementalBonuses && typeof it.elementalBonuses === 'object') {
            Object.keys(it.elementalBonuses).forEach((k) => addGearElementBonus(k, it.elementalBonuses[k]));
        }
        ...
        if (it.elementalResists && typeof it.elementalResists === 'object') {
            Object.keys(it.elementalResists).forEach((k) => addGearElementResist(k, it.elementalResists[k]));
        }
    ...
    if (playerHasTalent(p, 'mage_frostward'))
        addTalentElementResist('frost', 15);
    if (playerHasTalent(p, 'mage_arcane_ward'))
        addTalentElementResist('arcane', 15);
    if (playerHasTalent(p, 'warrior_frostward'))
        addTalentElementResist('frost', 15);
    if (playerHasTalent(p, 'blood_shadowward'))
        addTalentElementResist('shadow', 15);
    if (playerHasTalent(p, 'paladin_holyward'))
        addTalentElementResist('holy', 15);
    if (playerHasTalent(p, 'rogue_shadowward'))
        addTalentElementResist('shadow', 15);
    if (playerHasTalent(p, 'cleric_lightward'))
        addTalentElementResist('holy', 15);
    if (playerHasTalent(p, 'necromancer_graveward'))
        addTalentElementResist('shadow', 15);
    if (playerHasTalent(p, 'shaman_stormward'))
        addTalentElementResist('lightning', 15);
    if (playerHasTalent(p, 'berserker_fireward'))
        addTalentElementResist('fire', 15);
    if (playerHasTalent(p, 'vampire_shadowward'))
        addTalentElementResist('shadow', 15);
    if (playerHasTalent(p, 'vampire_mistward'))
        addTalentElementResist('frost', 15);
    // Talent spell focus bonuses (UI breakdown only; damage calcs apply these separately).
    if (playerHasTalent(p, 'mage_ember_focus'))
        addTalentElementBonus('fire', 10);
    if (playerHasTalent(p, 'mage_glacial_edge'))
        addTalentElementBonus('frost', 10);
    if (playerHasTalent(p, 'blood_hemomancy'))
        addTalentElementBonus('shadow', 10);
    if (playerHasTalent(p, 'ranger_nature_attunement'))
        addTalentElementBonus('nature', 10);
    if (playerHasTalent(p, 'paladin_radiant_focus'))
        addTalentElementBonus('holy', 10);
    if (playerHasTalent(p, 'cleric_holy_focus'))
        addTalentElementBonus('holy', 10);
    if (playerHasTalent(p, 'necromancer_shadow_mastery'))
        addTalentElementBonus('shadow', 10);
    if (playerHasTalent(p, 'necromancer_plague_touch'))
        addTalentElementBonus('poison', 10);
    if (playerHasTalent(p, 'shaman_tempest_focus'))
        addTalentElementBonus('lightning', 10);
    if (playerHasTalent(p, 'shaman_nature_attunement'))
        addTalentElementBonus('nature', 10);
    if (playerHasTalent(p, 'vampire_shadow_focus'))
        addTalentElementBonus('shadow', 10);
    // Talent-derived stat adjustments (Patch 1.2.32)
    if (playerHasTalent(p, 'mage_mystic_reservoir') && p.resourceKey === 'mana')
        extraMaxRes += 20;
    if (playerHasTalent(p, 'warrior_sunder'))
        p.stats.armorPen += 10;
    if (playerHasTalent(p, 'warrior_ironhide')) {
        p.stats.armor += 6;
        p.stats.resistAll += 5;
    }
    if (playerHasTalent(p, 'warrior_battle_trance'))
        p.stats.haste += 10;
    if (playerHasTalent(p, 'blood_blood_armor')) {
        p.maxHp += 12;
        p.stats.armor += 2;
    }
    if (playerHasTalent(p, 'ranger_camouflage'))
        p.stats.dodgeChance += 8;
    if (playerHasTalent(p, 'ranger_called_shot'))
        p.stats.critChance += 10;
    // Additional class talent stat adjustments (Patch 1.2.32)
    if (playerHasTalent(p, 'paladin_aura_of_faith'))
        p.stats.resistAll += 5;
    if (playerHasTalent(p, 'paladin_sanctified_plate'))
        p.stats.armor += 8;
    if (playerHasTalent(p, 'paladin_zeal'))
        p.stats.critChance += 8;
    if (playerHasTalent(p, 'paladin_divine_haste'))
        p.stats.haste += 10;
    if (playerHasTalent(p, 'paladin_mana_font') && p.resourceKey === 'mana')
        extraMaxRes += 20;
    if (playerHasTalent(p, 'rogue_deadly_precision'))
        p.stats.critChance += 10;
    if (playerHasTalent(p, 'rogue_smokefoot'))
        p.stats.dodgeChance += 8;
    if (playerHasTalent(p, 'rogue_armor_sunder'))
        p.stats.armorPen += 10;
    if (playerHasTalent(p, 'rogue_adrenaline'))
        p.stats.haste += 10;
    if (playerHasTalent(p, 'cleric_mana_font') && p.resourceKey === 'mana')
        extraMaxRes += 20;
    if (playerHasTalent(p, 'cleric_bastion')) {
        p.stats.armor += 6;
        p.stats.resistAll += 5;
    }
    if (playerHasTalent(p, 'cleric_grace'))
        p.stats.dodgeChance += 8;
    if (playerHasTalent(p, 'cleric_divine_haste'))
        p.stats.haste += 10;
    if (playerHasTalent(p, 'necromancer_soul_battery') && p.resourceKey === 'mana')
        extraMaxRes += 20;
    if (playerHasTalent(p, 'necromancer_bone_plating')) {
        p.stats.armor += 4;
        p.stats.resistAll += 8;
    }
    if (playerHasTalent(p, 'necromancer_dark_haste'))
        p.stats.haste += 10;
    if (playerHasTalent(p, 'shaman_mana_font') && p.resourceKey === 'mana')
        extraMaxRes += 20;
    if (playerHasTalent(p, 'shaman_spirit_guard')) {
        p.stats.armor += 6;
        p.stats.resistAll += 5;
    }
    if (playerHasTalent(p, 'shaman_swift_steps'))
        p.stats.dodgeChance += 8;
    if (playerHasTalent(p, 'berserker_bloodthirst'))
        p.stats.lifeSteal += 8;
    if (playerHasTalent(p, 'berserker_hardened'))
        p.stats.armor += 6;
    if (playerHasTalent(p, 'berserker_ferocity'))
        p.stats.critChance += 10;
    if (playerHasTalent(p, 'berserker_battle_trance'))
        p.stats.haste += 10;
    if (playerHasTalent(p, 'vampire_essence_reservoir') && p.resourceKey === 'essence')
        extraMaxRes += 20;
    if (playerHasTalent(p, 'vampire_dark_agility'))
        p.stats.dodgeChance += 8;
    if (playerHasTalent(p, 'vampire_bloodletting'))
        p.stats.lifeSteal += 10;
    if (playerHasTalent(p, 'vampire_crimson_crit'))
        p.stats.critChance += 10;
...
function openTavernModal() {
    ...
    const tavernAlreadyPlaying = !!(audioState.interiorOpen &&
        tavernTrack &&
    ...
    openModal('Cheat Menu', (body) => {
        ...
        function fitCheatStatusbarTwoRows() {
            ...
            Array.from(statusBar.querySelectorAll('.cheat-stat')).forEach((el) => {
                if (el && el.dataset && el.dataset.full)
            ...
            Array.from(statusBar.querySelectorAll('.cheat-stat')).forEach((el) => {
                if (el && el.dataset && el.dataset.short)
        ...
        function renderCheatStatus() {
            const day = state && state.time && typeof state.time.dayIndex === 'number'
                ? Math.floor(Number(state.time.dayIndex))
                : 0;
            const part = state && state.time && state.time.part
            ...
            const resName = String(resNameRaw).length > 10
            ...
            const resShortLabel = String(resNameRaw)
                ...
                .slice(0, 4)
                .replace(/\s+/g, '');
            const resShort = (resShortLabel ? resShortLabel : 'Res') +
                ' ' +
                (p.resource || 0) +
                '/' +
                (p.maxResource || 0);
            ...
            if (area)
                locBits.push(area);
            if (activeDiff && activeDiff.name)
                locBits.push(activeDiff.name);
            if (locBits.length) {
            ...
            const flagsTxt = 'God ' + (state.flags.godMode ? 'ON' : 'OFF') + ' • Crit ' + critLabel;
            const flagsShort = 'God ' + (state.flags.godMode ? 'ON' : 'OFF') + ' • C ' + critLabel.slice(0, 1);
        ...
        btnCrit.addEventListener('click', () => {
            ...
            if (state.flags.alwaysCrit)
                state.flags.neverCrit = false;
            addLog('Always-crit mode ' +
        ...
        btnNeverCrit.addEventListener('click', () => {
            ...
            if (state.flags.neverCrit)
                state.flags.alwaysCrit = false;
            addLog('Never-crit mode ' +
        ...
        function ensureMainQuestPresent() {
            ...
            if (state.quests && state.quests.main) {
                ...
                if (!Number.isFinite(Number(state.quests.main.step)))
                    state.quests.main.step = 0;
            }
        }
        function applyMainQuestPrereqs(step) {
            ...
            if (n >= 12)
                f.quietRootsTrialDone = true;
            if (n >= 13)
                f.ashWardenMet = true;
            if (n >= 14)
                f.blackbarkGateFound = true;
            ...
            if (n >= 16) {
                ...
                if (!f.chapter3CouncilDone) {
                    ...
                    if (!f.chapter3CouncilStance)
                        f.chapter3CouncilStance = 'investigate';
                }
            }
            if (n >= 17)
            ...
            if (n >= 19)
                f.chapter3StarIronPin = true;
            if (n >= 20)
                f.chapter3GraveLatch = true;
            if (n >= 21) {
                f.chapter3RitualAllyChosen = true;
                if (!f.chapter3RitualAlly)
                    f.chapter3RitualAlly = 'rowan';
            }
            if (n >= 22)
                f.hollowRegentDefeated = true;
            // Chapter IV prerequisites
            if (n >= 23) {
                f.chapter3FinalChoiceMade = true;
                if (!f.chapter3Ending)
                    f.chapter3Ending = 'seal';
            ...
            if (n >= 25)
                f.chapter4VerdantLens = true;
            if (n >= 26)
                f.chapter4MarshWritsDone = true;
            if (n >= 27)
                f.chapter4FrozenWrit = true;
            if (n >= 28)
                f.chapter4BoneWrit = true;
            if (n >= 29)
                f.chapter4SealOfVerdict = true;
            if (n >= 30)
                f.chapter4MagistrateDefeated = true;
            if (n >= 31) {
                f.chapter4FinalChoiceMade = true;
                if (!f.chapter4Ending)
                    f.chapter4Ending = 'rewrite';
            }
        ...
        function setStoryPosition(step, opts = {}) {
            ...
            if (chkReset.input.checked)
                resetQuestStoryFlags();
            ensureMainQuestPresent();
            if (chkFill.input.checked)
                applyMainQuestPrereqs(target);
            if (state.quests && state.quests.main)
                state.quests.main.step = target;
            // Auto-place the player where this beat is most testable.
            if (chkEnv.input.checked) {
                if (opts.area)
                    setArea(String(opts.area), { source: 'cheat:storyJump' });
        ...
        btnRefundTalents.addEventListener('click', () => {
            ensurePlayerTalents(p);
            const owned = p.talents && typeof p.talents === 'object'
        ...
        btnRecalc.addEventListener('click', () => {
            try {
                ...
                addLog('Cheat: stat recalc failed: ' + (e && e.message ? e.message : e), 'system');
            }
            updateHUD();
            requestSave('legacy');
            renderCheatStatus();
        ...
        function snapshotTown() {
            ...
            const decreeRemaining = decree && decree.petitionId && typeof decree.expiresOnDay === 'number'
        ...
        function spawnMaxLootSet(equipNow) {
            ...
            addLog('Cheat: spawned max-level loot: ' +
                names +
                (equipNow ? ' (equipped).' : '.'), 'system');
            updateHUD();
            requestSave('legacy');
        }
        ...
        function ensureGamblingDebug() {
            if (!state.gamblingDebug) {
                ...
                if (typeof state.gamblingDebug.payoutMultiplier !== 'number' ||
        ...
        function updateGambleStatus() {
            ...
            const modeLabel = dbg.mode === 'playerFavored'
                ? 'Player-Favored'
                : dbg.mode === 'houseFavored'
                    ? 'House-Favored'
                    : 'Normal';
            const mult = typeof dbg.payoutMultiplier === 'number' &&
        ...
        function ensureGovAndVillage() {
            const absDay = state.time && typeof state.time.absoluteDay === 'number'
            ...
            const village = g && g.villages && g.villages.village
                ? g.villages.village
                : null;
            return { g, village };
        }
        ...
        function populateGovFields() {
            ...
            if (typeof getGovernmentSummary === 'function') {
                ...
                if (summary && summary.hasGovernment) {
                    ...
                    const pr = typeof m.prosperity === 'number' ? m.prosperity : 0;
                    const rp = typeof m.royalPopularity === 'number'
                        ? m.royalPopularity
                        : 0;
                    const co = typeof m.corruption === 'number' ? m.corruption : 0;
        ...
        btnApplyGov.addEventListener('click', function () {
            ...
            addLog('Cheat: adjusted kingdom government metrics and Emberwood attitudes.', 'system');
            populateGovFields();
            updateHUD();
            requestSave('legacy');
        });
        ...
        btnClearTownHall.addEventListener('click', function () {
            const today = state && state.time && typeof state.time.dayIndex === 'number'
                ? state.time.dayIndex
                : 0;
...
function payCost(cost) {
    const p = state.player;
    if (cost.mana && p.resourceKey === 'mana')
        p.resource -= cost.mana;
    if (cost.fury && p.resourceKey === 'fury')
        p.resource -= cost.fury;
    if (cost.blood && p.resourceKey === 'blood')
        p.resource -= cost.blood;
    if (cost.essence && p.resourceKey === 'essence')
        p.resource -= cost.essence;
    if (cost.hp)
        p.hp -= cost.hp;
}
function useAbilityInCombat(id) {
    ...
    if (ctx && ctx._mageRhythmActive && ctx._manaRefund && p.resourceKey === 'mana') {
        ...
        if (refund > 0) {
            ...
            if (gained > 0)
                addLog('Arcane Rhythm refunds ' + gained + ' Mana.', 'system');
        }
    }
...
function resolvePendingEnemyDefeats() {
    ...
    for (let i = 0; i < ordered.length; i++) {
        if (!state.inCombat)
            break;
        const e = ordered[i];
        if (!e || e._defeatHandled)
            continue;
        if (finiteNumber(e.hp, 0) <= 0)
            handleEnemyDefeat(e);
    }
}
function openCharacterSheet() {
    ...
    const cls = PLAYER_CLASSES[p.classId];
    const diff = getActiveDifficultyConfig();
    const areaName = state.area === 'village'
        ? 'Emberwood Village'
        : state.area === 'forest'
            ? 'Emberwood Forest'
            : state.area === 'ruins'
    ...
    // Equipment bonuses
    const weaponAtkBonus = p.equipment.weapon && p.equipment.weapon.attackBonus
        ? p.equipment.weapon.attackBonus
        : 0;
    const weaponMagicBonus = p.equipment.weapon && p.equipment.weapon.magicBonus
    ...
    const sumGear = (field) => gearSlots.reduce((acc, k) => {
    ...
    openModal('Character Sheet', (body) => {
        ...
        function wireSheetAccordions(root) {
            ...
            sections.forEach((sec) => {
                ...
                try {
                    if (sec.dataset.sheetSectionWired)
                        return;
    ...
    `;
      ...
      <div class="char-section">
        ...
        <div class="stat-grid">
          ...
          <div class="stat-value"><span class="sheet-core-resource">${Math.round(p.resource)} / ${p.maxResource}</span></div>
    ...
    `;
        ...
        const slotName = (slot) => {
            ...
            if (!it)
                return '<span class="equip-empty">None</span>';
            return ('<span class="equip-name rarity-' +
                rarityKey(it.rarity) +
                '">' +
                escHtml(it.name) +
                '</span>');
    ...
    `;
                ...
                root.querySelectorAll('.talent-unlock').forEach((btn) => {
                    btn.addEventListener('click', () => {
                        ...
                        if (unlockTalent(p, id)) {
                            ...
                            try {
                                refreshCharacterSheetLiveValues(p, body);
                            }
                            catch (_) { }
        ...
        try {
            wireSheetAccordions(panelsWrapper);
        }
        catch (_) { }
        // --- TAB SWITCH LOGIC -----------------------------------------------------
...
function openEnemySheet() {
    ...
    const enemyElementInfoText = (() => {
        ...
        if (enemy.affinities) {
            ...
            if (wk.length)
                parts.push('Weak: ' + wk.join(', '));
            if (rk.length)
                parts.push('Resist: ' + rk.join(', '));
        }
        if (enemy.elementalResists && typeof enemy.elementalResists === 'object') {
            ...
            if (ek.length)
                parts.push('Flat resist: ' + ek.join(', '));
        }
        return parts.join(' • ');
    })();
    ...
    function describeAffix(id) {
        ...
        if (def.vampiricHealPct)
            parts.push('Heals ' + Math.round(def.vampiricHealPct * 100) + '% of damage dealt');
        if (def.thornsReflectPct)
            parts.push('Reflects ' + Math.round(def.thornsReflectPct * 100) + '% of damage taken');
        if (def.chillChance)
            parts.push('On hit: ' + fmtPct(def.chillChance) + ' to apply Chilled (' + (def.chillTurns || 1) + 't)');
        if (def.bleedChance)
            parts.push('On hit: ' + fmtPct(def.bleedChance) + ' to apply Bleed (' + (def.bleedTurns || 2) + 't)');
        if (def.onShieldCastNextDmgPct)
            parts.push('After casting a shield: next damage +' + Math.round(def.onShieldCastNextDmgPct) + '%');
        if (def.onKillGain && def.onKillGain.key)
            parts.push('On kill: +' + def.onKillGain.amount + ' ' + def.onKillGain.key);
        if (def.hexTurns)
            parts.push('On hit: applies Hex (' + def.hexTurns + 't)');
        if (def.berserkThreshold)
            parts.push('Below ' + Math.round(def.berserkThreshold * 100) + '% HP: +' + Math.round((def.berserkAtkPct || 0) * 100) + '% attack');
        if (def.regenPct)
            parts.push('Regenerates ' + Math.round(def.regenPct * 100) + '% max HP at end of turn');
    ...
    `;
        ...
        const abilityLines = (() => {
            ...
            if (!arr.length)
                return '<div class="modal-subtitle">No special abilities.</div>';
            return arr
                .map((aid) => {
    ...
    `;
        ...
        const eliteHtml = enemy.isElite
            ...
            : '';
        const statusParts = [];
        if (enemy.bleedTurns && enemy.bleedTurns > 0)
            statusParts.push('Bleeding (' + enemy.bleedTurns + 't)');
...
function autoDistributeSkillPoints(p) {
    ...
    const baseWeights = CLASS_STARTING_SKILLS[p.classId] || CLASS_STARTING_SKILLS.default;
    ...
    const deltas = keys
        ...
        .filter(Boolean);
    addLog(`Auto-distributed ${pointsToSpend} skill point(s): ${deltas.join(', ')}.`, 'good');
}
function openSkillLevelUpModal() {
    ...
    if (closeBtn)
        closeBtn.style.display = 'none'; // force choice
...
(function ensureAbilityElementMetadata() {
    function inferFromText(text) {
        ...
        if (key.indexOf('fire') >= 0 || key.indexOf('flame') >= 0 || key.indexOf('ember') >= 0)
            return 'fire';
        if (key.indexOf('frost') >= 0 || key.indexOf('ice') >= 0 || key.indexOf('chill') >= 0)
            return 'frost';
        if (key.indexOf('arcane') >= 0)
            return 'arcane';
        if (key.indexOf('shadow') >= 0 || key.indexOf('void') >= 0 || key.indexOf('nec') >= 0)
            return 'shadow';
        if (key.indexOf('holy') >= 0 || key.indexOf('light') >= 0 || key.indexOf('radi') >= 0 || key.indexOf('divin') >= 0)
            return 'holy';
        if (key.indexOf('poison') >= 0 || key.indexOf('toxin') >= 0 || key.indexOf('venom') >= 0 || key.indexOf('decay') >= 0)
            return 'poison';
        if (key.indexOf('storm') >= 0 || key.indexOf('lightning') >= 0 || key.indexOf('tempest') >= 0 || key.indexOf('thunder') >= 0)
            return 'lightning';
        if (key.indexOf('earth') >= 0 || key.indexOf('stone') >= 0 || key.indexOf('quake') >= 0)
            return 'earth';
        if (key.indexOf('nature') >= 0 || key.indexOf('thorn') >= 0 || key.indexOf('vine') >= 0 || key.indexOf('spirit') >= 0)
            return 'nature';
        return null;
    }
    ...
    function ensureOneAbility(abId, ab, effectFn) {
        ...
        try {
            if (Array.isArray(ab.tags) && ab.tags.indexOf('physical') >= 0) {
                ...
                if (ab.elementType)
                    ab.elementType = normalizeElementType(ab.elementType);
                if (!ab.elementType)
                    delete ab.elementType;
                return;
            }
        }
        catch (_) { }
        ...
        if (!Array.isArray(ab.tags))
            ab.tags = [];
        if (ab.tags.indexOf('physical') < 0)
            ab.tags.push('physical');
    }
    try {
...
function normalizeElementType(elementType) {
    ...
    if (k === 'piercing' || k === 'slashing' || k === 'blunt' || k === 'physical')
        return null;
...
function getPlayerElementalBonusPct(elementType) {
    ...
    if (!p || !p.stats || !p.stats.elementalBonuses || !et)
        return 0;
...
function getPlayerElementalResistPct(elementType) {
    const p = state.player;
    const et = normalizeElementType(elementType);
    if (!p || !p.stats || !p.stats.elementalResists || !et)
        return 0;
    const cap = Number(PLAYER_RESIST_CAP) || 75;
...
function applyPlayerOnHitEffects(damageDealt, elementType) {
    ...
    const resolvedElementType = normalizeElementType(elementType || state.lastPlayerDamageElementType || null);
    // Lifesteal
...
function getEffectiveEnemyAttack(enemy) {
    if (!enemy)
        return 0;
    const base = Number(typeof enemy.baseAttack === 'number' ? enemy.baseAttack : enemy.attack || 0);
    const down = enemy.atkDownTurns && enemy.atkDownTurns > 0
...
function getEffectiveEnemyMagic(enemy) {
    if (!enemy)
        return 0;
    const base = Number(typeof enemy.baseMagic === 'number' ? enemy.baseMagic : enemy.magic || 0);
    const down = enemy.magDownTurns && enemy.magDownTurns > 0
...
function applyEnemyAtkDown(enemy, flatAmount, turns) {
    ...
    if (amt <= 0 || t <= 0)
        return;
    const baseAtk = Number(typeof enemy.baseAttack === 'number' ? enemy.baseAttack : enemy.attack || 0);
...
function applyEnemyPostureFromPlayerHit(enemy, damageDealt, meta = {}) {
    ...
    if (typeof enemy.posture !== 'number')
        enemy.posture = 0;
    ...
    if (meta && meta.isBasic)
        gain += 1;
    // Crits and interrupts spike posture more.
    if (state.lastPlayerHitWasCrit)
        gain = Math.round(gain * 1.5);
    if (meta && meta.tag === 'interrupt')
        gain += 2;
    if (enemy.isBoss)
        gain = Math.max(1, Math.round(gain * 0.75));
    if (enemy.isElite)
        gain = Math.max(1, Math.round(gain * 0.85));
...
function ensureCombatTurnState() {
    ...
    if (!state.combat.phase)
        state.combat.phase = 'player';
    if (typeof state.combat.busy !== 'boolean')
        state.combat.busy = false;
    if (typeof state.combat.round !== 'number')
        state.combat.round = 1;
    if (typeof state.combat.battleDrops !== 'number')
...
function ensureCombatPointers() {
    ...
    if (!state.currentEnemy) {
        ...
        if (state.enemy)
            state.currentEnemy = state.enemy;
    }
...
function _runPostPlayerTurnSequence() {
    ...
    if (typeof runPostPlayerTurnSequence !== 'function')
        return Promise.resolve();
    try {
...
function playerBasicAttack() {
    ...
    ctx.didDamage = true;
    _consumeCompanionBoonIfNeeded(p, ctx);
    if (ctx.consumeFirstHitBonus)
        st.firstHitBonusAvailable = false;
    addLog('You strike ' + enemy.name + ' for ' + dmg + ' damage.', 'good', { domain: 'combat', kind: 'damage', actor: 'player', breakdown: state._lastDamageBreakdown || null });
...
function playerInterrupt() {
    ...
    if (enemy.hp <= 0) {
        ...
        addLog('You interrupt for ' + dmg + ' damage.', 'good', { domain: 'combat', kind: 'damage', actor: 'player', breakdown: state._lastDamageBreakdown || null });
    }
    updateHUD();
    updateEnemyPanel();
    if (state.inCombat) {
...
function ensureEnemyAbilityStat(enemy, aid) {
    if (!enemy.memory)
        return { value: 0, uses: 0 };
    if (!enemy.memory.abilityStats)
        enemy.memory.abilityStats = {};
...
function tickEnemyCooldowns() {
    ...
    list.forEach((enemy) => {
        if (!enemy || !enemy.abilityCooldowns)
            return;
        Object.keys(enemy.abilityCooldowns).forEach((k) => {
            if (enemy.abilityCooldowns[k] > 0)
                enemy.abilityCooldowns[k] -= 1;
        });
    });
}
function scoreEnemyAbility(enemy, p, aid) {
    ...
    if (ab.type && ab.type.indexOf('damage') >= 0) {
        ...
        const enrageMult = enemy.enrageAtkPct
            ...
            : 1;
        const elemHit = ab.elementType || (isMagic ? enemy.magicElementType : enemy.attackElementType) || null;
...
function applyEnemyAbilityToPlayer(enemy, p, aid) {
    ...
    if (ab.type === 'buff') {
        enemy.enrageTurns = Math.max(enemy.enrageTurns || 0, ab.enrageTurns || 2);
        enemy.enrageAtkPct = Math.max(enemy.enrageAtkPct || 0, ab.enrageAtkPct || 0.2);
    ...
    if (ab.shatterShieldFlat && status.shield > 0) {
        ...
        if (shieldShattered > 0) {
            addLog('Your shield fractures for ' + shieldShattered + '!', 'system');
        }
    }
    // Apply absorption
    let remaining = dmg;
    ...
    if (ab.vulnerableTurns) {
        status.vulnerableTurns = Math.max(status.vulnerableTurns || 0, ab.vulnerableTurns);
    }
    if (ab.armorDown) {
        status.armorDown = Math.max(status.armorDown || 0, ab.armorDown);
        status.armorDownTurns = Math.max(status.armorDownTurns || 0, debuffTurns);
    }
    if (ab.atkDown) {
        status.atkDown = Math.max(status.atkDown || 0, ab.atkDown);
    ...
    if (ab.bleedTurns && ab.bleedBase) {
        const lvl = Number(enemy.level || (state.player ? state.player.level : 1) || 1);
    ...
    if (ab.drainResourcePct && state.player) {
        const cut = Math.max(0, Math.round((state.player.maxResource || 0) * ab.drainResourcePct));
        if (cut > 0) {
            state.player.resource = Math.max(0, (state.player.resource || 0) - cut);
            addLog('You feel your power ebb (' + cut + ').', 'danger');
        }
    }
    // Text
    if (dmg > 0) {
        addLog(enemy.name + ' uses ' + ab.name + ' on you for ' + dmg + ' damage.', 'danger', { domain: 'combat', kind: 'damage', actor: 'enemy', breakdown: state._lastDamageBreakdown || null });
    }
    else if (healDone > 0) {
...
function updateEnemyLearning(enemy, aid, reward) {
    ...
    enemy.memory.exploration = Math.max(0.06, (enemy.memory.exploration || 0.2) * 0.996);
}
    ...
    try {
        ...
        if (enemy.intent && enemy.intent.aid) {
            ...
            if (enemy.intent.turnsLeft <= 0) {
                ...
                const shieldDelta = Math.max(0, beforeShield - (p.status && p.status.shield ? p.status.shield : 0));
...
function applyEndOfTurnEffectsEnemy(enemy) {
    if (enemy.bleedTurns && enemy.bleedTurns > 0 && enemy.bleedDamage) {
        ...
        enemy.bleedTurns -= 1;
        addLog(enemy.name + ' bleeds for ' + enemy.bleedDamage + ' damage.', 'good', { domain: 'combat', kind: 'status', actor: 'player', effect: 'bleed' });
    ...
    if (enemy.burnTurns && enemy.burnTurns > 0 && enemy.burnDamage) {
        ...
        enemy.burnTurns -= 1;
        addLog(enemy.name + ' burns for ' + enemy.burnDamage + ' damage.', 'good', { domain: 'combat', kind: 'status', actor: 'player', effect: 'burn' });
...
function applyStartOfTurnEffectsPlayer(p) {
    ...
    if (!p)
        p = (state && state.player) ? state.player : (typeof PLAYER !== 'undefined' ? PLAYER : null);
...
function beginPlayerTurn() {
    ...
    if (c._lastPlayerTurnRoundApplied === round)
        return;
...
function recordBattleResult(outcome) {
    ...
    if (dd.tooEasyStreak >= threshold && dd.band < 2) {
        ...
        dd.tooEasyStreak = 0;
        changed = true;
        addLog('The realm grows more dangerous as you dominate your foes.', 'system');
    }
    // ramp down when you're struggling
        ...
        dd.struggleStreak = 0;
        changed = true;
        addLog('The realm seems to ease up as your struggles are noticed.', 'system');
    }
    if (changed) {
        updateHUD();
        requestSave('legacy');
...
function handleEnemyDefeat(enemyArg) {
    return withSaveTxn('combat:enemyDefeat', () => {
        ...
        if (enemy._defeatHandled)
            return;
        enemy._defeatHandled = true;
        // Mark dead
        enemy.hp = 0;
        const rarityTag = enemy.rarityLabel && Number.isFinite(enemy.rarityTier) && enemy.rarityTier >= 3
            ? ' [' + enemy.rarityLabel + ']'
            : '';
        const all = getAllEnemies();
        const alive = getAliveEnemies();
        ...
        addLog('You defeated ' + enemy.name + (enemy.isElite ? ' [Elite]' : '') + rarityTag + '!', 'good');
        ...
        if (dropsSoFar < dropCap && rand('loot.drop') < dropChance) {
            ...
            if (drops && drops.length) {
                ...
                if (c)
                    c.battleDrops = (c.battleDrops || 0) + 1;
            }
        }
        // World event (consumed by questEvents + autosave plugins)
        try {
            _engine && _engine.emit && _engine.emit('world:enemyDefeated', { enemy });
        }
        catch (_) { }
        // Legacy quest hook (fallback when questEvents plugin isn't present)
        if (!_questEventsEnabled()) {
            try {
                quests && quests.applyQuestProgressOnEnemyDefeat && quests.applyQuestProgressOnEnemyDefeat(enemy);
            }
            catch (_) { }
        }
...
function handlePlayerDefeat() {
    ...
    // Clamp to dead state
    if (state.player && !state.flags.godMode)
        state.player.hp = 0;
...
function tryFlee() {
    ...
    if (rand('encounter.pick') < chance) {
        ...
        state.enemies = [];
        resetPlayerCombatStatus(state.player);
        setScene('On the Path', 'You catch your breath after fleeing. The forest remains dangerous, but you live to fight again.');
        updateHUD();
        updateEnemyPanel();
        renderActions();
        requestSave('legacy');
...
function applyDirectDamageToPlayer(player, amount, opts = {}) {
    ...
    if (opts && opts.source) {
        try {
            addLog(opts.source + ' deals ' + dmg + ' damage.', 'danger', { domain: 'combat', kind: 'damage', actor: 'enemy', breakdown: null });
        }
        catch (_) { }
    }
    ...
    if (!god) {
        ...
        if (state && state.inCombat && player.hp <= 0)
            player.hp = 1;
    }
    try {
        if (state && state.inCombat)
...
function isAreaUnlocked(areaId) {
    ...
    if (!flags.mainQuestAccepted && areaId !== 'forest')
        return false;
    if (areaId === 'forest') {
        return true;
    }
    if (areaId === 'ruins') {
        return !!(flags.goblinBossDefeated ||
    ...
    if (areaId === 'frostpeak') {
        return !!(flags.marshWitchDefeated ||
    ...
    if (areaId === 'catacombs') {
        return !!(flags.frostGiantDefeated ||
            flags.catacombsUnlocked ||
            flags.lichDefeated ||
            flags.obsidianKingDefeated);
    }
    if (areaId === 'keep') {
        return !!(flags.lichDefeated ||
            flags.keepUnlocked ||
            flags.obsidianKingDefeated);
    }
    if (areaId === 'oathgrove') {
...
function getAreaDisplayName(areaId) {
    if (areaId === 'village')
        return 'Emberwood Village';
    if (areaId === 'forest')
        return 'Emberwood Forest';
    if (areaId === 'ruins')
        return 'Ruined Spire';
    if (areaId === 'marsh')
        return 'Ashen Marsh';
    if (areaId === 'frostpeak')
        return 'Frostpeak Pass';
    if (areaId === 'catacombs')
        return 'Sunken Catacombs';
    if (areaId === 'keep')
        return 'Obsidian Keep';
    if (areaId === 'oathgrove')
        return 'The Oathgrove';
    if (areaId === 'blackbarkDepths')
        return 'Blackbark Depths';
    if (areaId === 'starfallRidge')
        return 'Starfall Ridge';
    return areaId;
}
...
function exploreArea() {
    ...
    if (quests.handleExploreQuestBeats(area))
        return;
    // --- WANDERING MERCHANT (outside village) ---------------------------------
    if (area !== 'village') {
        // Small chance each explore click to meet a traveling merchant
        if (rand('encounter.rare') < 0.1) {
            let sceneText = 'Along the road, a lone cart creaks to a stop. A cloaked figure raises a hand in greeting.';
            if (area === 'forest') {
                sceneText =
    ...
    if (encounterList.length && rand('encounter.listUse') < 0.7) {
        const id = encounterList[randInt(0, encounterList.length - 1, 'encounter.listPick')];
        startBattleWith(id);
        return;
    }
    // --- NO ENCOUNTER: FLAVOR TEXT --------------------------------------------
    let title = 'Exploring';
    let text = 'You search the surroundings but find only rustling leaves and distant cries.';
    if (area === 'village') {
...
function levelUp() {
    ...
    addLog('You reach level ' + p.level + '! Choose a skill to improve.', 'good');
...
function openGovernmentModal() {
    ...
    openModal('Realm & Government', (body) => {
        ...
        monarchLine.textContent = `${summary.monarchTitle || 'Ruler'} ${summary.monarchName || 'Unknown'}`;
        ...
        if (gov && Array.isArray(gov.history) && gov.history.length) {
            ...
            recent.forEach((ev) => {
                ...
                const dayLabel = typeof ev.day === 'number' ? `Day ${ev.day}` : 'Unknown day';
...
function openInGameSettingsModal() {
    openModal('Settings', (body) => {
        ...
        const engineSettings = (() => {
            try {
                return _engine && _engine.getService ? _engine.getService('settings') : null;
            }
            catch (_) {
                return null;
            }
        ...
        // Text size (named buckets -> numeric scale)
        {
            ...
            sel.addEventListener('change', () => {
                ...
                try {
                    if (engineSettings && engineSettings.set)
                        engineSettings.set('a11y.textScale', scale);
                }
                catch (_) { }
                requestSave('legacy');
            });
        ...
        // High contrast (tri-state: auto/on/off)
        {
            ...
            sel.addEventListener('change', () => {
                ...
                try {
                    if (engineSettings && engineSettings.set) {
                        if (v === 'on')
                            engineSettings.set('a11y.highContrast', true);
                        else if (v === 'off')
                            engineSettings.set('a11y.highContrast', false);
                        else
                            engineSettings.set('a11y.highContrast', 'auto');
                    }
                }
                catch (_) { }
                requestSave('legacy');
...
const saveManager = createSaveManager({
    ...
    ensureCombatPointersBeforeSave: () => { try {
        ensureCombatPointers();
    }
    catch (_) { } },
    runIntegrityAudit,
...
function exportCurrentSaveToFile() {
    try {
        ...
        if (!ok)
            alert('Export failed. Your browser may be blocking downloads.');
        return ok;
    }
    catch (e) {
    ...
    openModal('Save / Load', (body) => {
        ...
        if (state.player && state.player.hp > 0) {
            ...
            btnNew.addEventListener('click', () => {
                const defaultLabel = (state.player.name || 'Hero') +
                    ' Lv ' +
...
function initSettingsFromState() {
    ...
    if (!volumeSlider && !textSpeedSlider && !settingsDifficulty && !autoEquipLootEl && !highContrastEl && !textSizeEl && !colorSchemeEl && !uiScaleEl && !showCombatNumbersEl && !autoSaveEl)
        return;
    ...
    if (musicToggleEl)
        musicToggleEl.checked = state.musicEnabled !== false;
    if (sfxToggleEl)
        sfxToggleEl.checked = state.sfxEnabled !== false;
    if (motionToggleEl)
        motionToggleEl.checked = !!state.settingsReduceMotion;
    ...
    try {
        ...
        if (highContrastEl && settings && typeof settings.get === 'function') {
            ...
            if (pref === true)
                highContrastEl.value = 'on';
            else if (pref === false)
                highContrastEl.value = 'off';
            else
                highContrastEl.value = 'auto';
        }
    }
    catch (_) { }
    ...
    try {
        ...
        if (textSizeEl && settings && typeof settings.get === 'function') {
            ...
            if (s <= 0.92)
                textSizeEl.value = 'small';
            else if (s < 1.05)
                textSizeEl.value = 'default';
            else if (s < 1.16)
                textSizeEl.value = 'large';
            else
                textSizeEl.value = 'xlarge';
        }
    }
    catch (_) { }
    if (autoEquipLootEl)
        autoEquipLootEl.checked = !!state.settingsAutoEquipLoot;
    if (showCombatNumbersEl)
        showCombatNumbersEl.checked = (state.settingsShowCombatNumbers !== false);
    if (autoSaveEl)
        autoSaveEl.checked = (state.settingsAutoSave !== false);
...
function applySettingsChanges() {
    ...
    if (volumeSlider) {
        ...
        try {
            engineSettings && engineSettings.set && engineSettings.set('audio.masterVolume', state.settingsVolume);
        }
        catch (_) { }
    }
    if (textSpeedSlider) {
        ...
        try {
            engineSettings && engineSettings.set && engineSettings.set('ui.textSpeed', state.settingsTextSpeed);
        }
        catch (_) { }
    }
    ...
    if (musicToggle) {
        ...
        try {
            engineSettings && engineSettings.set && engineSettings.set('audio.musicEnabled', !!state.musicEnabled);
        }
        catch (_) { }
    ...
    if (sfxToggle) {
        ...
        try {
            engineSettings && engineSettings.set && engineSettings.set('audio.sfxEnabled', !!state.sfxEnabled);
        }
        catch (_) { }
    ...
    if (autoEquipToggle) {
        ...
        try {
            engineSettings && engineSettings.set && engineSettings.set('gameplay.autoEquipLoot', !!state.settingsAutoEquipLoot);
        }
        catch (_) { }
    }
    // Color scheme
    ...
    if (colorSchemeEl) {
        ...
        try {
            engineSettings && engineSettings.set && engineSettings.set('ui.colorScheme', scheme);
        }
        catch (_) { }
    }
    // UI scale
    ...
    if (uiScaleEl) {
        ...
        try {
            engineSettings && engineSettings.set && engineSettings.set('ui.scale', scale);
        }
        catch (_) { }
    ...
    if (showCombatNumbersEl) {
        ...
        try {
            engineSettings && engineSettings.set && engineSettings.set('gameplay.showCombatNumbers', !!state.settingsShowCombatNumbers);
        }
        catch (_) { }
    }
    // Auto-save
    ...
    if (autoSaveEl) {
        ...
        try {
            engineSettings && engineSettings.set && engineSettings.set('gameplay.autoSave', !!state.settingsAutoSave);
        }
        catch (_) { }
    }
...
function setReduceMotionEnabled(enabled) {
    ...
    try {
        ...
        const settings = _engine && _engine.getService ? _engine.getService('settings') : null;
        if (settings && settings.set)
            settings.set('a11y.reduceMotion', state.settingsReduceMotion);
    }
    catch (_) { }
    applyMotionPreference();
}
    ...
    openModal('Feedback / Bug Report', (bodyEl) => {
        ...
        if (btnClearCrash) {
            ...
            btnClearCrash.addEventListener('click', () => {
                ...
                if (status)
                    status.textContent = '🧹 Cleared last crash report.';
...
function qaScanNegativeCounters(s) {
    ...
    try {
        const p = s && s.player;
        if (p && typeof p.gold === 'number' && p.gold < 0)
            issues.push('player.gold < 0 (' + p.gold + ')');
        if (p && typeof p.hp === 'number' && p.hp < 0)
            issues.push('player.hp < 0 (' + p.hp + ')');
        if (p && typeof p.maxHp === 'number' && p.maxHp < 0)
            issues.push('player.maxHp < 0 (' + p.maxHp + ')');
        if (p && typeof p.resource === 'number' && p.resource < 0)
            issues.push('player.resource < 0 (' + p.resource + ')');
        if (p && typeof p.maxResource === 'number' && p.maxResource < 0)
            issues.push('player.maxResource < 0 (' + p.maxResource + ')');
        if (p && Array.isArray(p.inventory)) {
            p.inventory.forEach((it, i) => {
                if (!it)
                    return;
                if (typeof it.quantity === 'number' && it.quantity < 0)
                    issues.push('player.inventory[' + i + '].quantity < 0 (' + it.quantity + ')');
            });
        }
        const bank = s && s.bank;
        if (bank && typeof bank.balance === 'number' && bank.balance < 0)
            issues.push('bank.balance < 0 (' + bank.balance + ')');
        if (bank && typeof bank.investments === 'number' && bank.investments < 0)
            issues.push('bank.investments < 0 (' + bank.investments + ')');
        if (bank && bank.loan && typeof bank.loan.balance === 'number' && bank.loan.balance < 0)
            issues.push('bank.loan.balance < 0 (' + bank.loan.balance + ')');
                ...
                for (const merchant in regionObj) {
                    ...
                    for (const key in bucket) {
                        ...
                        if (typeof v === 'number' && v < 0)
                            issues.push('merchantStock.' + region + '.' + merchant + '.' + key + ' < 0 (' + v + ')');
                    }
                }
            }
        }
...
function qaScanStatSanity(s) {
    const issues = [];
    const push = (msg) => { if (issues.length < 140)
        issues.push(msg); };
    try {
        const p = s && s.player;
        if (!p)
        ...
        const pct = (v, name, min, max) => {
            const n = num(v);
            if (n === null)
                return push(name + ' is non-finite (' + String(v) + ')');
            if (n < min - 1e-6)
                push(name + ' below ' + min + '% (' + n + ')');
            if (n > max + 1e-6)
                push(name + ' above ' + max + '% (' + n + ')');
        };
        const nonneg = (v, name) => {
            const n = num(v);
            if (n === null)
        ...
        if (num(p.maxHp) !== null && num(p.hp) !== null && num(p.hp) > num(p.maxHp) + 1e-6)
            push('player.hp exceeds maxHp (' + p.hp + ' > ' + p.maxHp + ')');
        if (num(p.maxResource) !== null && num(p.resource) !== null && num(p.resource) > num(p.maxResource) + 1e-6)
            push('player.resource exceeds maxResource (' + p.resource + ' > ' + p.maxResource + ')');
        const st = p.stats || {};
        ...
        const scanElemVals = (m, label, min, max) => {
            ...
            Object.keys(m).forEach((k) => {
                const v = num(m[k]);
                if (v === null)
                    return push(label + '.' + String(k) + ' is non-finite (' + String(m[k]) + ')');
                if (v < min - 1e-6)
                    push(label + '.' + String(k) + ' below ' + min + ' (' + v + ')');
                if (v > max + 1e-6)
                    push(label + '.' + String(k) + ' above ' + max + ' (' + v + ')');
            });
        };
        ...
        if (st.elementalResists && typeof st.elementalResists === 'object') {
            Object.keys(st.elementalResists).forEach((k) => {
                ...
                if (v !== null && v > 200)
                    push('player.stats.elementalResists.' + String(k) + ' extremely high (' + v + '%)');
            });
        }
    }
    catch (_) { }
...
function qaScanCombatRuntimeSanity(s) {
    const issues = [];
    const push = (msg) => { if (issues.length < 120)
        issues.push(msg); };
    try {
        if (!s || typeof s !== 'object')
            return ['state is null/invalid'];
        const c = s.combat;
        if (!c || typeof c !== 'object')
            return issues;
        // Busy should never be true when not in combat.
        if (!s.inCombat && c.busy)
            push('combat.busy=true while inCombat=false');
        if (typeof c.round === 'number' && (!Number.isFinite(c.round) || c.round < 1))
            push('combat.round invalid (' + String(c.round) + ')');
        if (c.phase && ['player', 'enemy', 'loot', 'win', 'lose'].indexOf(String(c.phase)) < 0)
            push('combat.phase unknown (' + String(c.phase) + ')');
        // Action context multipliers should be finite if present
        const ctx = s && s._playerAbilityCtx;
        if (ctx && typeof ctx === 'object') {
            if (ctx.dmgMult !== undefined && !(typeof ctx.dmgMult === 'number' && Number.isFinite(ctx.dmgMult)))
                push('_playerAbilityCtx.dmgMult non-finite (' + String(ctx.dmgMult) + ')');
            if (ctx.healMult !== undefined && !(typeof ctx.healMult === 'number' && Number.isFinite(ctx.healMult)))
                push('_playerAbilityCtx.healMult non-finite (' + String(ctx.healMult) + ')');
        }
    }
    catch (_) { }
    return issues;
...
function qaScanElementKeyIssues(s) {
    ...
    const scanMap = (obj, label) => {
        ...
        Object.keys(obj).forEach((raw) => {
            ...
            if (r !== nk)
                push(label + ': non-normalized key "' + String(raw) + '" -> "' + nk + '"');
            if (!buckets[nk])
                buckets[nk] = [];
            buckets[nk].push(raw);
        });
...
function qaScanAbilityElementCoverage() {
    const issues = [];
    const push = (msg) => { if (issues.length < 200)
        issues.push(msg); };
    ...
    const scan = (bucket, label) => {
        ...
        Object.keys(bucket).forEach((id) => {
            const ab = bucket[id];
            if (!ab || typeof ab !== 'object')
                return push(label + '.' + id + ': missing def');
            ...
            if (!et && !hasPhysical)
                push(label + '.' + id + ': missing elementType and missing physical tag');
            if (et && !allowed[et])
                push(label + '.' + id + ': unknown elementType "' + String(ab.elementType) + '"');
            if (et && hasPhysical)
                push(label + '.' + id + ': has both elementType "' + et + '" and physical tag');
        });
    };
    try {
...
function qaScanTalentIntegrity(s) {
    const issues = [];
    const push = (msg) => { if (issues.length < 160)
        issues.push(msg); };
    try {
        const p = s && s.player;
        if (!p || typeof p !== 'object')
            return issues;
        ensurePlayerTalents(p);
        if (!Number.isFinite(Number(p.talentPoints)))
            push('player.talentPoints is not finite');
        if (Number(p.talentPoints) < 0)
            push('player.talentPoints < 0 (' + String(p.talentPoints) + ')');
        const classId = String(p.classId || '');
        const classTalents = (TALENT_DEFS && TALENT_DEFS[classId]) ? TALENT_DEFS[classId] : [];
        if (!classTalents || !classTalents.length)
            push('no talent table found for classId="' + classId + '"');
        const allowed = {};
        classTalents.forEach((t) => { if (t && t.id)
            allowed[t.id] = true; });
        const owned = p.talents && typeof p.talents === 'object' ? Object.keys(p.talents).filter((k) => p.talents[k]) : [];
        owned.forEach((tid) => {
            if (!allowed[tid])
                push('talent unlocked but not in class table: ' + String(tid));
        });
        ...
        if (classTalents && classTalents.length) {
            ...
            classTalents.forEach((t) => {
                if (!t || !t.id)
                    return;
                if (seen[t.id])
                    push('duplicate talent id in table: ' + t.id);
                seen[t.id] = true;
            });
            if (classTalents.length !== 8)
                push('class talent table count != 8 (' + classTalents.length + ')');
        }
    }
    catch (_) { }
    return issues;
    ...
    try {
        ...
        if (p && p.cooldowns && typeof p.cooldowns === 'object') {
            Object.keys(p.cooldowns).forEach((k) => {
                ...
                if (!Number.isFinite(v))
                    push('player.cooldowns.' + k + ' not finite (' + String(p.cooldowns[k]) + ')');
                if (v < 0)
                    push('player.cooldowns.' + k + ' < 0 (' + v + ')');
                if (v > 99)
                    push('player.cooldowns.' + k + ' unusually high (' + v + ')');
            });
        }
        if (s && s.inCombat && Array.isArray(s.enemies)) {
            s.enemies.forEach((e, i) => {
                ...
                if (e.abilityCooldowns && typeof e.abilityCooldowns === 'object') {
                    Object.keys(e.abilityCooldowns).forEach((k) => {
                        ...
                        if (!Number.isFinite(v))
                            push('enemies[' + i + '].abilityCooldowns.' + k + ' not finite');
                        if (v < 0)
                            push('enemies[' + i + '].abilityCooldowns.' + k + ' < 0 (' + v + ')');
                    });
                }
            });
        }
...
function qaScanReferenceIntegrity(s) {
    ...
    try {
        ...
        const p = s.player;
        if (!p)
            push('state.player missing');
        if (p) {
            if (!PLAYER_CLASSES[p.classId])
                push('player.classId unknown: ' + String(p.classId));
            if (Array.isArray(p.equippedSpells)) {
                p.equippedSpells.forEach((id) => { if (!ABILITIES || !ABILITIES[id])
                    push('player.equippedSpells missing ability: ' + String(id)); });
            }
            if (Array.isArray(p.spells)) {
                p.spells.forEach((id) => { if (!ABILITIES || !ABILITIES[id])
                    push('player.spells missing ability: ' + String(id)); });
            }
        }
        if (s.inCombat) {
            if (!Array.isArray(s.enemies) || !s.enemies.length)
                push('inCombat=true but enemies[] missing/empty');
            if (s.currentEnemy) {
                if (Array.isArray(s.enemies) && s.enemies.indexOf(s.currentEnemy) < 0)
                    push('currentEnemy not present in enemies[] (pointer leak)');
            }
            const idx = Number(s.targetEnemyIndex);
            if (Array.isArray(s.enemies) && Number.isFinite(idx)) {
                if (idx < 0 || idx >= s.enemies.length)
                    push('targetEnemyIndex out of range: ' + String(s.targetEnemyIndex));
            }
        }
    }
    catch (_) { }
...
function qaCollectBugScannerFindings(s) {
    ...
    const hasIssues = !!(nonFinite.length ||
        negatives.length ||
        elementKeys.length ||
        refs.length ||
...
function classifyIntegritySeverity(invariant, scanners) {
    try {
        ...
        if (counts && counts.nonFinite && counts.nonFinite > 0)
            return 'critical';
        ...
        for (let i = 0; i < invIssues.length; i++) {
            ...
            if (code.indexOf('.missing') >= 0)
                return 'critical';
            if (code.indexOf('.nan') >= 0)
                return 'critical';
            if (code.indexOf('time.dayIndex.bad') >= 0)
                return 'critical';
            if (code.indexOf('player.hp.range') >= 0)
                return 'warn';
        }
        // Reference issues + negative counters are usually recoverable but should be investigated.
        if (counts) {
            if (counts.negatives && counts.negatives > 0)
                return 'warn';
            if (counts.refs && counts.refs > 0)
                return 'warn';
            if (counts.combatRuntime && counts.combatRuntime > 0)
                return 'warn';
            if (counts.statSanity && counts.statSanity > 0)
                return 'warn';
            if (counts.elementKeys && counts.elementKeys > 0)
                return 'warn';
        }
...
function runIntegrityAudit(s, stage, opts = {}) {
    ...
    return perfWrap(st, 'qa:integrityAudit', { stage: label }, () => {
        ...
        try {
            if (!st.debug || typeof st.debug !== 'object')
                st.debug = {};
            st.debug.lastAudit = {
                stage: report.stage,
                time: report.time,
...
function formatIntegrityAuditReport(report) {
    ...
    try {
        if (r.time)
            lines.push('Time (UTC): ' + new Date(Number(r.time)).toISOString());
    }
    catch (_) { }
    lines.push('');
    if (inv && inv.ok) {
        ...
        if (issues.length > 16)
            lines.push('  … +' + String(issues.length - 16) + ' more');
    }
    lines.push('');
    const fmtCount = (k, label) => {
        const v = Number(counts && counts[k]);
        if (Number.isFinite(v) && v > 0)
            lines.push('Scanner: ' + label + ' — ' + v);
    };
    ...
    if (!hasAny)
        lines.push('Scanners: ✓ No findings');
    ...
    try {
        try {
            if (typeof window !== 'undefined')
                live.ref = window.__emberwoodStateRef;
        }
        catch (_) { }
        ...
        if (live.recordInput)
            recordInput = () => { };
        try {
            if (typeof setUiDisabled === 'function')
                setUiDisabled(true);
        }
        catch (_) { }
        ...
        try {
            setDeterministicRngEnabled(s, true);
        }
        catch (_) { }
        try {
            setRngSeed(s, seed);
        ...
        try {
            if (!s.time)
                initTimeState(s);
            if (!s.villageEconomy)
                initVillageEconomyState(s);
            if (!s.government)
                initGovernmentState(s, 0);
            ...
            for (let i = 0; i < lootRolls; i++) {
                ...
                if (Array.isArray(drop)) {
                    drop.forEach((it) => {
                        ...
                        if (cat === 'potion')
                            lootCounts.potions += 1;
                        else if (cat === 'weapon')
                            lootCounts.weapons += 1;
                        else if (cat === 'armor')
                            lootCounts.armor += 1;
        ...
        try {
            if (!live.state.debug || typeof live.state.debug !== 'object')
                live.state.debug = {};
        ...
        try {
            ...
            syncGlobalStateRef();
            if (typeof window !== 'undefined' && live.ref)
                window.__emberwoodStateRef = live.ref;
        }
        catch (_) { }
        try {
            if (live.saveGame)
                saveGame = live.saveGame;
            if (live.updateHUD)
                updateHUD = live.updateHUD;
            if (live.recordInput)
                recordInput = live.recordInput;
        }
        catch (_) { }
        try {
            if (typeof setUiDisabled === 'function')
                setUiDisabled(!!live.uiWasDisabled);
        }
        catch (_) { }
        ...
        try {
            if (typeof quests !== 'undefined' && quests) {
                if (live.updateQuestBox)
                    quests.updateQuestBox = live.updateQuestBox;
                // Ensure the pinned-quest UI is consistent with the restored live state.
                if (typeof quests.updateQuestBox === 'function')
                    quests.updateQuestBox();
            }
        }
        catch (_) { }
...
function formatScenarioRunnerReport(res) {
    ...
    if (typeof r.ms === 'number')
        lines.push('Runtime: ' + String(Math.round(r.ms)) + ' ms');
    lines.push('');
...
function buildBugReportBundle() {
    ...
    const diag = {
        ...
        perfSnapshot: (() => { try {
            return qaCollectPerfSnapshotSync(state);
        }
        catch (_) {
            return null;
        } })(),
...
function formatBugReportBundle(bundle) {
    ...
    if (s.timeUtc)
        lines.push('Time (UTC): ' + String(s.timeUtc));
    if (s.saveSchema !== undefined)
        lines.push('Save schema: ' + String(s.saveSchema));
    if (s.userAgent)
        lines.push('UA: ' + String(s.userAgent));
    if (s.locale)
        lines.push('Locale: ' + String(s.locale));
    lines.push('');
    // Snapshot
    ...
    if (enemyName)
        lines.push('  Current enemy: ' + enemyName);
    lines.push('  Player: ' + String(p.name || 'Player') + ' • ' + String(p.classId || '?') + ' • Lv ' + String(p.level || '?'));
    if (p.maxHp)
        lines.push('  HP: ' + String(p.hp) + ' / ' + String(p.maxHp));
    if (p.maxResource !== undefined)
        lines.push('  Resource: ' + String(p.resource) + ' / ' + String(p.maxResource));
    if (p.gold !== undefined)
        lines.push('  Gold: ' + String(p.gold));
    lines.push('');
    // Issues
    const issues = [];
    ...
    if (scanners && scanners.hasIssues) {
        ...
        if (c.nonFinite)
            issues.push('scan: non-finite numbers (' + String(c.nonFinite) + ')');
        if (c.negatives)
            issues.push('scan: negative counters (' + String(c.negatives) + ')');
        if (c.elementKeys)
            issues.push('scan: element key anomalies (' + String(c.elementKeys) + ')');
        if (c.refs)
            issues.push('scan: reference integrity (' + String(c.refs) + ')');
        if (c.abilityElements)
            issues.push('scan: ability element tags (' + String(c.abilityElements) + ')');
        if (c.statSanity)
            issues.push('scan: derived stat sanity (' + String(c.statSanity) + ')');
        if (c.combatRuntime)
            issues.push('scan: combat runtime sanity (' + String(c.combatRuntime) + ')');
        if (c.talentIntegrity)
            issues.push('scan: talent wiring integrity (' + String(c.talentIntegrity) + ')');
    }
    lines.push('Findings');
    if (!issues.length) {
        ...
        if (issues.length > 12)
            lines.push('  … +' + String(issues.length - 12) + ' more');
    }
    lines.push('');
    ...
    if (_sc && _sc.hasIssues) {
        ...
        const show = (title, arr) => {
            ...
            if (arr.length > 6)
                lines.push('  … +' + String(arr.length - 6) + ' more');
            lines.push('');
        };
    ...
    if (diag.lastCrashReport) {
        ...
        if (cr.kind)
            lines.push('  Kind: ' + String(cr.kind));
        ...
        if (cr.message)
            lines.push('  Message: ' + String(cr.message));
        if (cr.stack) {
...
function runSmokeTests(opts = {}) {
    ...
    const _liveUiDisabledSnap = (() => {
        try {
            return typeof isUiDisabled === 'function' ? !!isUiDisabled() : false;
        }
        catch (_) {
            return false;
        }
    ...
    const _perfSnapLive = (() => {
        try {
            return qaCollectPerfSnapshotSync(live);
        }
        catch (_) {
            return null;
        }
    ...
    const _normalizePinnedQuestRef = (pinRaw) => {
        try {
            ...
            if (pinRaw && typeof pinRaw === 'object') {
                if (pinRaw.kind === 'main')
                    return { kind: 'main' };
                if (pinRaw.kind === 'side' && pinRaw.id)
                    return { kind: 'side', id: String(pinRaw.id) };
                return null;
            }
    ...
    const _prevSmoke = (() => {
        try {
            return !!(state && state.debug && state.debug.smokeTestRunning);
        }
        catch (_) {
            return false;
        }
    ...
    try {
        if (typeof console !== 'undefined') {
            console.error = (...args) => {
                ...
                try {
                    if (_liveConsoleError)
                        _liveConsoleError(...args);
                }
                catch (_) { }
            };
            console.assert = (cond, ...args) => {
                if (!cond)
                    consoleAssertLog.push(args.length ? args.map(_stringifyConsoleArg).join(' ') : 'console.assert failed');
                try {
                    if (_liveConsoleAssert)
                        _liveConsoleAssert(cond, ...args);
                }
                catch (_) { }
            };
        }
    ...
    const section = (title) => {
        if (currentSection)
            sectionStats.push(currentSection);
            ...
            try {
                if (state) {
                    ...
                    if (!state.combat || typeof state.combat !== 'object') {
                        ...
                        state.combat.phase = 'player';
                        state.combat.busy = false;
                        if (typeof state.combat.round !== 'number' || !Number.isFinite(state.combat.round))
                            state.combat.round = 1;
                        if (typeof state.combat.battleDrops !== 'number' || !Number.isFinite(state.combat.battleDrops))
                            state.combat.battleDrops = 0;
                    }
                }
            }
    ...
    try {
        ...
        test('level 12 unlock grants all class unlock spells', () => {
            ...
            classes.forEach((cid) => {
                ...
                unlocks.forEach((u) => {
                    if (u.level <= p.level && !p.spells.includes(u.spell))
                        p.spells.push(u.spell);
                });
        ...
        test('switchScreen ignores missing DOM nodes', () => {
            return withUiEnabled(() => {
                ...
                try {
                    ...
                    if (oldPlayMusicTrack)
                        playMusicTrack = oldPlayMusicTrack;
                    if (audioState && oldInteriorOpen !== null)
                        audioState.interiorOpen = oldInteriorOpen;
        ...
        test('combat sanity (damage/heal finite + clamped)', () => {
            ...
            try {
                ...
                if (_companionAct)
                    companionActIfPresent = _companionAct;
                if (_handleEnemyDefeat)
                    handleEnemyDefeat = _handleEnemyDefeat;
                ...
                if (p.stats)
                    p.stats.dodgeChance = snap.playerDodgeChance;
        ...
        test('elemental stats: gear elemental resists apply to player stats', () => {
            ...
            if (!(v > 0))
                throw new Error('expected arcane resist from robeApprentice');
        ...
        test('elemental stats: talent elemental resist applies to player stats', () => {
            ...
            if (v < 10)
                throw new Error('expected frost resist from warrior_frostward talent');
            p.equipment = snap.equipment;
            p.classId = snap.classId;
            p.level = snap.level;
            p.talents = snap.talents;
        ...
        test('enemy elemental resists scale with difficulty', () => {
            ...
            if (easyAff && hardAff) {
                Object.keys(easyAff).forEach((k) => {
                    ...
                    if (em < 1)
                        assert(hm <= em + 1e-6, 'expected hard resist multiplier <= easy for ' + k);
                });
            }
        });
        ...
        test('combat: enemy affix thorns reflects (and respects shield)', () => {
            ...
            try {
                ...
                if (_enemyTurn)
                    enemyTurn = _enemyTurn;
                if (_handlePlayerDefeat)
                    handlePlayerDefeat = _handlePlayerDefeat;
                state.inCombat = snap.inCombat;
                state.currentEnemy = snap.currentEnemy;
                p.hp = snap.playerHp;
                if (p.status)
        ...
        test('combat: enemy affix vampiric heals on hit', () => {
            ...
            try {
                ...
                if (ENEMY_ABILITIES && ENEMY_ABILITIES.enemyStrike)
                    ENEMY_ABILITIES.enemyStrike.undodgeable = true;
                if (p.status)
                    p.status.shield = 0;
                ...
                if (ENEMY_ABILITIES && ENEMY_ABILITIES.enemyStrike)
                    ENEMY_ABILITIES.enemyStrike.undodgeable = snap.strikeUndodgeable;
                state.inCombat = snap.inCombat;
                state.currentEnemy = snap.currentEnemy;
                p.hp = snap.playerHp;
                if (p.status)
        ...
        test('combat: elite rarity is uncommon-or-rare', () => {
            ...
            const rngSnap = (() => { try {
                return JSON.parse(JSON.stringify(state.rng));
            }
            catch (_) {
                return null;
            } })();
        ...
        test('combat: cooldown integrity (telegraph + interrupt)', () => {
            ...
            try {
                ...
                if (!_enemyTurn)
                    throw new Error('enemyTurn not available');
                enemyTurn();
        ...
        test('combat: Broken increases damage taken (deterministic)', () => {
            ...
            try {
                ...
                state.inCombat = snap.inCombat;
                state.currentEnemy = snap.currentEnemy;
                if (snap.seed !== null && state.debug)
                    state.debug.rngSeed = snap.seed;
                if (snap.idx !== null && state.debug)
                    state.debug.rngIndex = snap.idx;
            }
        });
            ...
            const runOne = (seed) => {
                ...
                for (let step = 0; step < QA_COUNTS.fuzz2Actions; step++) {
                    ...
                    if (r === 0) {
                        ...
                        if (inv.length)
                            equipItemFromInventory(rngInt(state, 0, inv.length - 1, 'smoke.fuzz2.equipIdx'), { stayOpen: true });
                    }
                    else if (r === 3) {
                        const inv = state.player && Array.isArray(state.player.inventory) ? state.player.inventory : [];
                        if (inv.length)
                            sellItemFromInventory(rngInt(state, 0, inv.length - 1, 'smoke.fuzz2.sellIdx'), 'village');
                    }
                    else if (r === 4) {
            ...
            for (let i = 0; i < Math.min(ids.length, 80); i++) {
                ...
                for (let d = 0; d < diffIds.length; d++) {
                    ...
                    if (built % 3 === 0) {
                        e.isElite = true;
                        try {
                            applyEliteModifiers(e, getActiveDifficultyConfig());
                        }
                        catch (_) { }
                    }
                    const cap = 1 + (built % 3);
                    try {
                        applyEnemyAffixes(e, { maxAffixes: cap });
                    }
                    catch (_) { }
                    try {
                        rebuildEnemyDisplayName(e);
                    }
                    catch (_) { }
                    try {
                    ...
                    ['attack', 'magic', 'armor', 'magicRes'].forEach((k) => {
                        if (e[k] !== undefined)
                            assert(isFiniteNum(Number(e[k])), 'enemy ' + k + ' invalid for ' + tid);
                    });
                    ...
                    built += 1;
                    if (built >= QA_COUNTS.stressEnemies)
                        break;
                }
                if (built >= QA_COUNTS.stressEnemies)
                    break;
                ...
                try {
                    ...
                    if (Array.isArray(drops)) {
                        ...
                        if (p.inventory.length > 60)
                            p.inventory.splice(0, p.inventory.length - 60);
                    }
                }
                catch (_) { }
                if (i % 3 === 2) {
        ...
        test('stress: unlock all talents for all classes yields finite derived stats', () => {
            ...
            classIds.forEach((cid) => {
                ...
                ['critChance', 'dodge', 'resistAll'].forEach((k) => {
                    if (s[k] !== undefined)
                        assert(isFiniteNum(Number(s[k])), cid + ': stat ' + k + ' invalid');
                });
            });
        });
        ...
        if (failCount > 0) {
            ...
            if (consoleErrorLog.length) {
                ...
                if (consoleErrorLog.length > 20)
                    lines.push('  … +' + (consoleErrorLog.length - 20) + ' more');
            }
            if (consoleAssertLog.length) {
                lines.push('');
                lines.push('console.assert failures (first 20):');
                consoleAssertLog.slice(0, 20).forEach((x) => lines.push('  - ' + x));
                if (consoleAssertLog.length > 20)
                    lines.push('  … +' + (consoleAssertLog.length - 20) + ' more');
            }
        }
        ...
        const bugHasIssues = !!((_bugReportLive && _bugReportLive.debug && Array.isArray(_bugReportLive.debug.invariantIssues) && _bugReportLive.debug.invariantIssues.length) ||
        ...
        try {
            if (typeof console !== 'undefined') {
                if (_liveConsoleError)
                    console.error = _liveConsoleError;
                if (_liveConsoleAssert)
                    console.assert = _liveConsoleAssert;
            }
        }
        catch (_) { }
        // Restore global functions first.
        if (_liveSaveGame)
            saveGame = _liveSaveGame;
        if (_liveUpdateHUD)
            updateHUD = _liveUpdateHUD;
        if (_liveRecordInput)
            recordInput = _liveRecordInput;
        ...
        try {
            if (typeof setUiDisabled === 'function')
                setUiDisabled(_liveUiDisabledSnap);
        }
        catch (_) { }
        ...
        try {
            if (state) {
                ...
                if (pin && pin.kind === 'main') {
                    ...
                    if (q && String(q.status) === 'active')
                        state.quests.pinned = JSON.parse(JSON.stringify(pin));
                }
        ...
        try {
            ...
            if (_liveUpdateHUD)
                _liveUpdateHUD();
            if (_liveUpdateEnemyPanel)
                _liveUpdateEnemyPanel();
        }
        catch (_) { }
    }
}
...
export function bootGame(engine) {
    ...
    try {
        ...
        _engine.use(createSavePolicyBridgePlugin({
            ...
            isUiDisabled: () => {
                try {
                    ...
                    if (ui && typeof ui.isUiDisabled === 'function')
                        return ui.isUiDisabled();
                }
                catch (_) { }
                return false;
            }
        ...
        _engine.use(createUiCommandsPlugin({
            openPauseMenu,
            openCharacterSheet,
            openQuestJournal: () => { try {
                quests && quests.openQuestJournal && quests.openQuestJournal();
            }
            catch (_) { } },
            toggleDiagnosticsOverlay: () => { try {
                openSmokeTestsModal && openSmokeTestsModal();
            }
            catch (_) { } },
            isEnemySheetOpen: () => { try {
                return !!isEnemyModalOpen();
            }
            catch (_) {
                return false;
            } },
        ...
        _engine.use(createGameCommandsPlugin({
            ...
            openElderRowan: () => { try {
                quests && quests.openElderRowanDialog && quests.openElderRowanDialog();
            }
            catch (_) { } },
            // UI helpers
            openInventory: (inCombat) => { try {
                openInventoryModal(!!inCombat);
            }
            catch (_) { } },
            openSpells: (inCombat) => { try {
                openSpellsModal(!!inCombat);
            }
            catch (_) { } },
            // Combat
            combatAttack: () => { try {
            ...
            inventoryUsePotion: (payload) => {
                try {
                    ...
                    if (idx >= 0)
                        usePotionFromInventory(idx, inCombat, { stayOpen: !inCombat });
                }
                catch (_) { }
            },
            ...
            // Bank
            bankDeposit: (payload) => { try {
                bankDeposit({ state, amt: payload?.amt, addLog, recordInput, updateHUD, saveGame: () => requestSave('bank:deposit') });
            }
            catch (_) { } },
            bankWithdraw: (payload) => { try {
                bankWithdraw({ state, amt: payload?.amt, addLog, recordInput, updateHUD, saveGame: () => requestSave('bank:withdraw') });
            }
            catch (_) { } },
            bankInvest: (payload) => { try {
                bankInvest({ state, amt: payload?.amt, addLog, recordInput, updateHUD, saveGame: () => requestSave('bank:invest') });
            }
            catch (_) { } },
            bankCashOut: () => { try {
                bankCashOut({ state, addLog, updateHUD, saveGame: () => requestSave('bank:cashOut') });
            }
            catch (_) { } },
            bankBorrow: (payload) => { try {
                bankBorrow({ state, amt: payload?.amt, addLog, recordInput, updateHUD, saveGame: () => requestSave('bank:borrow') });
            }
            catch (_) { } },
            bankRepay: (payload) => { try {
                bankRepay({ state, amt: payload?.amt, addLog, recordInput, updateHUD, saveGame: () => requestSave('bank:repay') });
            }
            catch (_) { } }
        }));
        ...
        _engine.on('engine:started', () => {
            ...
            try {
                _companionRuntime = _engine.getService('companionRuntime') || null;
            }
            catch (_) { }
        });
    }
        ...
        try {
            console.error('[bootGame] plugin registration failed', e);
        }
        catch (_) { }
    }
    // =============================================================================
    ...
    try {
        ...
        try {
            console.error('[bootGame] Engine start failed', e);
        }
        catch (_) { }
    }
        ...
        if (sched && typeof sched.after === 'function') {
            sched.after(0, () => {
                try {
                    ...
                    if (!a || !_engine || typeof _engine.emit !== 'function')
                        return;
