...
export function qaFormatPerfSnapshotText(snap) {
    ...
    if (boot && boot.durations) {
        ...
        if (d.prefetchMs !== null)
            lines.push('  Asset prefetch: ' + fmt(d.prefetchMs) + ' • Ready paint: ' + fmt(d.readyPaintMs));
        if (d.preflightMs !== null)
            lines.push('  Preflight scan: ' + fmt(d.preflightMs));
    }
    else {
    ...
    if (perf) {
        ...
        const fmtTriple = (r) => {
            ...
            if (wallA !== null && wallM !== null)
                s += ' (wall ' + String(wallA) + ' / ' + String(wallM) + ')';
            if (waitA !== null && waitM !== null && (waitA > 0 || waitM > 0))
                s += ' (wait ' + String(waitA) + ' / ' + String(waitM) + ')';
            return s;
        };
        lines.push('  Perf capture: ' + (perf.captureEnabled ? 'ON' : 'off') +
        ...
        if (Array.isArray(perf.worst) && perf.worst.length) {
            ...
            perf.worst.slice(0, 10).forEach((r) => {
                ...
                lines.push('    - [' + String(r.subsystem || 'other') + '] ' + String(r.label) +
