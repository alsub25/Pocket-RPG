...
export function getVillageEconomySummary(state) {
    ...
    if (govEffect && govEffect.hasData) {
        const pMod = typeof govEffect.prosperityModifier === "number"
            ? govEffect.prosperityModifier
            : 0; // -0.3 .. +0.3
        const sMod = typeof govEffect.safetyModifier === "number"
            ? govEffect.safetyModifier
            : 0; // -0.3 .. +0.3
        // Nudge the metrics toward government influence.
        // We keep it fairly gentle so economy still matters.
        effectiveProsperity = clamp(rawProsperity + Math.round(rawProsperity * pMod), 0, 100);
        effectiveTrade = clamp(rawTrade + Math.round(rawTrade * pMod * 0.7), 0, 100);
        effectiveSecurity = clamp(rawSecurity + Math.round(rawSecurity * sMod), 0, 100);
    }
...
export function getRestCost(state) {
    ...
    const today = t && typeof t.dayIndex === "number" ? t.dayIndex : 0;
...
export function handleEconomyDayTick(state, absoluteDay) {
    const econ = initVillageEconomyState(state);
    if (econ.lastDayUpdated === absoluteDay)
...
function _handleEconomyAfterPurchaseImpl(state, goldSpent, context) {
    ...
    if (!Number.isFinite(goldSpent) || goldSpent <= 0)
        return;
    if (context === "village") {
