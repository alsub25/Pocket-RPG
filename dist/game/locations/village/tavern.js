...
function isActiveDecree(state) {
    ...
    if (!eff || typeof eff.expiresOnDay !== "number")
        return false;
...
function buildRumorPool(state, econSummary) {
    ...
    if (hall && hall.councilRecess) {
        rumors.push("They say the council chambers are shuttered—no votes, no petitions, just sealed letters and long faces.");
    }
    else {
        const pet = hall && hall.currentPetition;
        if (pet && pet.status === "pending") {
            rumors.push("Folks whisper about a petition at the Town Hall—someone’s trying to sway the village before the vote lands.");
        }
    }
    ...
    rumors.push("Caravans swear goblins still stalk the Emberwood trails at night—especially when the fog settles low.", "Someone claims they saw a robed figure near the Ruined Spire, speaking to the dark like it could answer.", "Hunters talk about a huge wolf deeper in the forest—clever enough to herd smaller packs.", "The elder keeps old maps of the Spire locked away. If you knew the right question, you might get a peek.", "A traveler insists the dragon’s roar can still be heard on stormy nights—though nobody agrees on which direction it comes from.");
...
function buildTownSummaryModal({ openModal, closeModal, newTime, before, after, econNudge, moodChange, dailyReport }) {
    ...
    openModal('Town Summary', body => {
        ...
        const econDescBits = [];
        if (econTier)
            econDescBits.push(`Prices feel ${econTier.priceDescriptor}.`);
        if (after.econ) {
        ...
        if (econNudge && (econNudge.deltas?.prosperity || econNudge.deltas?.trade || econNudge.deltas?.security)) {
            ...
            const bits = [];
            if (d.prosperity)
                bits.push(`Prosperity ${d.prosperity > 0 ? '+' : ''}${d.prosperity}`);
            if (d.trade)
                bits.push(`Trade ${d.trade > 0 ? '+' : ''}${d.trade}`);
            if (d.security)
                bits.push(`Security ${d.security > 0 ? '+' : ''}${d.security}`);
            if (bits.length)
                econDescBits.push(`Decree nudge: ${bits.join(' · ')}.`);
        }
        addCard({
        ...
        if (typeof after.mood === 'number') {
            ...
            if (delta)
                moodBits.push(`Change: ${delta > 0 ? '+' : ''}${delta}.`);
            const reasons = moodChange?.reasons || [];
            if (delta && reasons.length)
                moodBits.push(`Why: ${reasons.join(' ')}`);
        }
        addCard({
            title: 'Villager Mood',
...
function makeCardFactory(body) {
    ...
    return function addCard({ title, rightTag, desc, actions }) {
        ...
        if (typeof rightTag === "string" && rightTag.includes("<"))
            meta.innerHTML = rightTag;
        else
            meta.textContent = rightTag || "";
        header.appendChild(meta);
        card.appendChild(header);
        if (desc)
            card.appendChild(el("div", { className: "modal-subtitle", text: desc }));
...
function runDayTicks(deps, absoluteDay) {
    const { state, addLog, runDailyTicks, handleEconomyDayTick, handleGovernmentDayTick, handleTownHallDayTick, handlePopulationDayTick } = deps;
...
export function openTavernModalImpl(deps) {
    const { state, openModal, addLog, recordInput, getVillageEconomySummary, getRestCost, handleEconomyAfterPurchase, jumpToNextMorning, updateHUD, updateTimeDisplay, saveGame, closeModal, openGambleModal, 
    // Optional quest hooks (provided by Main.js)
    questDefs, ensureQuestStructures, startSideQuest, advanceSideQuest, completeSideQuest, updateQuestBox, setScene } = deps;
    const player = state?.player;
    if (!player || typeof openModal !== "function")
        return;
        ...
        renderTavern = () => {
            ...
            const restBtn = el("button", {
                ...
                onClick: () => {
                    ...
                    try {
                        recordInput?.('tavern.rest', { cost, beforeDay });
                    }
                    catch (_) { }
                    ...
                    if (typeof player.maxResource === "number")
                        player.resource = player.maxResource;
                    // Clear common wound-over-time effects
                    if (player.status) {
                        if ("bleedTurns" in player.status)
                            player.status.bleedTurns = 0;
                        if ("bleedDamage" in player.status)
                ...
                const speakBtn = el("button", {
                    ...
                    onClick: () => {
                        openModal?.("Ink and Footprints", (body) => {
                            body.appendChild(el("p", {
                                className: "modal-subtitle",
                                text: "The Bark‑Scribe sits with his back to the hearth, so the light falls on the table and not his face. He writes on bark as if paper were a luxury the world no longer deserves."
                            }));
                            body.appendChild(el("p", {
                                text: "“Rowan doesn’t ask for courage,” he says without looking up. “He asks for attention. Goblins learned the shape of our roads, and now they think the roads belong to them.”"
                            }));
                            body.appendChild(el("p", {
                                text: "He turns a strip of bark toward you. On it: crude stamps — circles broken by short notches, like teeth marks. “Trail‑marks,” he murmurs. “Not just navigation. A promise. If you see a mark with three notches… don’t follow it straight. That’s bait.”"
                            }));
                            body.appendChild(el("p", {
                                text: "The scribe finally meets your eyes. “If you want to push deep enough to find their leader, don’t go raw. Bitterleaf on the wrist. It dulls the forest’s itch — and the goblins’ dogs won’t like the scent.”"
                            }));
                            ...
                            actions.appendChild(el("button", {
                                ...
                                onClick: () => {
                                    ...
                                    if (state.quests?.main)
                                        state.quests.main.step = Math.max(Number(state.quests.main.step) || 0, 0.75);
                ...
                const speakBtn = el("button", {
                    ...
                    onClick: () => {
                        openModal?.("The Bark‑Scribe", (body) => {
                            body.appendChild(el("p", {
                                className: "modal-subtitle",
                                text: "A man sits where the firelight can’t quite reach. His fingers are stained with ink and sap. He does not look up when you approach—only listens."
                            }));
                            body.appendChild(el("p", {
                                text: "“Rowan sends everyone eventually,” he says. “You cut down a king and think you ended a story. But stories are forests. You only cleared one tree.”"
                            }));
                            body.appendChild(el("p", {
                                text: "He slides a thin book across the table. Its pages are bark—real bark—pressed flat. The writing isn’t ink. It’s scorched."
                            }));
                            body.appendChild(el("p", {
                                text: "“The Blackbark Oath wasn’t a promise to the village,” he continues. “It was a promise to the thing beneath it. The Wardens fed it. They starved it. They kept it asleep.”"
                            }));
                            body.appendChild(el("p", {
                                text: "“Now it’s awake enough to notice you.” He taps three symbols—sap, reed, bone. “Bring me three splinters of what the oath was built from. The forest will not let you take them gently, but it will let you take them.”"
                            }));
                            const actions = el("div", { className: "modal-actions" });
                            actions.appendChild(el("button", {
                                className: "btn small",
                                ...
                                onClick: () => {
                                    ...
                                    if (state.quests?.main)
                                        state.quests.main.step = Math.max(state.quests.main.step || 0, 10);
                                    addLog?.("The Bark‑Scribe marks three symbols: Sap‑Run, Witch‑Reed, Bone‑Char.", "system");
                                    // Offer an immediate side quest hook
                                    if (typeof startSideQuest === "function")
                                        startSideQuest("barkThatBleeds");
                                    updateQuestBox?.();
                                    saveGame?.();
                                    closeModal?.();
                                    // Optional: set a scene beat, if provided
                                    setScene?.("Bark‑Scribe’s Warning", [
                ...
                const speakBtn = el("button", {
                    ...
                    onClick: () => {
                        openModal?.("Quiet Ink", (body) => {
                            body.appendChild(el("p", {
                                className: "modal-subtitle",
                                text: "The Bark‑Scribe lays three splinters on bark‑paper. They don’t touch. They lean away from each other like magnets."
                            }));
                            body.appendChild(el("p", {
                                text: "“Good,” he says. “They still remember being separate. That means the oath is still alive enough to argue.”"
                            }));
                            body.appendChild(el("p", {
                                text: "He crushes a bead of tar‑black resin between thumb and nail. It drinks the hearth‑light. “Quiet Ink. It doesn’t write what you say. It writes what you meant.”"
                            }));
                            body.appendChild(el("p", {
                                text: "“We can bind the splinters so they stop pulling apart,” he continues. “But the ink is guarded. The Oathgrove is where the old vows were fed. Something there still thinks it owns the right to keep score.”"
                            }));
                            const actions = el("div", { className: "modal-actions" });
                            actions.appendChild(el("button", {
                                className: "btn small",
                                ...
                                onClick: () => {
                                    ...
                                    if (state.quests?.main)
                                        state.quests.main.step = Math.max(Number(state.quests.main.step) || 0, 10.75);
                ...
                const speakBtn = el("button", {
                    ...
                    onClick: () => {
                        openModal?.("Ink Against a Memory", (body) => {
                            ...
                            body.appendChild(el("p", {
                                text: "“This is not a relic,” he murmurs. “It’s a verdict. Someone tried to crown the wound—and the forest remembered the weight.”"
                            }));
                            body.appendChild(el("p", {
                                text: "He sketches a triangle of symbols: iron, grave, name. “If you want to speak to what’s coming, you need three anchors. A Star‑Iron Pin to hold the words. A Grave‑Latch to hold the door. And a living voice willing to be blamed.”"
                            }));
                            const actions = el("div", { className: "modal-actions" });
                            actions.appendChild(el("button", {
                                className: "btn small",
                                ...
                                onClick: () => {
                                    ...
                                    if (state.quests?.main)
                                        state.quests.main.step = Math.max(Number(state.quests.main.step) || 0, 17.5);
            ...
            if (gestureQ && gestureQ.status === "active" && gestureQ.status !== "completed") {
                ...
                addCard({
                    ...
                    desc: "Three small choices, not heroic ones: mercy, restraint, protection. The forest cares about the small choices.",
                    actions: [
                        el("button", {
                            className: "btn small outline",
            ...
            if (availableCount) {
                ...
                const renderEmpty = () => {
                    ...
                    listPanel.appendChild(el("div", { className: "mini-empty", text: "No new notices right now." }));
                    detailPanel.appendChild(el("div", {
