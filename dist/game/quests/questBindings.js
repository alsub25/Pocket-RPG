...
import { ensureQuestStructures, initMainQuest, openElderRowanDialog, openQuestJournal, startSideQuest, advanceSideQuest, completeSideQuest, updateQuestBox, maybeTriggerSideQuestEvent, handleExploreQuestBeats, applyQuestProgressOnItemGain, applyQuestProgressOnEnemyDefeat, hasAllOathSplinters, buildProgressionAuditReport } from './questSystem.js';
...
export function createQuestBindings(ctx) {
    ...
    return {
        ...
        advanceSideQuest: (id, nextStep) => advanceSideQuest(getState(), id, nextStep, api),
        completeSideQuest: (id, rewardsFn) => completeSideQuest(getState(), id, rewardsFn, api),
        // UI
        updateQuestBox: () => updateQuestBox(getState()),
        openQuestJournal: () => openQuestJournal(getState(), api),
        // Event runners
        maybeTriggerSideQuestEvent: (areaId) => maybeTriggerSideQuestEvent(getState(), areaId, api),
        handleExploreQuestBeats: (areaId) => handleExploreQuestBeats(getState(), areaId, api),
        applyQuestProgressOnItemGain: (itemId, qty = 1) => applyQuestProgressOnItemGain(getState(), itemId, qty, api),
        applyQuestProgressOnEnemyDefeat: (enemy) => applyQuestProgressOnEnemyDefeat(getState(), enemy, api),
        // Utilities
        hasAllOathSplinters: () => hasAllOathSplinters(getState()),
        buildProgressionAuditReport: () => buildProgressionAuditReport(getState(), {
