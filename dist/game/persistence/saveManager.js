...
function _validateEngineSnapshot(snapshot) {
    try {
        ...
        if (!snapshot.checksum || !snapshot.checksumAlg)
        ...
        if (snapshot.checksum !== expected)
            return { ok: false, reason: 'checksum_mismatch', expected };
        return { ok: true };
    }
    catch (e) {
...
export function createSaveManager(deps) {
    if (!deps || typeof deps !== 'object')
        throw new Error('createSaveManager: missing deps');
    const { 
    // Keys/schema/version
    SAVE_KEY, SAVE_KEY_PREFIX, SAVE_INDEX_KEY, SAVE_SCHEMA, GAME_PATCH, 
    // Engine core (optional): engine.save/load + scheduler
    engine, 
    // State access
    getState, setState, 
    // Engine runtime hooks (behavior preserved)
    createEmptyState, syncGlobalStateRef, initRngState, 
    // Systems / repair hooks used during load
    quests, initTimeState, initVillageEconomyState, initGovernmentState, ensureVillagePopulation, cleanupTownHallEffects, 
    // Combat restore helpers
    ensureEnemyRuntime, ensureCombatTurnState, ensureCombatPointers, getTimeInfo, 
    // Post-load recalcs
    recalcPlayerStats, rescaleActiveCompanion, 
    // UI hooks
    updateHUD, updateEnemyPanel, renderLog, setScene, addLog, renderActions, switchScreen, updateTimeDisplay, alertFn, showCorruptSaveModal, 
    // Integrity / diagnostics
    ensureCombatPointersBeforeSave, runIntegrityAudit, recordCrash, setLastSaveError, 
    // Migration helpers owned by engine.js
    ensurePlayerSpellSystems, CLASS_LEVEL_UNLOCKS, MAX_EQUIPPED_SPELLS } = deps;
    // -------------------- MIGRATION --------------------
    function _normalizeAnyItemQuantity(it) {
        ...
        try {
            ...
            it.quantity = 1;
            try {
                if (Object.prototype.hasOwnProperty.call(it, 'qty'))
                    delete it.qty;
            }
            catch (_) { }
        }
    ...
    function migrateSaveData(data) {
        ...
        while (data.meta.schema < SAVE_SCHEMA) {
            ...
            if (data.meta.schema === 2) {
                ...
                if (typeof data.inCombat !== 'boolean')
                    data.inCombat = false;
                if (data.inCombat) {
            ...
            if (data.meta.schema === 6) {
                ...
                if (data.player && typeof data.player === 'object') {
                    ...
                    if (typeof st.comboPoints !== 'number' || !Number.isFinite(st.comboPoints))
                        st.comboPoints = 0;
                    if (typeof st.soulShards !== 'number' || !Number.isFinite(st.soulShards))
                        st.soulShards = 0;
                    if (typeof st.lichTurns !== 'number' || !Number.isFinite(st.lichTurns))
                        st.lichTurns = 0;
                    if (typeof st.totemType !== 'string')
                        st.totemType = '';
                    if (typeof st.totemTurns !== 'number' || !Number.isFinite(st.totemTurns))
                        st.totemTurns = 0;
                    if (typeof st.vanishTurns !== 'number' || !Number.isFinite(st.vanishTurns))
                        st.vanishTurns = 0;
                }
                data.meta.schema = 7;
                continue;
        ...
        try {
            const inv = data.player && Array.isArray(data.player.inventory) ? data.player.inventory : null;
            if (inv)
                inv.forEach(_normalizeAnyItemQuantity);
                ...
                Object.keys(ms).forEach((k) => {
                    ...
                    if (Array.isArray(entry)) {
                        ...
                        if (Array.isArray(entry.items))
                            entry.items.forEach(_normalizeAnyItemQuantity);
                    }
                });
            }
        }
        ...
        try {
            ...
            const markDeadHandled = (e) => {
                ...
                if (finiteNumber(e.hp, 0) <= 0)
                    e._defeatHandled = true;
            };
            if (data.inCombat) {
                if (Array.isArray(data.enemies))
                    data.enemies.forEach(markDeadHandled);
    ...
    function _buildSaveBlob() {
        ...
        return {
            ...
            currentEnemy: state.inCombat && Array.isArray(state.enemies) && state.enemies.length
                ? (state.enemies[Number.isFinite(state.targetEnemyIndex)
                    ? Math.max(0, Math.floor(state.targetEnemyIndex))
                    : 0] || state.enemies[0] || null)
                : state.inCombat && state.currentEnemy
                    ? state.currentEnemy
                    : null
        };
            ...
            try {
                ...
                try {
                    if (typeof runIntegrityAudit === 'function') {
                        ...
                        if (state.debug)
                            state.debug.lastInvariantIssues = invIssues.length ? invIssues : null;
    ...
    function loadGame(fromDefeat) {
        try {
            ...
            if (!json) {
                if (!fromDefeat)
                    (alertFn || alert)('No save found on this device.');
                return false;
            }
            ...
            try {
                ...
                if (isSnapshot) {
                    ...
                    if (!v.ok) {
                        ...
                        try {
                            if (!fromDefeat)
                                showCorruptSaveModal && showCorruptSaveModal('Save data failed integrity validation and cannot be loaded.');
                        }
                        catch (_) { }
                        return false;
                    }
                    ...
                    if (!parsed.meta || typeof parsed.meta !== 'object')
                        parsed.meta = {};
                    if (!Number.isFinite(Number(parsed.meta.schema)))
                        parsed.meta.schema = SAVE_SCHEMA;
                    if (!parsed.meta.patch)
                        parsed.meta.patch = GAME_PATCH;
                }
            }
            catch (_) { }
            ...
            if (data && data.__corrupt) {
                if (!fromDefeat)
                    showCorruptSaveModal && showCorruptSaveModal('Save data failed validation and cannot be loaded.');
                return false;
            }
            if (!data || !data.player) {
                if (!fromDefeat)
                    showCorruptSaveModal && showCorruptSaveModal('Save is missing essential player data.');
                return false;
            }
            ...
            if (state.inCombat) {
                if (state.enemies && state.enemies.length) {
                    for (let i = 0; i < state.enemies.length; i++) {
                        ...
                        if (e && typeof e === 'object')
                            ensureEnemyRuntime && ensureEnemyRuntime(e);
                    }
                    let pick = null;
                ...
                try {
                    ensureCombatTurnState && ensureCombatTurnState();
                }
                catch (_) { }
                try {
                    ensureCombatPointers && ensureCombatPointers();
                }
                catch (_) { }
            }
            else {
            ...
            if (!fromDefeat)
                showCorruptSaveModal && showCorruptSaveModal('Save data is corrupt or incompatible.');
            return false;
        }
    }
    // -------------------- MANUAL SAVE SLOTS --------------------
