...
import { ECONOMY_TIERS, getVillageEconomySummary, getMerchantPrice, getRestCost } from '../locations/village/villageEconomy.js';
// Economic constants
...
export function createVillageEconomyService(engine) {
    if (!engine)
        throw new Error('VillageEconomyService requires engine instance');
    const rng = engine.get('rng');
    if (!rng)
        throw new Error('VillageEconomyService requires RNG service');
    /**
