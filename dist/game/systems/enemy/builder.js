...
function _getDiffId(diff) {
    ...
    if (id === 'dynamic')
        return String(diff.closestId || 'normal').toLowerCase();
    return id;
}
// --- Elemental scaling ------------------------------------------------------
// Notes:
...
function _pickWeighted(pairs, rand, tag) {
    ...
    for (let i = 0; i < pairs.length; i++)
        total += Math.max(0.0001, Number(pairs[i][1] || 0));
    ...
    if (enemy.affinities && enemy.affinities.resist && typeof enemy.affinities.resist === 'object') {
        ...
        for (const k in r) {
            ...
            if (!Number.isFinite(m))
                continue;
            if (m >= 1)
                continue; // not a resist multiplier
            const reduction = 1 - m;
...
function rollEnemyElementalTrait(enemy, ctx = {}) {
    ...
    if (String(areaId).toLowerCase() === 'village')
        return;
    ...
    if (rand('enemy.elemTrait.roll') >= chance)
        return;
    ...
    if (!Number.isFinite(existingRes) || existingRes >= 1)
        enemy.affinities.resist[chosen] = resistMult;
    else
        enemy.affinities.resist[chosen] = Math.min(existingRes, resistMult);
    // Apply weakness: keep the stronger weakness.
    const existingWeak = Number(enemy.affinities.weak[weakElem] || 1);
    if (!Number.isFinite(existingWeak) || existingWeak <= 1)
        enemy.affinities.weak[weakElem] = weakMult;
    else
        enemy.affinities.weak[weakElem] = Math.max(existingWeak, weakMult);
    ...
    if (!Array.isArray(enemy.elementalTraits))
        enemy.elementalTraits = [];
...
function assignEnemyOffenseElements(enemy, ctx = {}) {
    ...
    if (!primary) {
        ...
        if (/(frozen|frost|ice|chill|glacier|snow)/.test(corpus))
            primary = 'frost';
        else if (/(ember|flame|fire|burn|cinder|dragon)/.test(corpus))
            primary = 'fire';
        else if (/(lightning|storm|thunder|shock|spark)/.test(corpus))
            primary = 'lightning';
        else if (/(holy|radiant|sun|blessed)/.test(corpus))
            primary = 'holy';
        else if (/(void|shadow|wraith|nec|vampir|curse)/.test(corpus))
            primary = 'shadow';
        else if (/(arcane|mage|witch|sorcer|mystic)/.test(corpus))
            primary = 'arcane';
        else if (/(poison|toxic|venom|plague|spit)/.test(corpus))
            primary = 'poison';
        else if (/(thorn|vine|nature|mire|swamp|bog|marsh)/.test(corpus))
            primary = 'nature';
        else if (/(earth|stone|golem|rock|sand)/.test(corpus))
            primary = 'earth';
    }
    if (!primary)
        return;
    if (!hasAttack)
        enemy.attackElementType = primary;
    if (!hasMagic)
        enemy.magicElementType = primary;
}
