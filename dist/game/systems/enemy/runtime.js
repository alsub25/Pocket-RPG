...
function inferEnemyOffenseElement(enemy) {
    ...
    if (/(frozen|frost|ice|chill|glacier|snow)/.test(corpus))
        return 'frost';
    if (/(ember|flame|fire|burn|cinder|dragon)/.test(corpus))
        return 'fire';
    if (/(lightning|storm|thunder|shock|spark)/.test(corpus))
        return 'lightning';
    if (/(holy|radiant|sun|blessed)/.test(corpus))
        return 'holy';
    if (/(void|shadow|wraith|nec|vampir|curse)/.test(corpus))
        return 'shadow';
    if (/(arcane|mage|witch|sorcer|mystic)/.test(corpus))
        return 'arcane';
    if (/(poison|toxic|venom|plague|spit)/.test(corpus))
        return 'poison';
    if (/(thorn|vine|nature|mire|swamp|bog|marsh)/.test(corpus))
        return 'nature';
    if (/(earth|stone|golem|rock|sand)/.test(corpus))
        return 'earth';
    return null;
}
...
export function ensureEnemyRuntime(enemy, deps = {}) {
    ...
    if (!Number.isFinite(enemy.guardTurns))
        enemy.guardTurns = 0;
    if (!Number.isFinite(enemy.guardArmorBonus))
        enemy.guardArmorBonus = 0;
    if (!Number.isFinite(enemy.enrageTurns))
        enemy.enrageTurns = 0;
    if (!Number.isFinite(enemy.enrageAtkPct))
        enemy.enrageAtkPct = 0;
    if (!Number.isFinite(enemy.baseAttack))
        enemy.baseAttack = finiteNumber(enemy.attack, 0);
    if (!Number.isFinite(enemy.baseMagic))
        enemy.baseMagic = finiteNumber(enemy.magic, 0);
    if (!Number.isFinite(enemy.atkDownTurns))
        enemy.atkDownTurns = 0;
    if (!Number.isFinite(enemy.atkDownFlat))
        enemy.atkDownFlat = 0;
    if (!Number.isFinite(enemy.magDownTurns))
        enemy.magDownTurns = 0;
    if (!Number.isFinite(enemy.magDownFlat))
        enemy.magDownFlat = 0;
    if (!Number.isFinite(enemy.brokenTurns))
        enemy.brokenTurns = 0;
    if (!Number.isFinite(enemy.postureMax) || enemy.postureMax <= 0) {
        enemy.postureMax = computeEnemyPostureMax(enemy);
    }
    if (!Number.isFinite(enemy.posture))
        enemy.posture = 0;
    if (!('intent' in enemy))
        enemy.intent = null;
    ...
    if (hasAtkElem)
        enemy.attackElementType = String(enemy.attackElementType).trim().toLowerCase();
    if (hasMagElem)
        enemy.magicElementType = String(enemy.magicElementType).trim().toLowerCase();
    ...
    if (enemy.rarity) {
        ...
        if (!Number.isFinite(enemy.rarityDropMult))
            enemy.rarityDropMult = finiteNumber(_rdef.dropMult, 1);
        if (!Number.isFinite(enemy.rarityAffixCapBonus))
            enemy.rarityAffixCapBonus = finiteNumber(_rdef.affixCapBonus, 0);
    }
    if (typeof enemy._rarityApplied !== 'boolean')
        enemy._rarityApplied = !!enemy._rarityApplied;
    if (!Number.isFinite(enemy.affixThornsPct))
        enemy.affixThornsPct = 0;
    if (!Number.isFinite(enemy.affixVampiricHealPct))
        enemy.affixVampiricHealPct = 0;
    if (!Number.isFinite(enemy.affixRegenPct))
        enemy.affixRegenPct = 0;
    if (!Number.isFinite(enemy.affixChillChance))
        enemy.affixChillChance = 0;
    if (!Number.isFinite(enemy.affixChillTurns))
        enemy.affixChillTurns = 0;
    if (!Number.isFinite(enemy.affixHexTurns))
        enemy.affixHexTurns = 0;
    if (!Number.isFinite(enemy.affixHexAtkDown))
        enemy.affixHexAtkDown = 0;
    if (!Number.isFinite(enemy.affixHexArmorDown))
        enemy.affixHexArmorDown = 0;
    if (!Number.isFinite(enemy.affixHexResDown))
        enemy.affixHexResDown = 0;
    if (!Number.isFinite(enemy.affixBerserkThreshold))
        enemy.affixBerserkThreshold = 0;
    if (!Number.isFinite(enemy.affixBerserkAtkPct))
        enemy.affixBerserkAtkPct = 0;
    if (typeof enemy.affixBerserkActive !== 'boolean')
        enemy.affixBerserkActive = false;
