...
function _pickEnemyRarityWeighted(enemy, diffCfg) {
    ...
    if (diffId === 'hard') {
        ...
        if (lvl < 8)
            weights = weights.filter(([id]) => id !== 'epic');
        if (lvl < 10)
            weights = weights.filter(([id]) => id !== 'legendary');
        if (lvl < 20)
            weights = weights.filter(([id]) => id !== 'mythic');
    }
    else {
    ...
    if (!weights.length)
        return diffId === 'hard' ? 'rare' : 'uncommon';
...
export function applyEnemyRarity(enemy, ctx = {}) {
    ...
    if (typeof enemy._rarityRolled !== 'boolean')
        enemy._rarityRolled = false;
    ...
    if (shouldRoll) {
        ...
        if (diffId === 'easy') {
            ...
            if (diffId === 'hard') {
                ...
                else if (lvl >= 10 && r < 0.10)
                    enemy.rarity = 'legendary';
                // Epic is uncommon but present.
                else if (lvl >= 8 && r < 0.28)
                    enemy.rarity = 'epic';
                // Mostly Rare; a little Uncommon.
                else
                    enemy.rarity = r < 0.38 ? 'uncommon' : 'rare';
            }
            else {
                const r = rand('enemy.rarity.elite');
    ...
    if (enemy.rarity && !enemy._rarityRolled)
        enemy._rarityRolled = true;
