...
function randomNameForGender(gender) {
    if (gender === "male")
        return randChoice(NAME_FRAGMENTS.male);
    if (gender === "female")
        return randChoice(NAME_FRAGMENTS.female);
...
export function initGovernmentState(state, absoluteDay = 0) {
    ...
    if (!g.monarch)
        g.monarch = createInitialMonarch(absoluteDay);
...
function updateCouncil(government) {
    ...
    council.forEach(member => {
        ...
        switch (member.ideology) {
            case "populist":
                ...
                break;
            case "hawk":
                if (currentPolicies.militaryPosture === "war")
                    delta += 1.5;
                if (currentPolicies.militaryPosture === "peace")
                    delta -= 1;
                break;
        ...
        if (member.loyalty > 75)
            member.mood = "pleased";
        else if (member.loyalty < 30)
            member.mood = "resentful";
        else if (member.loyalty < 50)
            member.mood = "uneasy";
        else
            member.mood = "calm";
    });
    ...
    for (let i = 0; i < council.length; i++) {
        ...
        if (c.loyalty <= 10) {
            const role = c.role;
            council[i] = {
                ...createInitialCouncil(government.lastUpdatedDay).find(m => m.role === role),
...
function updateVillageAttitudes(government, state) {
    ...
    Object.values(villages).forEach(v => {
        ...
        const justiceStyle = government.currentPolicies && government.currentPolicies.justiceStyle;
        let fearDelta = 0;
        if (justiceStyle === "harsh")
            fearDelta += 1;
        else if (justiceStyle === "lenient")
            fearDelta -= 1;
        ...
        v.prosperityModifier = Math.max(-0.3, Math.min(0.3, (prosperity - 50) / 100));
        v.safetyModifier = Math.max(-0.3, Math.min(0.3, (stability - 50) / 100));
...
function maybeIssueRoyalDecree(government, absoluteDay, hooks) {
    ...
    if (rngFloat(null, 'government.randomDecree') > baseChance)
        return;
    ...
    if (addLog) {
        addLog(`${government.monarch.title} ${government.monarch.name} issues a decree: ${title}.`, "system");
    }
}
/**
...
export function getVillageGovernmentEffect(state, areaId) {
    ...
    if (!g || !g.villages || !g.villages[areaId]) {
        return {
            ...
            populationSize: null,
            populationMood: null,
            description: "The crown's influence on this region is uncertain."
        };
    }
...
function buildVillageInfluenceDescription(village, government, popSnapshot) {
    ...
    if (popSnapshot && typeof popSnapshot.size === "number") {
        parts.push(`Roughly ${popSnapshot.size.toLocaleString()} souls call this place home.`);
    }
    if (popSnapshot && typeof popSnapshot.mood === "number") {
        ...
        if (mood >= 50) {
            ...
            parts.push("Anger simmers just below the surface whenever the crown is mentioned.");
        }
        else if (mood <= -20) {
