...
export function createCombatMath(deps) {
    const { getState, 
    // numeric + rng
    clampNumber, rand, _roundIntStable, 
    // combat context
    getActiveDifficultyConfig, _getPlayerAbilityContext, 
    // element helpers
    normalizeElementType, getPlayerElementalBonusPct, getPlayerElementalResistPct, 
    // flags/talents + logging
    playerHasTalent, addLog, } = deps;
    ...
    function getEnemyAffinityMultiplier(enemy, elementType) {
        ...
        if (weakV !== null && weakV !== undefined)
            return _normalizeAffinityMult(weakV, 'weak');
        const resV = pick(a.resist);
        if (resV !== null && resV !== undefined)
            return _normalizeAffinityMult(resV, 'resist');
        return 1;
    }
    function getEnemyElementalResistPct(enemy, elementType) {
        ...
        // Try canonical first.
        if (Object.prototype.hasOwnProperty.call(map, et))
            return getV(et);
        ...
        if (Object.prototype.hasOwnProperty.call(map, raw))
            return getV(raw);
        const low = raw.toLowerCase();
        if (Object.prototype.hasOwnProperty.call(map, low))
            return getV(low);
        const etLow = et.toLowerCase();
        if (Object.prototype.hasOwnProperty.call(map, etLow))
            return getV(etLow);
    ...
    function calcPhysicalDamage(baseStat, elementType, enemyOverride) {
        ...
        const atkDown = p && p.status && p.status.atkDownTurns > 0 ? p.status.atkDown || 0 : 0;
        ...
        let effArmor = (((enemy && enemy.armor) || 0) + ((enemy && enemy.armorBuff) || 0)) *
            (1 - penPct / 100);
        ...
        const et = normalizeElementType(elementType || (p && p.stats ? p.stats.weaponElementType : null) || null);
    ...
    function calcMagicDamage(baseStat, elementType, enemyOverride) {
        ...
        const p = state.player;
        const ctx = _getPlayerAbilityContext();
        const magDown = p && p.status && p.status.magicDownTurns > 0
        ...
        const penPct = clampNumber(p && p.stats ? p.stats.armorPen || 0 : 0, 0, 80);
        let effRes = (((enemy && enemy.magicRes) || 0) +
        ...
        if (p && p.classId === 'paladin' && et === 'holy' && playerHasTalent(p, 'paladin_radiant_focus'))
            dmg *= 1.10;
        if (p && p.classId === 'cleric' && et === 'holy' && playerHasTalent(p, 'cleric_holy_focus'))
            dmg *= 1.10;
        if (p && p.classId === 'necromancer' && et === 'shadow' && playerHasTalent(p, 'necromancer_shadow_mastery'))
            dmg *= 1.10;
        if (p && p.classId === 'necromancer' && et === 'poison' && playerHasTalent(p, 'necromancer_plague_touch'))
            dmg *= 1.10;
        if (p && p.classId === 'shaman' && et === 'lightning' && playerHasTalent(p, 'shaman_tempest_focus'))
            dmg *= 1.10;
        if (p && p.classId === 'shaman' && et === 'nature' && playerHasTalent(p, 'shaman_nature_attunement'))
            dmg *= 1.10;
        if (p && p.classId === 'vampire' && et === 'shadow' && playerHasTalent(p, 'vampire_shadow_focus'))
            dmg *= 1.10;
    ...
    function calcEnemyDamage(baseStat, elementType) {
        ...
        const armorDown = p && p.status && p.status.armorDownTurns > 0 ? p.status.armorDown || 0 : 0;
        const resDown = p && p.status && p.status.magicResDownTurns > 0
        ...
        const opts = elementType && typeof elementType === 'object' && !Array.isArray(elementType)
            ? elementType
            : null;
        const damageTypeHint = opts && typeof opts.damageType === 'string'
            ...
            : opts && typeof opts.isMagic === 'boolean'
                ? opts.isMagic
                    ...
                    : 'physical'
                : null;
        const elemRaw = opts && typeof opts.elementType === 'string'
            ? opts.elementType
