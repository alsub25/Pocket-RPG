...
export function buildAbilityEffects(deps = {}) {
    const { getState, _dealPlayerMagic, _dealPlayerMagicAoe, _dealPlayerPhysical, _dealPlayerPhysicalAoe, _healPlayer, _addShield, _applyTimedBuff, playerHasTalent, applyEnemyAtkDown, finiteNumber, getAliveEnemies, grantCompanion, addLog, getPlayerHasteMultiplier, rand } = deps;
    ...
    const getGainMult = (p) => {
        ...
        if (typeof deps.getGainMult === 'function')
            return deps.getGainMult(p);
        if (typeof deps.getGainMultFor === 'function')
            return deps.getGainMultFor(p);
        if (typeof getPlayerHasteMultiplier === 'function')
            return getPlayerHasteMultiplier(p, 150);
    ...
    const ABILITY_EFFECTS = {
        ...
        tempest: (p, enemy, ctx) => {
            ...
            if (st.totemTurns > 0) {
                hit.hits.forEach(({ enemy: e }) => {
                    if (!e || e.isBoss || finiteNumber(e.hp, 0) <= 0)
                        return;
            ...
            if (stunned > 0 && enemy && enemy.name)
                addLog(enemy.name + ' is stunned by the Tempest!', 'good');
