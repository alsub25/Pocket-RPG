...
export function createDiagnosticsOverlayPlugin(opts = {}) {
    ...
    return {
        ...
        init(engine) {
            ...
            const qa = (() => {
                try {
                    return engine && typeof engine.getService === 'function' ? engine.getService('qa') : null;
                }
                catch (_) {
                    return null;
                }
            ...
            if (qa) {
                // QA suite wiring
                if (!merged.runSmokeTests)
                    merged.runSmokeTests = qa.runSmokeTests;
                if (!merged.buildBugReportBundle)
                    merged.buildBugReportBundle = qa.buildBugReportBundle;
                if (!merged.formatBugReportBundle)
                    merged.formatBugReportBundle = qa.formatBugReportBundle;
                if (!merged.runIntegrityAudit)
                    merged.runIntegrityAudit = qa.runIntegrityAudit;
                if (!merged.formatIntegrityAuditReport)
                    merged.formatIntegrityAuditReport = qa.formatIntegrityAuditReport;
                if (!merged.runScenarioRunner)
                    merged.runScenarioRunner = qa.runScenarioRunner;
                if (!merged.formatScenarioRunnerReport)
                    merged.formatScenarioRunnerReport = qa.formatScenarioRunnerReport;
                // Perf snapshot helpers
                if (!merged.qaReadBootMetrics)
                    merged.qaReadBootMetrics = qa.qaReadBootMetrics;
                if (!merged.qaReadMemorySnapshot)
                    merged.qaReadMemorySnapshot = qa.qaReadMemorySnapshot;
                if (!merged.qaSummarizePerfLog)
                    merged.qaSummarizePerfLog = qa.qaSummarizePerfLog;
                if (!merged.qaCollectPerfSnapshotSync)
                    merged.qaCollectPerfSnapshotSync = qa.qaCollectPerfSnapshotSync;
                if (!merged.qaFormatPerfSnapshotText)
                    merged.qaFormatPerfSnapshotText = qa.qaFormatPerfSnapshotText;
                if (!merged.qaSampleFps)
                    merged.qaSampleFps = qa.qaSampleFps;
            }
            _diag = createDiagnosticsUI({ ...merged, engine });
            // Expose service
            try {
                engine.registerService('diagnostics', _diag);
            }
            catch (_) { }
