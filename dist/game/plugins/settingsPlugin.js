        ...
        start(engine) {
            ...
            if (!hadUnified) {
                try {
                    ...
                    if (theme)
                        settings.set('ui.theme', String(theme), { persist: false });
                }
                catch (_) { }
                try {
                    const rm = safeStorageGet('pq-reduce-motion');
                    if (rm === '1')
                        settings.set('a11y.reduceMotion', true, { persist: false });
                    else if (rm === '0')
                        settings.set('a11y.reduceMotion', false, { persist: false });
                }
                catch (_) { }
                try {
                    ...
                    if (vol != null && vol !== '')
                        settings.set('audio.masterVolume', _num(vol, 100), { persist: false });
                }
                catch (_) { }
                try {
                    const ts = safeStorageGet('pq-text-speed');
                    if (ts != null && ts !== '')
                        settings.set('ui.textSpeed', _num(ts, 100), { persist: false });
                }
                catch (_) { }
                try {
                    const m = safeStorageGet('pq-music-enabled');
                    if (m === '1')
                        settings.set('audio.musicEnabled', true, { persist: false });
                    else if (m === '0')
                        settings.set('audio.musicEnabled', false, { persist: false });
                }
                catch (_) { }
                try {
                    const s = safeStorageGet('pq-sfx-enabled');
                    if (s === '1')
                        settings.set('audio.sfxEnabled', true, { persist: false });
                    else if (s === '0')
                        settings.set('audio.sfxEnabled', false, { persist: false });
                }
                catch (_) { }
                try {
                    const ae = safeStorageGet('pq-auto-equip-loot');
                    if (ae === '1')
                        settings.set('gameplay.autoEquipLoot', true, { persist: false });
                    else if (ae === '0')
                        settings.set('gameplay.autoEquipLoot', false, { persist: false });
                }
                catch (_) { }
            ...
            if (state) {
                ...
                try {
                    ...
                    if (rmPref === true)
                        state.settingsReduceMotion = true;
                    else if (rmPref === false)
                        state.settingsReduceMotion = false;
                ...
                engine.listen('system:settings', 'settings:changed', (evt) => {
                    ...
                    if (k === 'audio.masterVolume') {
                        ...
                        if (Number.isFinite(nv))
                            st.settingsVolume = Math.max(0, Math.min(100, nv));
                    }
                    if (k === 'ui.textSpeed') {
                        const nv = _num(v, st.settingsTextSpeed);
                        if (Number.isFinite(nv))
                            st.settingsTextSpeed = Math.max(0, Math.min(200, nv));
                    }
                    ...
                    if (k === 'gameplay.autoEquipLoot') {
                        if (typeof v === 'boolean')
                            st.settingsAutoEquipLoot = v;
                    }
                    if (k === 'gameplay.showCombatNumbers') {
                        if (typeof v === 'boolean')
                            st.settingsShowCombatNumbers = v;
                    }
                    if (k === 'gameplay.autoSave') {
                        if (typeof v === 'boolean')
                            st.settingsAutoSave = v;
                    }
                    if (k === 'a11y.reduceMotion') {
                        if (v === true)
                            st.settingsReduceMotion = true;
                        else if (v === false)
                            st.settingsReduceMotion = false;
        ...
        stop(engine) {
            try {
                if (engine && typeof engine.disposeOwner === 'function')
                    engine.disposeOwner('system:settings');
            }
            catch (_) { }
        },
        dispose(engine) {
