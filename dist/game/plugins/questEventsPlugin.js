    ...
    return {
        ...
        start(engine) {
            ...
            const onItem = (p) => {
                try {
                    ...
                    if (!itemId)
                        return;
                    if (quests.applyQuestProgressOnItemGain)
                        quests.applyQuestProgressOnItemGain(itemId, qty);
                }
                catch (_) { }
            };
            const onDefeat = (p) => {
                try {
                    ...
                    if (!enemy)
                        return;
                    if (quests.applyQuestProgressOnEnemyDefeat)
        ...
        stop(engine) {
            try {
                ...
                if (h && h.onItem)
                    engine.off('world:itemGained', h.onItem);
                if (h && h.onDefeat)
                    engine.off('world:enemyDefeated', h.onDefeat);
            }
            catch (_) { }
            try {
                delete engine.__ewQuestEventsHandlers;
            }
            catch (_) { }
        }
    };
