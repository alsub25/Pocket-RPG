...
export function createUiCommandsPlugin({ openPauseMenu = null, openCharacterSheet = null, openQuestJournal = null, toggleDiagnosticsOverlay = null, 
...
isEnemySheetOpen = null, closeEnemySheet = null, } = {}) {
    return {
        id: 'ew.uiCommands',
        ...
        start(engine) {
            ...
            engine.commands.use((ctx, next) => {
                ...
                if (type === 'UI_OPEN_QUEST_JOURNAL') {
                    try {
                        if (typeof openQuestJournal === 'function')
                            openQuestJournal();
                    }
                    catch (_) { }
                    return;
        ...
        stop(engine) {
            try {
                ...
                if (h && h.onEsc)
                    engine.input.offAction('UI_ESCAPE', h.onEsc);
                if (h && h.onChar)
                    engine.input.offAction('UI_OPEN_CHARACTER', h.onChar);
                if (h && h.onQuest)
                    engine.input.offAction('UI_OPEN_QUEST_JOURNAL', h.onQuest);
                if (h && h.onDiag)
                    engine.input.offAction('UI_TOGGLE_DIAGNOSTICS', h.onDiag);
            }
            catch (_) { }
            try {
                delete engine.__ewUiCommandInputHandlers;
            }
            catch (_) { }
        }
    };
