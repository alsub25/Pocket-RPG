...
export function createGameCommandsPlugin(deps = {}) {
    ...
    return {
        id: 'ew.gameCommands',
        start(engine) {
            if (!engine || !engine.commands || typeof engine.commands.use !== 'function')
                return;
            engine.commands.use((ctx, next) => {
                ...
                if (type === 'GAME_CHANGE_AREA') {
                    try {
                        fns.setExploreChoiceMadeFalse?.();
                    }
                    catch (_) { }
                    try {
                        fns.openExploreModal?.(payload);
                    }
                    catch (_) { }
                    return;
                }
                ...
                if (type === 'GAME_OPEN_INVENTORY') {
                    try {
                        fns.openInventory?.(payload?.inCombat);
                    }
                    catch (_) { }
                    return;
                }
                if (type === 'GAME_OPEN_SPELLS') {
                    try {
                        fns.openSpells?.(payload?.inCombat);
                    }
                    catch (_) { }
                    return;
                }
                ...
                if (type === 'INVENTORY_USE_POTION') {
                    try {
                        fns.inventoryUsePotion?.(payload);
                    }
                    catch (_) { }
                    return;
                }
...
//# sourceMappingURL=gameCommandsPlugin.js.map