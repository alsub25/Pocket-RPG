...
function getCurrentRates(state) {
  ...
  const prosperity =
    typeof summary.prosperity === "number" ? summary.prosperity : 50;
  const trade =
    typeof summary.trade === "number" ? summary.trade : 50;
  const security =
    typeof summary.security === "number" ? summary.security : 50;
  ...
  const clampRate = r => {
    ...
    if (r < 0) return 0;
    if (r > 0.5) return 0.5; // cap at 50% per week just in case
    return r;
  };
...
export function openBankModalImpl({
  state,
  openModal,
  addLog,
  recordInput,
  ...
  openModal("Emberwood Bank", body => {
    ...
    rateLine.textContent =
      `Weekly rates — Savings: ${formatPercent(
        rates.depositRate
      )}, ` +
    ...
    if (weeksApplied > 0) {
      ...
      const nothingEarned =
        savings <= 0 && investments <= 0 && loans <= 0;
    ...
    btnDeposit.addEventListener("click", () => {
      ...
      const max = p.gold;
      const raw = prompt(
        `How much gold would you like to deposit? (1–${max})`
      );
      if (!raw) return;
    ...
    btnWithdraw.addEventListener("click", () => {
      ...
      const raw = prompt(
        `How much gold would you like to withdraw? (1–${max})`
      );
      if (!raw) return;
      const amt = Math.floor(Number(raw));
      if (!Number.isFinite(amt) || amt <= 0) return;
    ...
    investTag.textContent = `${formatPercent(
    ...
    btnInvest.addEventListener("click", () => {
      ...
      const max = p.gold;
      const raw = prompt(
        `How much gold would you like to invest? (1–${max})`
      );
      if (!raw) return;
      const amt = Math.floor(Number(raw));
      if (!Number.isFinite(amt) || amt <= 0) return;
    ...
    btnBorrow.addEventListener("click", () => {
      ...
      const raw = prompt(
        "How much gold would you like to borrow? (max 500g)"
      );
      if (!raw) return;
      const amt = Math.floor(Number(raw));
      if (!Number.isFinite(amt) || amt <= 0) return;
      if (amt > 500) {
        addLog(
          "The bank refuses to lend you that much in one go.",
          "system"
        );
        return;
      }
      ...
      if (currentLoan + amt > 1000) {
        addLog(
          'The banker frowns. "Settle some of your existing debt before borrowing more."',
          "danger"
        );
        return;
      }
    ...
    btnRepay.addEventListener("click", () => {
      ...
      if (p.gold <= 0) {
        addLog(
          "You have no gold available to repay your loan.",
          "system"
        );
        return;
      }
      ...
      const raw = prompt(
        `How much gold would you like to repay? (1–${max})`
      );
      if (!raw) return;
      const amt = Math.floor(Number(raw));
      if (!Number.isFinite(amt) || amt <= 0) return;
      if (amt > max) {
        addLog(
          "You cannot repay more than you currently owe and carry.",
          "system"
        );
        return;
      }
