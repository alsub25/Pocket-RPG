...
} from './ui/uiRuntime.js'
import { initUIBindings } from './ui/uiBindings.js'
import { createDiagnosticsUI } from './devtools/diagnosticsUI.js'
...
import { _STORAGE_DIAG_KEY_LAST_CRASH, _STORAGE_DIAG_KEY_LAST_SAVE_FAIL, safeStorageGet, safeStorageSet, safeStorageRemove } from './storageRuntime.js'
import { _perfNow, _ensurePerfDebug, perfRecord, perfWrap, perfWrapAsync } from './perf.js'
...
/* =============================================================================
 * COMPANIONS
 * Party members that can join the hero and contribute passive/active effects.
 * ============================================================================= */
...
/* =============================================================================
 * COMPANION ABILITIES
 * Companion skill kits and how they apply during combat/exploration.
 * ============================================================================= */
 ...
 * Defines enemy special moves and the decision logic for when enemies use them.
 * ============================================================================= */
//
...
// --- Modular combat engines ---
// These are injected with engine helpers and exposed via legacy wrappers below.
const CombatMath = createCombatMath({
    getState: () => state,
    clampNumber,
    // Dynamic RNG wrapper (tests temporarily override `rand`)
    rand: (...args) => rand(...args),
    _roundIntStable,
...
const quests = createQuestBindings({
    ...
    addItemToInventory,
    // Late-bound save hook (assigned after saveManager is created)
    saveGame: () => {
        try {
...
function addItemToInventory(itemId, quantity) {
    ...
    // Quest objective hook (collect-type objectives)
    try {
    ...
    openModal('Cheat Menu', (body) => {
        ...
        function setStoryPosition(step, opts = {}) {
            ...
            if (chkEnv.input.checked) {
                if (opts.area) state.area = String(opts.area)
...
const { runPostPlayerTurnSequence } = createPostTurnSequencer({
    getState: () => state,
    ensureCombatTurnState,
    combatPause,
    _combatDelayMs,
...
function companionActIfPresent() {
    ...
    if (!chosenAbility || chosenScore < damageFallbackScore - 2) {
        ...
        enemy.hp -= dmg
        addLog(
            comp.name + ' strikes ' + enemy.name + ' for ' + dmg + ' damage.',
            'good',
            { domain: 'combat', kind: 'damage', actor: 'companion', breakdown: null }
        )
...
function initSettingsFromState() {
    ...
    if (!volumeSlider && !textSpeedSlider && !settingsDifficulty && !autoEquipLootEl) return
...
// =============================================================================
// UI BOOTSTRAP (extracted)
// =============================================================================

// Configure UI module with live hooks (state/audio/combat accessors).
try {
    configureUI({
        getState: () => state,
...
// Diagnostics / QA UI (Smoke Tests modal, pill visibility) lives in its own module.
try {
    _diagnosticsUI = createDiagnosticsUI({
...
// Wire UI bindings once DOM is ready.
initUIBindings({
    patchLabel: `Emberwood Patch V${GAME_PATCH} — ${GAME_PATCH_NAME}`,
    getState: () => state,
    // boot/runtime
    initCrashCatcher,
