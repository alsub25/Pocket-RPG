...
export function createScreenAssetPreloadPlugin({
  ...
  labelForGroup = (groupId) => {
    ...
    if (g.startsWith('screen:')) return 'Loading ' + g.slice('screen:'.length) + '…'
    if (g.startsWith('modal:')) return 'Loading ' + g.slice('modal:'.length) + '…'
    if (g.startsWith('area:')) return 'Loading ' + g.slice('area:'.length) + '…'
    if (g) return 'Loading…'
    return 'Working…'
  }
} = {}) {
  return {
    ...
    start(engine) {
      ...
      const onProgress = (evt) => {
        try {
          ...
          if (active.total > 0 && active.done >= active.total) {
            ...
            try { schedule?.cancelOwner?.(owner) } catch (_) {}
            try {
      ...
      try { engine.listen('system:assetPreload', 'assets:progress', onProgress) } catch (_) {}
      ...
      const prewarmAll = async () => {
        try {
          ...
          if (!all.length) { prewarmInProgress = false; return }
      ...
      try {
        ...
        try { Promise.resolve().then(() => prewarmAll()) } catch (_) { prewarmAll() }
      }

    },

    stop(engine) {
      try { engine?.disposeOwner?.('system:assetPreload') } catch (_) {}
    },
