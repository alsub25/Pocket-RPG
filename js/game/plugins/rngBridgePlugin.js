    ...
    return {
        ...
        init(engine) {
            ...
            engine.registerService('gameRng', {
                ...
                float(tag) {
                    try { return rngFloat(getS(), tag) } catch (_) { return 0 }
                },
                int(min, max, tag) {
                    try { return rngInt(getS(), min, max, tag) } catch (_) { return min }
                },
                pick(arr, tag) {
                    try { return rngPick(getS(), arr, tag) } catch (_) { return Array.isArray(arr) ? arr[0] : null }
                }
            })
        }
    }
