    ...
    return {
        ...
        stop(engine) {
            try {
                ...
                if (h && h.onScreenEnter) engine.off('screen:enter', h.onScreenEnter)
                if (h && h.onScreenLeave) engine.off('screen:leave', h.onScreenLeave)
                if (h && h.onModalOpen) engine.off('modal:open', h.onModalOpen)
                if (h && h.onModalClose) engine.off('modal:close', h.onModalClose)
            } catch (_) {}
            try { delete engine.__ewInputContextsHandlers } catch (_) {}
