...
export function createAutosavePlugin({ saveGame } = {}) {
    return {
        ...
        init(engine) {
            engine.registerService('autosave', {
                request(reason = 'unknown') {
                    try { engine.emit('autosave:request', { reason: String(reason || '') }) } catch (_) {}
                    const savePolicy = engine.getService ? engine.getService('savePolicy') : null
                    if (savePolicy && typeof savePolicy.markDirty === 'function') {
                        try { savePolicy.markDirty('autosave:' + String(reason || 'unknown')) } catch (_) {}
                        try { savePolicy.flush({ reason: 'autosave' }) } catch (_) {}
                        return
                    }
        ...
        start(engine) {
            ...
            const req = (reason) => { try { svc.request(reason) } catch (_) {} }
        ...
        stop(engine) {
            try {
                ...
                if (h && h.onLoot) engine.off('world:itemGained', h.onLoot)
                if (h && h.onDefeat) engine.off('world:enemyDefeated', h.onDefeat)
                if (h && h.onBattleEnd) engine.off('world:battleEnded', h.onBattleEnd)
            } catch (_) {}
            try { delete engine.__ewAutosaveHandlers } catch (_) {}
