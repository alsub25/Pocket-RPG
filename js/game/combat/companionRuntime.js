...
export function createCompanionRuntime(deps) {
    ...
    function rescaleActiveCompanion(opts) {
        ...
        if (delta !== 0) {
            ...
            if (delta > 0) {
                ...
                if (_num(p.hp, 0) > p.maxHp) p.hp = p.maxHp
            }
        }
    }
    ...
    function dismissCompanion(silent) {
        const st = getState()
        if (!st || !st.companion) return
        if (!silent) addLog(st.companion.name + ' leaves your side.', 'system')
    ...
    function grantCompanion(id) {
        ...
        if (st.player && inst.appliedHpBonus) {
            ...
            st.player.hp = Math.min(
                st.player.maxHp,
                _num(st.player.hp, st.player.maxHp) + inst.appliedHpBonus
            )
            updateHUD()
        }
    ...
    function useCompanionAbility(comp, abilityId) {
        ...
        if (ab.type === 'damage') {
            const baseStat = Math.round(
                (_num(comp.attack, 0) + _num(p.stats && p.stats.attack, 0) * 0.15) * _num(ab.potency, 1)
            )
            let dmg = Math.max(
                1,
            ...
            if (ab.critSpike && rand('ability.critSpike') < ab.critSpike) {
                ...
                dmg += spike
                addLog(
                    comp.name + ' lands a devastating strike for ' + spike + ' bonus damage!',
                    'good'
                )
            }
            enemy.hp -= dmg
            if (ab.stunTurns) enemy.stunTurns = (_num(enemy.stunTurns, 0) + ab.stunTurns)
        ...
        if (ab.type === 'heal') {
            ...
            const healAmount = Math.max(
                5,
                Math.round(_num(p.maxHp, 0) * _num(ab.potency, 0.25) + missing * 0.12)
            )
    ...
    function companionActIfPresent() {
        ...
        const enemy = st && st.currentEnemy
        const p = st && st.player
        if (!comp || !enemy || !st || !st.inCombat || !p) return
        ...
        function scoreAbility(abilityId) {
            const ab = companionAbilities[abilityId]
            if (!ab) return -9999
            if (ab.type === 'ward' && comp.wardActive) return -9999
            ...
            if (ab.type === 'heal') {
                if (playerHpPct >= 0.95) return -9999
                score += (1 - playerHpPct) * 80
                if (enemyHpPct < 0.25) score -= 15
            }

            if (ab.type === 'ward') {
                if (enemyHpPct > 0.65 && !comp.wardActive) score += 45
                if (!comp.wardActive) score += 20
            }
            ...
            if (ab.type === 'shield+damage' || ab.type === 'ward') {
                ...
                if (!_num(p.status && p.status.shield, 0) || _num(p.status && p.status.shield, 0) < 6) score += 6
            }

            if (ab.type === 'damage' || ab.type === 'damage+debuff') {
                score += enemyHpPct * 40
                const expected = _num(comp.attack, 6) * potency * 0.9
                if (expected >= _num(enemy.hp, 0) - 6) score += 60
            }

            if (ab.type === 'damage+debuff') {
                if (!_num(enemy.debuffTurns, 0) || _num(enemy.debuffTurns, 0) <= 0) score += 18
                if (_num(p.status && p.status.buffFromCompanionTurns, 0) > 0) score += 10
            }
            ...
            if (ab.critSpike && (_num(p.stats && p.stats.critChance, 0) > 12 || enemyHpPct > 0.6)) score += 12
        ...
        let reward = 0
        if (dmgDone > 0) reward += (dmgDone / Math.max(1, _num(enemy.maxHp, 1))) * 1.2
        if (healDone > 0) reward += (healDone / Math.max(1, _num(p.maxHp, 1))) * 1.5
        if (postEnemyHp <= 0) reward += 2.0
