...
export function createStatusEngine(deps) {
  ...
  function applyStatusSynergyOnPlayerHit(enemy, damageDealt, elementType, damageType) {
      ...
      if (et === 'fire' && (enemy.bleedTurns || 0) > 0) {
          ...
          if (before <= 0) addLog(enemy.name + ' ignites from the bleeding wound!', 'good')
      }
      ...
      if (dt === 'physical' && (enemy.chilledTurns || 0) > 0) {
          ...
          addLog(
              'Shatter! ' + enemy.name + ' takes ' + burst + ' bonus damage.',
              'good',
              { domain: 'combat', kind: 'proc', actor: 'player', proc: 'shatter', amount: burst }
          )
      }
  }
  ...
  function applyStartOfTurnEffectsPlayer(p) {
    const state = getState()
      ...
      if (
          ...
          addLog(
              'You bleed for ' + p.status.bleedDamage + ' damage.',
              'danger',
              { domain: 'combat', kind: 'status', actor: 'enemy', effect: 'bleed' }
          )
