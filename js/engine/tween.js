...
function _now(clock) {
  try { return Number(clock && clock.nowMs) || 0 } catch (_) { return 0 }
}

function _isEl(t) {
  try { return !!t && typeof t === 'object' && typeof t.style === 'object' } catch (_) { return false }
}
...
export function createTweenService({ clock = null, emit = null, logger = null } = {}) {
  ...
  function _log(lvl, msg, data) {
    try { if (logger && logger[lvl]) logger[lvl]('tween', msg, data) } catch (_) {}
  }
  ...
  function _unregisterOwner(owner, id) {
    ...
    if (!set) return
    set.delete(id)
    if (set.size === 0) _byOwner.delete(o)
  }
  ...
  function cancel(id) {
    ...
    try { if (t.onCancel) t.onCancel() } catch (_) {}
    try { if (emit) emit('tween:cancel', { id }) } catch (_) {}
    return true
  }
  ...
  function cancelOwner(owner) {
    ...
    if (!o) return 0
    const set = _byOwner.get(o)
    if (!set || set.size === 0) { _byOwner.delete(o); return 0 }
  ...
  function to(target, props, {
    ...
    if (!target || !props || typeof props !== 'object') return null
    const id = _nextId++
    ...
    if (styleProps) delete flatProps.style
    ...
    if (tween.owner) _registerOwner(tween.owner, id)
    try { if (emit) emit('tween:start', { id, owner: tween.owner, ms: dur }) } catch (_) {}
    return id
  }
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      try {
        if (k.startsWith('style.')) {
          const sk = k.slice(6)
          if (t && t.style) t.style[sk] = String(v)
        } else {
          t[k] = v
        }
      } catch (_) {}
    }
    try { if (typeof tween.onUpdate === 'function') tween.onUpdate(alpha) } catch (_) {}
  }

  function update() {
    // Called by engine on each clock tick.
    if (_tweens.size === 0) return
    ...
    for (let i = 0; i < ids.length; i++) {
      ...
      if (t >= 1) {
        ...
        try { if (typeof tw.onComplete === 'function') tw.onComplete() } catch (_) {}
        try { if (emit) emit('tween:complete', { id }) } catch (_) {}
      }
    }
  }
