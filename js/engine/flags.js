...
function _safeRemove(key) {
  if (!_hasStorage()) return false
  try { localStorage.removeItem(String(key || '')) ; return true } catch (_) { return false }
}
...
export function createFlagsService({
  storageKey = 'locus.flags.v1',
  logger = null
} = {}) {
  const _log = logger
  ...
  function load() {
    ...
    try {
      ...
      _flags = { ..._defaults }
      try { _log?.warn?.('flags', 'failed to parse flags storage; resetting', { e }) } catch (_) {}
    }
    return dump()
  }
  ...
  function save() {
    try {
      ...
      try { _log?.warn?.('flags', 'failed to persist flags', { e }) } catch (_) {}
    }
  }
  ...
  function get(key, fallback = null) {
    ...
    if (Object.prototype.hasOwnProperty.call(_flags, k)) return _flags[k]
    if (Object.prototype.hasOwnProperty.call(_defaults, k)) return _defaults[k]
    return fallback
  }
