...
function _safeSet(key, val) {
  ...
  try { localStorage.setItem(String(key || ''), String(val)); return true } catch (_) { return false }
}

export function createTelemetry(engine, {
  maxEvents = 220,
  storageKeyLastCrash = 'locus.telemetry.lastCrash.v1'
} = {}) {
  let _enabled = false
  const _buf = []
  const _unsubs = []
  ...
  function record(type, payload = null) {
    ...
    _buf.push(rec)
    if (_buf.length > maxEvents) _buf.splice(0, _buf.length - maxEvents)
    return rec
  }
  ...
  function _flags() {
    try { return engine?.getService?.('flags') || engine?.flags || null } catch (_) { return null }
  }
  ...
  function _isEnabledFlag(key, fallback = false) {
    try {
      ...
      if (typeof f.isEnabled === 'function') return !!f.isEnabled(key)
      if (typeof f.get === 'function') return !!f.get(key, fallback)
      return fallback
    } catch (_) {
      return fallback
    }
  ...
  function _persistCrash(bundle) {
    if (!_isEnabledFlag('telemetry.persistCrashes', true)) return
    try { _safeSet(storageKeyLastCrash, JSON.stringify(bundle)) } catch (_) {}
  }
  ...
  function buildBundle({ includeState = null } = {}) {
    ...
    const ctx = (() => {
      try { return _contextProvider ? _contextProvider() : null } catch (_) { return null }
    })()

    const flagsDump = (() => {
      try { return _flags()?.dump?.() || null } catch (_) { return null }
    })()

    const savePolicyStatus = (() => {
      try { return engine?.getService?.('savePolicy')?.getStatus?.() || null } catch (_) { return null }
    })()

    const replayMeta = (() => {
      try { return engine?.getService?.('replay')?.getLastMeta?.() || null } catch (_) { return null }
    })()

    const report = (() => {
      try { return engine?.errorBoundary?.buildReport?.({ includeState: include }) || null } catch (_) { return null }
    })()
  ...
  function start() {
    ...
    const listen = (eventName, mapper) => {
      ...
      try { engine?.on?.(eventName, fn) } catch (_) {}
      _unsubs.push(() => { try { engine?.off?.(eventName, fn) } catch (_) {} })
    }
    ...
    try { engine?.on?.('engine:error', onErr) } catch (_) {}
    _unsubs.push(() => { try { engine?.off?.('engine:error', onErr) } catch (_) {} })
  }

  function stop() {
    _enabled = false
    while (_unsubs.length) {
      try { _unsubs.pop()() } catch (_) {}
    }
  }
