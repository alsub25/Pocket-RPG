...
export function createA11yService({ settings = null, emit = null, logger = null } = {}) {
  ...
  function _log(lvl, msg, data) {
    try { if (logger && logger[lvl]) logger[lvl]('a11y', msg, data) } catch (_) {}
  }
  ...
  function refreshEnv() {
    _env = _readEnv()
    try { if (emit) emit('a11y:envChanged', getEnv()) } catch (_) {}
    return _env
  }
  ...
  function compute() {
    ...
    if (schemePref === 'dark') colorScheme = 'dark'
    else if (schemePref === 'light') colorScheme = 'light'
    else colorScheme = _env.prefersDarkScheme ? 'dark' : 'light'

    const derived = { reduceMotion, highContrast, colorScheme, textScale, uiScale }

    // Emit if meaningfully changed.
    const changed = (() => {
      try { return JSON.stringify(_lastDerived) !== JSON.stringify(derived) } catch (_) { return true }
    })()
    if (changed) {
      _lastDerived = derived
      try { if (emit) emit('a11y:changed', { ...derived }) } catch (_) {}
    }
    return derived
  }
  ...
  function startEnvListeners({ owner = 'system:a11y' } = {}) {
    ...
    return () => { subs.forEach(fn => { try { fn() } catch (_) {} }) }
  }
