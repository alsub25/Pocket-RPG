...
export function createSavePolicy(engine, {
  owner = 'system:savePolicy',
  coalesceMs = 900,
  retryMs = 1500
} = {}) {
  let _writer = null
  ...
  function beginTxn(label = 'txn') {
    ...
    try { engine?.emit?.('savePolicy:txnBegin', { ...token, depth: _txnDepth }) } catch (_) {}
    return token
  }

  function endTxn(token) {
    _txnDepth = Math.max(0, _txnDepth - 1)
    try { engine?.emit?.('savePolicy:txnEnd', { token, depth: _txnDepth }) } catch (_) {}
    // If we were waiting for txn completion, try flushing now.
    try { flush({ reason: 'txnEnd' }) } catch (_) {}
  }
  ...
  function markDirty(reason = 'unknown') {
    ...
    if (r) {
      ...
      if (_dirtyReasons.length > 20) _dirtyReasons.splice(0, _dirtyReasons.length - 20)
    }
    try { engine?.emit?.('savePolicy:dirty', { t: _nowIso(), reason: r }) } catch (_) {}
  }

  function _isSafe() {
    try { return _safePredicate ? !!_safePredicate() : true } catch (_) { return true }
  }

  function _clearTimer() {
    try {
      if (_timer && engine?.schedule?.cancel) engine.schedule.cancel(_timer)
    } catch (_) {}
    _timer = null
  }
  ...
  function flush({ force = false, reason = 'flush' } = {}) {
    ...
    if (_txnDepth > 0 && !force) {
      try { engine?.emit?.('savePolicy:flushBlocked', { t: _nowIso(), reason: 'txn', depth: _txnDepth }) } catch (_) {}
      return false
    }

    if (!_isSafe() && !force) {
      // Retry later.
      try { engine?.emit?.('savePolicy:flushBlocked', { t: _nowIso(), reason: 'unsafe' }) } catch (_) {}
      _clearTimer()
      try {
        if (engine?.schedule?.after) {
          _timer = engine.schedule.after(retryMs, () => {
            _timer = null
            try { flush({ force: false, reason: 'retry' }) } catch (_) {}
          }, { owner })
        }
      } catch (_) {}
      return false
  ...
  function _doFlush(reason) {
    ...
    if (!_writer) {
      try { engine?.emit?.('savePolicy:flushFail', { t: _nowIso(), reason, error: 'no-writer' }) } catch (_) {}
      return
    }
    ...
    try { engine?.emit?.('savePolicy:flushStart', { t: started, reason, reasons }) } catch (_) {}
    ...
    if (ok) {
      ...
      try { engine?.emit?.('savePolicy:flushOk', { t: _lastFlush.t, reason }) } catch (_) {}
    } else {
      _dirty = true
      try { engine?.emit?.('savePolicy:flushFail', { t: _nowIso(), reason, error: (err && err.message) ? err.message : String(err) }) } catch (_) {}
      // Retry after a short delay.
      try {
        if (engine?.schedule?.after) {
          _timer = engine.schedule.after(retryMs, () => {
            _timer = null
            try { flush({ reason: 'retry' }) } catch (_) {}
          }, { owner })
        }
      } catch (_) {}
    }
