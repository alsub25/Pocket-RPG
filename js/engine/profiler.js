...
export function createProfiler({
  ...
  function _log(lvl, msg, data) {
    try { if (logger && logger[lvl]) logger[lvl]('perf', msg, data) } catch (_) {}
  }
  ...
  function begin(name) {
    ...
    return () => {
      ...
      if (ms > slowFrameMs) {
        ...
        try { if (emit) emit('perf:slowScope', { name, ms }) } catch (_) {}
      }
      return ms
    }
  }
  ...
  function _frameLoop() {
    ...
    if (lastFrameT != null) {
      ...
      if (dt > slowFrameMs) {
        mark('frame', dt)
        try { if (emit) emit('perf:slowFrame', { dt }) } catch (_) {}
      }
    }
    lastFrameT = t
    try { rafId = requestAnimationFrame(_frameLoop) } catch (_) { rafId = null }
  }
