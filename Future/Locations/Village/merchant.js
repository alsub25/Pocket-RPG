...
function getOrCreateMerchantShopName(state, merchantId, fallbackName) {
    ...
    if (names[merchantId]) return names[merchantId]
...
function getResourcePotionKeyForPlayer(state) {
    ...
    if (key === 'mana') return 'potionMana'
    if (key === 'fury') return 'potionFury'
    if (key === 'blood') return 'potionBlood'
    if (key === 'essence') return 'potionEssence'
    return null
}
...
function getMerchantStockBucket(state, context, merchantId) {
    if (!state.merchantStock) state.merchantStock = {}

    const ctxKey = context || 'village'
    if (!state.merchantStock[ctxKey]) state.merchantStock[ctxKey] = {}
...
function ensureMerchantStock(
    ...
    stockKeys.forEach((key) => {
        if (typeof bucket[key] !== 'number') {
            ...
            let qty = 3
            if (def && def.type === 'potion') qty = 6
            else if (def && def.type === 'weapon') qty = 2
            else if (def && def.type === 'armor') qty = 2
            bucket[key] = qty
        }
    })
...
// This runs once per in-game day (wired through Future.js' runDailyTicks).
//
// Behavior:
...
export function handleMerchantDayTick(state, absoluteDay, cloneItemDef) {
    ...
    const capFor = (def) => {
        if (def && def.type === 'potion') return 6
        if (def && (def.type === 'weapon' || def.type === 'armor')) return 2
        return 3
    }
...
function buildItemRow({
    ...
    btnBuy.addEventListener('click', () => {
        ...
        addLog(
            'You purchase ' + def.name + ' for ' + finalPrice + ' gold.',
            'good'
        )
...
function openVillageMerchantHub({
    state,
    openModal,
    addLog,
    ...
    openModal('Village Merchants', (body) => {
        ...
        VILLAGE_MERCHANTS.forEach((merchant) => {
            const runtimeName = getOrCreateMerchantShopName(
                state,
                merchant.id,
                merchant.baseName
            )
...
function openSpecificMerchantShop({
    merchantId,
    state,
    openModal,
    addLog,
    ...
    const stockMap = ensureMerchantStock(
        state,
        'village',
        merchantId,
        stockKeys,
...
function openTravelingMerchantShop({
    state,
    openModal,
    addLog,
    getVillageEconomySummary,
...
export function openMerchantModalImpl({
    context = 'village',
    state,
    openModal,
    addLog,
