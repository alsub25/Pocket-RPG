...
// --- AUDIO / MUSIC ---------------------------------------------------------
// Simple ambient music controller (one track at a time)
...
function initAudio() {
  ...
  const villageDay = new Audio("./Audio/village_day.wav");
  villageDay.loop = true;
  villageDay.volume = 0.4; // tweak to taste
...
function isDaytime(info) {
  // You can tighten this if you only want Morning:
  // return info.partIndex === 0;
  return info.partLabel !== "Night";
}
...
function updateAreaMusic() {
  ...
  initAudio(); // make sure tracks exist

  const info = getTimeInfo(state); // from timeSystem.js
  const area = state.area || "village";

  if (area === "village" && isDaytime(info)) {
    // Village + day → play village daytime loop
    playMusicTrack(audioState.tracks.villageDay);
  } else {
    // For now, silence outside that case
    playMusicTrack(null);
  }
}
...
function renderExploreActions(actionsEl) {
  ...
  if (showVillageMenu) {
    // Back button to restore the normal action bar

// Village-appropriate actions
...
actionsEl.appendChild(
  makeActionButton('Back', () => {
    ...
    // We only want the village submenu here, so stop.
    return;
  }

  // 🔹 DEFAULT (NON-VILLAGE or VILLAGE NORMAL BAR) ----------------------------
  // Explore uses the "sticky" choice handler
  ...
  // Lets you bring the area picker back any time
  actionsEl.appendChild(
  ...
  // Always available (view-only / meta)
  actionsEl.appendChild(
  ...
  // Only show Cheats button if dev toggle is enabled
  if (cheatsEnabled()) {
    actionsEl.appendChild(
...
function openInGameSettingsModal() {
  openModal('Settings', body => {
    ...
    volSlider.addEventListener('input', () => {
      ...
      // Hook into your audio system here if you want live volume changes.
    });
